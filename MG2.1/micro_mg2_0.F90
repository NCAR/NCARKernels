...
! real(r8), intent(out) :: prer_evap(mgncol,nlev)
  ...
  real(r8) :: ttmpv(mgncol)
  real(r8) :: qtmpv(mgncol)
     ...
     do i=1,mgncol
        ...
        if ((pre(i,k)+prds(i,k))*precip_frac(i,k)+ice_sublim(i,k) < -1.e-20_r8) then
           ...
           if (qtmpv(i) > qvn) then
              ...
              qtmpv(i)=q(i,k)-(vap_dep(i,k)+mnuccd(i,k))*deltat
              ttmpv(i)=t(i,k)+((vap_dep(i,k)+mnuccd(i,k))*xxls)*deltat/cpp

              ! use rhw to allow ice supersaturation
              call qsat_water(ttmpv(i), p(i,k), esn, qvn)

              dum=(qtmpv(i)-qvn)/(1._r8 + xxlv_squared*qvn/(cpp*rv*ttmpv(i)**2))
              dum=min(dum,0._r8)
              ...
              call qsat_ice(ttmpv(i), p(i,k), esn, qvn)

              dum=(qtmpv(i)-qvn)/(1._r8 + xxls_squared*qvn/(cpp*rv*ttmpv(i)**2))
              dum=min(dum,0._r8)
...
!dir$ vector aligned
    do i=1,mgncol
      ...
      dum_2D(i,k) = qs(i,k)
      qs(i,k) = qsn(i,k)
      qstend(i,k) = qstend(i,k) + (dum_2D_qs)/deltat

      dum_2D(i,k) = ns(i,k)
      ns(i,k) = nsn(i,k)
      nstend(i,k) = nstend(i,k) + (dum_2D_ns)/deltat

      dum_2D(i,k) = qr(i,k)
      qr(i,k) = qrn(i,k)
      qrtend(i,k) = qrtend(i,k) + (dum_2D_qr)/deltat

      dum_2D(i,k) = nr(i,k)
      nr(i,k) = nrn(i,k)
      nrtend(i,k) = nrtend(i,k) + (dum_2D_nr)/deltat
    enddo
  enddo
...
!dir$ vector aligned
     ...
     do n = 1,nstep
        ...
        faltndi = falouti(k)*pdel_inv(i,k)
        faltndni = faloutni(k)*pdel_inv(i,k)
        ...
        do k = 2,nlev
           ...
           faltndqie=(falouti(k)-falouti(k-1))*pdel_inv(i,k)
           faltndi=(falouti(k)-dum1*falouti(k-1))*pdel_inv(i,k)
           faltndni=(faloutni(k)-dum1*faloutni(k-1))*pdel_inv(i,k)
     ...
     do n = 1,nstep
        ...
        ! add fallout terms to microphysical tendencies
        faltndc = faloutc(k)*pdel_inv(i,k)
        faltndnc = faloutnc(k)*pdel_inv(i,k)
        ...
        do k = 2,nlev
           ...
           dum=min(dum,1._r8)
           faltndqce=(faloutc(k)-faloutc(k-1))*pdel_inv(i,k)
           faltndc=(faloutc(k)-dum*faloutc(k-1))*pdel_inv(i,k)
           faltndnc=(faloutnc(k)-dum*faloutnc(k-1))*pdel_inv(i,k)
     ...
     do n = 1,nstep
        ...
        ! add fallout terms to microphysical tendencies
        faltndr = faloutr(k)*pdel_inv(i,k)
        faltndnr = faloutnr(k)*pdel_inv(i,k)
...
!       qrsedten(i,k)=qrsedten(i,k)-faltndr/nstep
        ...
        dumr(i,k) = dumr(i,k)-faltndr*deltat/nstep
        dumnr(i,k) = dumnr(i,k)-faltndnr*deltat/nstep

        do k = 2,nlev

           faltndr=(faloutr(k)-faloutr(k-1))*pdel_inv(i,k)
           faltndnr=(faloutnr(k)-faloutnr(k-1))*pdel_inv(i,k)
...
!          qrsedten(i,k)=qrsedten(i,k)-faltndr/nstep
     ...
     do n = 1,nstep
        ...
        ! add fallout terms to microphysical tendencies
        faltnds = falouts(k)*pdel_inv(i,k)
        faltndns = faloutns(k)*pdel_inv(i,k)
...
!       qssedten(i,k)=qssedten(i,k)-faltnds/nstep
           ...
           dums(i,k) = dums(i,k)-faltnds*deltat/nstep
           dumns(i,k) = dumns(i,k)-faltndns*deltat/nstep

        do k = 2,nlev

           faltnds=(falouts(k)-falouts(k-1))*pdel_inv(i,k)
           faltndns=(faloutns(k)-faloutns(k-1))*pdel_inv(i,k)
