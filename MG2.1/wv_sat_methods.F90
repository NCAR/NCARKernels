...
interface wv_sat_qsat_water
   module procedure wv_sat_qsat_water_line
   module procedure wv_sat_qsat_water_vect
end interface

interface wv_sat_qsat_ice
   module procedure wv_sat_qsat_ice_vect
   module procedure wv_sat_qsat_ice_line
end interface
...
function wv_sat_svp_to_qsat_vect(es, p, mgncol) result(qs)

  integer,                     intent(in) :: mgncol
  real(r8), dimension(mgncol), intent(in) :: es  ! SVP
  real(r8), dimension(mgncol), intent(in) :: p   ! Current pressure.
...
end function wv_sat_svp_to_qsat_vect

subroutine wv_sat_qsat_water_line(t, p, es, qs, idx)
  !------------------------------------------------------------------!
  ! Purpose:                                                         !
...
end subroutine wv_sat_qsat_water_line

subroutine wv_sat_qsat_water_vect(t, p, es, qs, mgncol, idx)
  !------------------------------------------------------------------!
  ! Purpose:                                                         !
  !   Calculate SVP over water at a given temperature, and then      !
  !   calculate and return saturation specific humidity.             !
  ...
  ! Outputs
  real(r8), dimension(mgncol), intent(out) :: es  ! Saturation vapor pressure
  real(r8), dimension(mgncol), intent(out) :: qs  ! Saturation specific humidity

  integer,                     intent(in), optional :: idx ! Scheme index

  integer :: i

  es = wv_sat_svp_water_vect(t, mgncol, idx=idx)
  qs = wv_sat_svp_to_qsat_vect(es, p, mgncol)
...
subroutine wv_sat_qsat_ice_line(t, p, es, qs, idx)
  !------------------------------------------------------------------!
  ! Purpose:                                                         !
...
end subroutine wv_sat_qsat_ice_line

subroutine wv_sat_qsat_ice_vect(t, p, es, qs, mgncol, idx)
  !------------------------------------------------------------------!
  ! Purpose:                                                         !
  !   Calculate SVP over ice at a given temperature, and then        !
  !   calculate and return saturation specific humidity.             !
  ...
  es = wv_sat_svp_ice_vect(t, mgncol, idx=idx)
...
function wv_sat_svp_water_vect(t, mgncol, idx) result(es)
  ...
  select case (use_idx)
  case(GoffGratch_idx)
     es(i) = GoffGratch_svp_water_line(t(i))
  case(MurphyKoop_idx)
     es(i) = MurphyKoop_svp_water_line(t(i))
  case(OldGoffGratch_idx)
     es(i) = OldGoffGratch_svp_water_line(t(i))
  case(Bolton_idx)
     es(i) = Bolton_svp_water_line(t(i))
  end select
  enddo

end function wv_sat_svp_water_vect
...
function wv_sat_svp_ice_vect(t, mgncol, idx) result(es)
  integer, intent(in) :: mgncol
  real(r8), dimension(mgncol), intent(in) :: t
  integer,                     intent(in), optional :: idx
  real(r8), dimension(mgncol) :: es
  ...
  select case (use_idx)
  case(GoffGratch_idx)
     es(i) = GoffGratch_svp_ice_line(t(i))
  case(MurphyKoop_idx)
     es(i) = MurphyKoop_svp_ice_line(t(i))
  case(OldGoffGratch_idx)
     es(i) = OldGoffGratch_svp_ice_line(t(i))
  case(Bolton_idx)
     es(i) = Bolton_svp_water_line(t(i))
  end select
  enddo

end function wv_sat_svp_ice_vect
...
function GoffGratch_svp_water_line(t) result(es)
  real(r8), intent(in) :: t  ! Temperature in Kelvin
  real(r8) :: es             ! SVP in Pa
...
end function GoffGratch_svp_water_line
...
function GoffGratch_svp_ice_line(t) result(es)
  real(r8), intent(in) :: t  ! Temperature in Kelvin
  real(r8) :: es             ! SVP in Pa
...
end function GoffGratch_svp_ice_line
...
function MurphyKoop_svp_water_line(t) result(es)
  real(r8), intent(in) :: t  ! Temperature in Kelvin
  real(r8) :: es             ! SVP in Pa
...
end function MurphyKoop_svp_water_line
...
function MurphyKoop_svp_ice_line(t) result(es)
  real(r8), intent(in) :: t  ! Temperature in Kelvin
  real(r8) :: es             ! SVP in Pa
...
end function MurphyKoop_svp_ice_line
...
function OldGoffGratch_svp_water_line(t) result(es)
  real(r8), intent(in) :: t
  real(r8) :: es
  real(r8) :: ps, e1, e2, f1, f2, f3, f4, f5, f
...
end function OldGoffGratch_svp_water_line
...
function OldGoffGratch_svp_ice_line(t) result(es)
  real(r8), intent(in) :: t
  real(r8) :: es
  real(r8) :: term1, term2, term3
...
end function OldGoffGratch_svp_ice_line
...
function Bolton_svp_water_line(t) result(es)
  real(r8),parameter :: c1 = 611.2_r8
  real(r8),parameter :: c2 = 17.67_r8
  real(r8),parameter :: c3 = 243.5_r8
...
end function Bolton_svp_water_line
