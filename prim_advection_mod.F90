...
! KGEN version: 0.4.4
    ...
    MODULE prim_advection_mod
        ...
        SUBROUTINE euler_step(kgen_unit)
            ...
            REAL(KIND=real_kind), dimension(np,np  ,nlev) :: dp_star,dp_star_inv
            ...
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        ...
                        ! call to kernel
                        dp_star_inv(:,:,:) = 1.0/dp_star
                        CALL limiter_optim_iter_full(qtens(:, :, :), smaug(:, :), qmin(:, q, ie), qmax(:, q, ie), dp_star(:, :, :), dp_star_inv(:,:,:))
                        ...
                        DO kgen_intvar=1,10000000
                            CALL limiter_optim_iter_full(qtens(:, :, :), smaug(:, :), qmin(:, q, ie), qmax(:, q, ie), dp_star(:, :, :), dp_star_inv(:,:,:))
                        END DO
                        call system_clock(c2, cr, cm); dt1 = dble(c2-c1)/dble(cr)
                        WRITE(*,*)
                        PRINT *, "Elapsed time (sec): ", dt1
...
!   call t_startf('limiter_optim_iter_full')
    ...
    do k = 1 , nlev
      do k1 = 1, np * np
        ptens(k1,k) = ptens(k1,k) * dpmassi(k1,k)
        weights(k1,k) = sphweights(k1) * dpmass(k1,k)
      enddo
    enddo
