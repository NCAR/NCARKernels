...
! Variables related to random number and seed 
      ...
      integer, dimension(ncol*nsubcol*nlay) :: seed1_str, seed2_str, seed3_str, seed4_str  ! seed to create random number (kissvec)
...
!       do i=1,changeSeed
        ...
        enddo
      elseif ( irnd == 1) then
        rand_length = ncol*nsubcol*nlay
!$OMP CRITICAL
        call dSFMT_init(iseed, rand_length, rng )
!$OMP END CRITICAL
      endif 

      call system_clock(c2, cr, cm); dt1 = dble(c2-c1)/dble(cr) + dt1
      if ( masterproc) then
        print *, 'Total time RANDOM: LW_OPTS:dt1 =  ', dt1, changeSeed
      endif
...
! i) pick a random value at every level
         ...
         elseif ( irnd == 1) then
!$OMP CRITICAL
           call get_rand_arr_close_open( rng, rand_str, rand_length )
!$OMP END CRITICAL
         endif
         ...
         if ( masterproc) then
           print *, 'Total time RANDOM: LW_OPTS:dt2 =  ', dt2
         endif
...
!$OMP END CRITICAL
         ...
         if ( masterproc) then
           print *, 'Total time RANDOM: LW_OPTS:dt3 =  ', dt3
         endif
...
!$OMP END CRITICAL
         ...
         if ( masterproc) then
           print *, 'Total time RANDOM: LW_OPTS:dt4 =  ', dt4
         endif
