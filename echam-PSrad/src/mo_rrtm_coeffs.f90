...
! --------------------------------------------------------------------------------------------
  SUBROUTINE lrtm_coeffs( kproma, kbdim                              , &
    ...
    !
    do jk=1,klev 
       forfac(1:kproma,jk) =  forfac(1:kproma,jk) * colh2o(1:kproma,jk)
       selffac(1:kproma,jk) = selffac(1:kproma,jk) * colh2o(1:kproma,jk)
    enddo
...
!dir$ vector aligned
      do jl=1,kproma
         rat_h2oco2  (jl, jk) = chi_mls(1,jp(jl, jk)  )/chi_mls(2,jp(jl, jk)  )
         rat_h2oco2_1(jl, jk) = chi_mls(1,jp(jl, jk)+1)/chi_mls(2,jp(jl, jk)+1)
         !
         ! Needed only in lower atmos (plog > 4.56_wp) 
         !
         rat_h2oo3   (jl, jk) = chi_mls(1,jp(jl, jk)  )/chi_mls(3,jp(jl, jk)  )
         rat_h2oo3_1 (jl, jk) = chi_mls(1,jp(jl, jk)+1)/chi_mls(3,jp(jl, jk)+1)
         rat_h2on2o  (jl, jk) = chi_mls(1,jp(jl, jk)  )/chi_mls(4,jp(jl, jk)  )
         rat_h2on2o_1(jl, jk) = chi_mls(1,jp(jl, jk)+1)/chi_mls(4,jp(jl, jk)+1)
         rat_h2och4  (jl, jk) = chi_mls(1,jp(jl, jk)  )/chi_mls(6,jp(jl, jk)  )
         rat_h2och4_1(jl, jk) = chi_mls(1,jp(jl, jk)+1)/chi_mls(6,jp(jl, jk)+1)
         rat_n2oco2  (jl, jk) = chi_mls(4,jp(jl, jk)  )/chi_mls(2,jp(jl, jk)  )
         rat_n2oco2_1(jl, jk) = chi_mls(4,jp(jl, jk)+1)/chi_mls(2,jp(jl, jk)+1)
         !
         ! Needed only in upper atmos (plog <= 4.56_wp) 
         !
         rat_o3co2   (jl, jk) = chi_mls(3,jp(jl, jk)  )/chi_mls(2,jp(jl, jk)  )
         rat_o3co2_1 (jl, jk) = chi_mls(3,jp(jl, jk)+1)/chi_mls(2,jp(jl, jk)+1)         
      enddo 
    END DO 
...
!dir$ vector aligned
       do jl = 1,kproma
          scaleminor  (jl,jk) = play(jl,jk)/tlay(jl,jk)
          scaleminorn2(jl,jk) = scaleminor(jl,jk) * &
                                      (wbroad(jl,jk)/(coldry(jl,jk)+wkl(jl,1,jk)))
          factor(jl,jk) = (tlay(jl,jk)-180.8_wp)/7.2_wp
          indminor(jl,jk)     = MIN(18, MAX(1, INT(factor(jl,jk))))
          minorfrac(jl,jk)    = (tlay(jl,jk)-180.8_wp)/7.2_wp - FLOAT(indminor(jl,jk))
       enddo
    enddo
