...
CONTAINS
  ...
  !!
  !
  FUNCTION set_spec_sampling_lw(strategy, num_gpts_ts, l_unique) RESULT (this) 
    ...
    IF (strategy == 2) THEN
      ...
      IF(PRESENT(l_unique   )) this%unique      = l_unique
    END IF 
  ...
  !!
  !
  FUNCTION set_spec_sampling_sw(strategy, num_gpts_ts, l_unique) RESULT (this) 
    INTEGER, INTENT(IN) :: strategy
    INTEGER, OPTIONAL, &
             INTENT(IN) :: num_gpts_ts
    LOGICAL, OPTIONAL, &
