...
MODULE mo_srtm_driver
  ...
  USE mo_radiation_parameters, &
                             ONLY : i_overlap, l_do_sep_clear_sky, rad_undef, do_gpoint
...
CONTAINS
  ...
  SUBROUTINE srtm( krow, ncid, ncvarids,  kproma, &
...
integer, intent(in) :: krow, ncid, ncvarids(:)
...
!        call nc_check(nf90_put_var(ncid, ncvarids(1), ztaut(:,:,ig), start = (/1, 1, ig, krow/)))
!        call nc_check(nf90_put_var(ncid, ncvarids(2), zomgt(:,:,ig), start = (/1, 1, ig, krow/)))
!        call nc_check(nf90_put_var(ncid, ncvarids(3), zasyt(:,:,ig), start = (/1, 1, ig, krow/)))
!        call nc_check(nf90_put_var(ncid, ncvarids(4), Rdir, start = (/1, 1, ig, krow/)))
!        call nc_check(nf90_put_var(ncid, ncvarids(5), Tdir, start = (/1, 1, ig, krow/)))
!        call nc_check(nf90_put_var(ncid, ncvarids(6), Rdif, start = (/1, 1, ig, krow/)))
!        call nc_check(nf90_put_var(ncid, ncvarids(7), Tdif, start = (/1, 1, ig, krow/)))
!        IF(ig == 1) call nc_check(nf90_put_var(ncid, ncvarids(8), SPREAD(cossza(:),DIM=2,NCOPIES=klev),start = (/1, 1, krow/)))
