...
!dir$ vector aligned
          do jl=1,kproma
             uflx (jl,jk) = uflx (jl,jk) &
                             + zgpfu(jl,jk) * gpt_scaling
             dflx (jl,jk) = dflx (jl,jk) &
                             + zgpfd(jl,jk) * gpt_scaling
          enddo
      enddo
...
!dir$ vector aligned
           do jl=1,kproma
              uflxc(jl,jk) = uflxc(jl,jk) + zgpcu(jl,jk) * gpt_scaling
              dflxc(jl,jk) = dflxc(jl,jk) + zgpcd(jl,jk) * gpt_scaling
           enddo
        enddo
      ELSE
        !
