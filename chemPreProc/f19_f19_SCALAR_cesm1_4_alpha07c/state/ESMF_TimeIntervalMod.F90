...
! !USES:
      ...
      USE esmf_shrtimemod, ONLY: kw_esmf_shrtimemod_esmf_time
      implicit none
!
!------------------------------------------------------------------------------
! !PRIVATE TYPES:
...
!==============================================================================
      ...
      PUBLIC kw_esmf_timeintervalmod_esmf_timeinterval
      contains
...
!read state subroutine for kw_esmf_timeintervalmod_esmf_timeinterval
RECURSIVE SUBROUTINE kw_esmf_timeintervalmod_esmf_timeinterval(var, kgen_unit, printvar)
    TYPE(esmf_timeinterval), INTENT(IN) :: var
    ...
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%mm
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%mm **" // NEW_LINE("A"), var%mm
    END IF 
    
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%starttime_set
    IF (PRESENT( printvar )) THEN
...
END SUBROUTINE kw_esmf_timeintervalmod_esmf_timeinterval
