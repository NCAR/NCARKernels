...
!
    ...
    USE mo_imp_sol, ONLY: kw_externs_in_mo_imp_sol
    USE mo_imp_sol, ONLY: kw_externs_out_mo_imp_sol
    USE chem_mods, ONLY: kw_externs_in_chem_mods
    USE time_manager, ONLY: kw_externs_in_time_manager
    USE time_manager, ONLY: kw_externs_out_time_manager
    USE cam_logfile, ONLY: kw_externs_in_cam_logfile
    USE shr_log_mod, ONLY: kw_externs_in_shr_log_mod
    USE mo_tracname, ONLY: kw_externs_in_mo_tracname
    implicit none
    ...
    DO WHILE( kgen_cur_rank < kgen_mpi_size )
        IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
            IF (ANY(kgen_counter == kgen_counter_at)) THEN
                ...
                kgen_filepath = "/glade/p/work/mickelso/cesm_sandbox/WACCM_pp/kgen/kernels/f19_f19_SCALAR_cesm1_4_alpha07c/kernel/imp_sol." // TRIM(ADJUSTL(kgen_counter_conv)) // "." // TRIM(ADJUSTL(kgen_mpi_rank_conv))
                ...
                WRITE (UNIT = kgen_unit) delt
                
                !extern input variables
                CALL kw_externs_in_mo_imp_sol(kgen_unit)
                CALL kw_externs_in_chem_mods(kgen_unit)
                CALL kw_externs_in_time_manager(kgen_unit)
                CALL kw_externs_in_cam_logfile(kgen_unit)
                CALL kw_externs_in_shr_log_mod(kgen_unit)
                CALL kw_externs_in_mo_tracname(kgen_unit)
                
                !local input variables
                IF (SIZE(invariants)==1) THEN
                    IF (UBOUND(invariants, 1)<LBOUND(invariants, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(invariants, 1)==0 .AND. LBOUND(invariants, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(invariants)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                ...
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(invariants), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) invariants
                END IF 
                IF (SIZE(extfrc)==1) THEN
                    IF (UBOUND(extfrc, 1)<LBOUND(extfrc, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(extfrc, 1)==0 .AND. LBOUND(extfrc, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(extfrc)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(extfrc), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) extfrc
                END IF 
                IF (SIZE(vmr)==1) THEN
                    IF (UBOUND(vmr, 1)<LBOUND(vmr, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(vmr, 1)==0 .AND. LBOUND(vmr, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(vmr)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(vmr), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) vmr
                END IF 
                IF (SIZE(reaction_rates)==1) THEN
                    IF (UBOUND(reaction_rates, 1)<LBOUND(reaction_rates, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(reaction_rates, 1)==0 .AND. LBOUND(reaction_rates, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(reaction_rates)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(reaction_rates), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) reaction_rates
                END IF 
                IF (SIZE(het_rates)==1) THEN
                    IF (UBOUND(het_rates, 1)<LBOUND(het_rates, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(het_rates, 1)==0 .AND. LBOUND(het_rates, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(het_rates)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(het_rates), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) het_rates
                END IF 
                IF (SIZE(ltrop_sol)==1) THEN
                    IF (UBOUND(ltrop_sol, 1)<LBOUND(ltrop_sol, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(ltrop_sol, 1)==0 .AND. LBOUND(ltrop_sol, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(ltrop_sol)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(ltrop_sol), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) ltrop_sol
                END IF 
                IF (SIZE(o3s_loss)==1) THEN
                    IF (UBOUND(o3s_loss, 1)<LBOUND(o3s_loss, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(o3s_loss, 1)==0 .AND. LBOUND(o3s_loss, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(o3s_loss)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(o3s_loss), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) o3s_loss
                END IF 
            END IF 
        END IF 
    ...
    DO WHILE( kgen_cur_rank < kgen_mpi_size )
        IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
            IF (ANY(kgen_counter == kgen_counter_at)) THEN
                ...
                CALL kw_externs_out_mo_imp_sol(kgen_unit)
                CALL kw_externs_out_time_manager(kgen_unit)
