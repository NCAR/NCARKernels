...
!==============================================================================
      ...
      PUBLIC kw_esmf_calendarmod_esmf_calkind_flag
      PUBLIC kw_esmf_calendarmod_esmf_daysperyear
      PUBLIC kw_esmf_calendarmod_esmf_calendar
      contains
...
!==============================================================================
!read state subroutine for kw_esmf_calendarmod_esmf_calkind_flag
RECURSIVE SUBROUTINE kw_esmf_calendarmod_esmf_calkind_flag(var, kgen_unit, printvar)
    TYPE(esmf_calkind_flag), INTENT(IN) :: var
    ...
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%caltype
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%caltype **" // NEW_LINE("A"), var%caltype
    END IF 
    
END SUBROUTINE kw_esmf_calendarmod_esmf_calkind_flag

!read state subroutine for kw_esmf_calendarmod_esmf_daysperyear
RECURSIVE SUBROUTINE kw_esmf_calendarmod_esmf_daysperyear(var, kgen_unit, printvar)
    TYPE(esmf_daysperyear), INTENT(IN) :: var
    INTEGER, INTENT(IN) :: kgen_unit
    CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
    LOGICAL :: kgen_istrue
    REAL(KIND=8) :: kgen_array_sum
    ...
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%dn
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%dn **" // NEW_LINE("A"), var%dn
    END IF 
    
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%dd
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%dd **" // NEW_LINE("A"), var%dd
    END IF 
    
END SUBROUTINE kw_esmf_calendarmod_esmf_daysperyear

!read state subroutine for kw_esmf_calendarmod_esmf_calendar
RECURSIVE SUBROUTINE kw_esmf_calendarmod_esmf_calendar(var, kgen_unit, printvar)
    TYPE(esmf_calendar), INTENT(IN) :: var
    INTEGER, INTENT(IN) :: kgen_unit
    CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
    LOGICAL :: kgen_istrue
    REAL(KIND=8) :: kgen_array_sum
    ...
    IF (PRESENT( printvar )) THEN
        CALL kw_esmf_calendarmod_esmf_calkind_flag(var%type, kgen_unit, printvar // "%type")
    ELSE
        CALL kw_esmf_calendarmod_esmf_calkind_flag(var%type, kgen_unit)
    END IF 
    
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%set
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%set **" // NEW_LINE("A"), var%set
    END IF 
    
    IF (SIZE(var%dayspermonth)==1) THEN
        IF (UBOUND(var%dayspermonth, 1)<LBOUND(var%dayspermonth, 1)) THEN
            kgen_istrue = .FALSE.
        ELSE IF (UBOUND(var%dayspermonth, 1)==0 .AND. LBOUND(var%dayspermonth, 1)==0) THEN
            kgen_istrue = .FALSE.
        ELSE
            kgen_istrue = .TRUE.
        END IF 
    ELSE IF (SIZE(var%dayspermonth)==0) THEN
        kgen_istrue = .FALSE.
    ELSE
        kgen_istrue = .TRUE.
    END IF 
    ...
    IF (kgen_istrue) THEN
        kgen_array_sum = REAL(SUM(var%dayspermonth), 8)
        WRITE (UNIT = kgen_unit) kgen_array_sum
        WRITE (UNIT = kgen_unit) var%dayspermonth
        IF (PRESENT( printvar )) THEN
    ...
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%secondsperday
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%secondsperday **" // NEW_LINE("A"), var%secondsperday
    END IF 
    
    kgen_istrue = .TRUE.
    WRITE (UNIT = kgen_unit) var%secondsperyear
    IF (PRESENT( printvar )) THEN
        WRITE (*, *) "** KGEN DEBUG: " // printvar // "%secondsperyear **" // NEW_LINE("A"), var%secondsperyear
    END IF 
    
    IF (PRESENT( printvar )) THEN
        CALL kw_esmf_calendarmod_esmf_daysperyear(var%daysperyear, kgen_unit, printvar // "%daysperyear")
    ELSE
        CALL kw_esmf_calendarmod_esmf_daysperyear(var%daysperyear, kgen_unit)
    END IF 
    
END SUBROUTINE kw_esmf_calendarmod_esmf_calendar
