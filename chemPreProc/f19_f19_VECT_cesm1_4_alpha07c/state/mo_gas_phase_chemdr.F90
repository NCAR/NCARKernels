module mo_gas_phase_chemdr
  ...
  use cam_history,      only : fieldname_len, add_default
...
!
    ...
    real(r8)     ::  reaction_rates_chnks(ncol*pver,max(1,rxntot))      ! reaction rates
    integer :: chnkpnts
    real(r8)     ::  depvel(ncol,gas_pcnst)                ! dry deposition velocity (cm/s)
    real(r8)     ::  het_rates(ncol,pver,max(1,gas_pcnst)) ! washout rate (1/s)
    real(r8)     ::  het_rates_chnks(ncol*pver,max(1,gas_pcnst)) ! washout rate (1/s)
...
!!$    call exp_sol( vmr, reaction_rates_chnks, het_rates, extfrc, delt,  ncol, lchnk, chnkpnts )
    ...
    !
    reaction_rates_chnks = reshape( reaction_rates, (/chnkpnts,max(1,rxntot)/) )
    het_rates_chnks = reshape( het_rates, (/chnkpnts,max(1,gas_pcnst)/) )
    ...
    DO WHILE( kgen_cur_rank < kgen_mpi_size )
        IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
            IF (ANY(kgen_counter == kgen_counter_at)) THEN
                ...
                kgen_filepath = "/glade/p/work/mickelso/cesm_sandbox/WACCM_pp/kgen/kernels/f19_f19_VECT_cesm1_4_alpha07c/kernel/imp_sol." // TRIM(ADJUSTL(kgen_counter_conv)) // "." // TRIM(ADJUSTL(kgen_mpi_rank_conv))
                ...
                END IF 
                IF (SIZE(reaction_rates_chnks)==1) THEN
                    IF (UBOUND(reaction_rates_chnks, 1)<LBOUND(reaction_rates_chnks, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(reaction_rates_chnks, 1)==0 .AND. LBOUND(reaction_rates_chnks, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(reaction_rates_chnks)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(reaction_rates_chnks), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) reaction_rates_chnks
                END IF 
                WRITE (UNIT = kgen_unit) chnkpnts
                IF (SIZE(het_rates_chnks)==1) THEN
                    IF (UBOUND(het_rates_chnks, 1)<LBOUND(het_rates_chnks, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(het_rates_chnks, 1)==0 .AND. LBOUND(het_rates_chnks, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(het_rates_chnks)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    kgen_array_sum = REAL(SUM(het_rates_chnks), 8)
                    WRITE (UNIT = kgen_unit) kgen_array_sum
                    WRITE (UNIT = kgen_unit) het_rates_chnks
                END IF 
            END IF 
        END IF 
    ...
    !$OMP END MASTER
    reaction_rates = reshape( reaction_rates_chnks, (/ncol,pver,max(1,rxntot)/) )
