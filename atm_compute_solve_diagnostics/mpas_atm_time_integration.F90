...
! KGEN version: 0.5.0
    ...
    MODULE atm_time_integration
        ...
        SUBROUTINE atm_srk3(dt, myrank, kgen_unit)
            ...
            REAL(KIND=rkind), dimension(:,:), allocatable :: divergence_new
            ...
            ! rtheta_p
                            ...
                            ! call to kernel
                  veclen = 55
                  nkblocks = ceiling(real(nVertLevels,kind=rkind)/real(veclen,kind=rkind))
                  kstart = 1

                  do ikb=1,nkblocks
                     kend = min(nVertLevels, (kstart + veclen - 1))
                     call atm_compute_solve_diagnostics(kstart,kend,nVertLevels,nEdges,nCells,nVertices, vertexDegree,nEdgesOnCell, nEdgesOnEdge, &
                                                     dt, 2, &
                  ...
                  enddo
                            ...
                            allocate(divergence_new(SIZE(divergence,dim=1),SIZE(divergence,dim=2)))
                            divergence_new(:,:) = divergence(:,:)
                            DO kgen_intvar=1,maxiter
				kstart = 1
				do ikb=1,nkblocks
				   kend = min(nVertLevels, (kstart + veclen - 1))
                                   CALL atm_compute_solve_diagnostics(kstart,kend,nvertlevels, nedges, ncells, &
nvertices, vertexdegree, nedgesoncell, nedgesonedge, dt, 2, cellthreadstart_jmd, cellthreadend_jmd, &
vertexthreadstart_jmd, vertexthreadend_jmd, edgethreadstart_jmd, edgethreadend_jmd, edgesoncell, &
edgesonedge, edgesonvertex, cellsonedge, cellsonvertex, verticesoncell, verticesonedge, kiteforcell, &
fvertex, fedge, areacell, areatriangle, invareacell, edgesonvertex_sign, edgesoncell_sign, weightsonedge, &
kiteareasonvertex, dcedge, dvedge, pv_edge, pv_vertex, pv_cell, gradpvt, gradpvn, u_jmd, v_jmd, vh, h_edge, &
h, vorticity, ke, divergence_new, config_apvm_upwinding, rk_step)
				   kstart = kend+1
				enddo
                            END DO
...
!DIR$ ATTRIBUTES NOINLINE :: atm_compute_solve_diagnostics
        SUBROUTINE atm_compute_solve_diagnostics(kstart,kend, nvertlevels, nedges, ncells, nvertices, vertexdegree, nedgesoncell, nedgesonedge,&
        ...
        pv_cell, gradpvt, gradpvn, u, v, vh, h_edge, h, vorticity, ke, divergence, config_apvm_upwinding, rk_step)
            ...
            REAL(KIND=rkind), dimension(:,:) :: vh, h_edge, h, vorticity, ke, divergence
      ...
      do iVertex=vertexStart,vertexEnd
         vorticity(kstart:kend,iVertex) = 0.0
         do i=1,vertexDegree
      ...
      do iCell=cellStart,cellEnd
         divergence(kstart:kend,iCell) = 0.0
