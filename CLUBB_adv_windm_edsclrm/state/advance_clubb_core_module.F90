      ...
      DO WHILE( kgen_cur_rank < kgen_mpi_size )
          IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
              IF (ANY(kgen_counter == kgen_counter_at)) THEN
                  ...
                  kgen_filepath = "/glade/u/home/youngsun/trepo/temp/kgen_system_test/branches/initial/test_cases/CLUBB_adv_windm_edsclrm/kernel/advance_windm_edsclrm." // TRIM(ADJUSTL(kgen_counter_conv)) // "." // TRIM(ADJUSTL(kgen_mpi_rank_conv))
                  ...
                  WRITE (UNIT = kgen_unit) l_implemented
                  WRITE (UNIT = kgen_unit) dt
                  WRITE (UNIT = kgen_unit) fcor
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(wm_zt, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(um_forcing, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(vm_forcing, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(rho_ds_zm, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(invrs_rho_ds_zt, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim2(edsclrm_forcing, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(wp2, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(up2, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(vp2, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(um, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(vm, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(upwp, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim1(vpwp, kgen_unit)
                  CALL kw_advance_clubb_core_real__core_rknd_dim2(edsclrm, kgen_unit)
                  ...
                  IF (SIZE(km_zm)==1) THEN
                      IF (UBOUND(km_zm, 1)<LBOUND(km_zm, 1)) THEN
                          kgen_istrue = .FALSE.
                      ELSE IF (UBOUND(km_zm, 1)==0 .AND. LBOUND(km_zm, 1)==0) THEN
                          kgen_istrue = .FALSE.
                      ELSE
                          kgen_istrue = .TRUE.
                      END IF 
                  ELSE IF (SIZE(km_zm)==0) THEN
                      kgen_istrue = .FALSE.
                  ELSE
                      kgen_istrue = .TRUE.
                  END IF 
                  ...
                  IF (kgen_istrue) THEN
                      kgen_array_sum = REAL(SUM(km_zm), 8)
                      WRITE (UNIT = kgen_unit) kgen_array_sum
                      WRITE (UNIT = kgen_unit) km_zm
                  END IF 
              END IF 
          END IF 
      ...
      DO WHILE( kgen_cur_rank < kgen_mpi_size )
          IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
              IF (ANY(kgen_counter == kgen_counter_at)) THEN
                  ...
                  IF (SIZE(um)==1) THEN
                      IF (UBOUND(um, 1)<LBOUND(um, 1)) THEN
                          kgen_istrue = .FALSE.
                      ELSE IF (UBOUND(um, 1)==0 .AND. LBOUND(um, 1)==0) THEN
                          kgen_istrue = .FALSE.
                      ELSE
                          kgen_istrue = .TRUE.
                      END IF 
                  ELSE IF (SIZE(um)==0) THEN
                      kgen_istrue = .FALSE.
                  ELSE
                      kgen_istrue = .TRUE.
                  END IF 
                  WRITE (UNIT = kgen_unit) kgen_istrue
                  IF (kgen_istrue) THEN
                      kgen_array_sum = REAL(SUM(um), 8)
                      WRITE (UNIT = kgen_unit) kgen_array_sum
                      WRITE (UNIT = kgen_unit) um
                  END IF 
                  IF (SIZE(vm)==1) THEN
                      IF (UBOUND(vm, 1)<LBOUND(vm, 1)) THEN
                          kgen_istrue = .FALSE.
                      ELSE IF (UBOUND(vm, 1)==0 .AND. LBOUND(vm, 1)==0) THEN
                          kgen_istrue = .FALSE.
                      ELSE
                          kgen_istrue = .TRUE.
                      END IF 
                  ELSE IF (SIZE(vm)==0) THEN
                      kgen_istrue = .FALSE.
                  ELSE
                      kgen_istrue = .TRUE.
                  END IF 
                  WRITE (UNIT = kgen_unit) kgen_istrue
                  IF (kgen_istrue) THEN
                      kgen_array_sum = REAL(SUM(vm), 8)
                      WRITE (UNIT = kgen_unit) kgen_array_sum
                      WRITE (UNIT = kgen_unit) vm
                  END IF 
                  IF (SIZE(upwp)==1) THEN
                      IF (UBOUND(upwp, 1)<LBOUND(upwp, 1)) THEN
                          kgen_istrue = .FALSE.
                      ELSE IF (UBOUND(upwp, 1)==0 .AND. LBOUND(upwp, 1)==0) THEN
                          kgen_istrue = .FALSE.
                      ELSE
                          kgen_istrue = .TRUE.
                      END IF 
                  ELSE IF (SIZE(upwp)==0) THEN
                      kgen_istrue = .FALSE.
                  ELSE
                      kgen_istrue = .TRUE.
                  END IF 
                  WRITE (UNIT = kgen_unit) kgen_istrue
                  IF (kgen_istrue) THEN
                      kgen_array_sum = REAL(SUM(upwp), 8)
                      WRITE (UNIT = kgen_unit) kgen_array_sum
                      WRITE (UNIT = kgen_unit) upwp
                  END IF 
                  IF (SIZE(vpwp)==1) THEN
                      IF (UBOUND(vpwp, 1)<LBOUND(vpwp, 1)) THEN
                          kgen_istrue = .FALSE.
                      ELSE IF (UBOUND(vpwp, 1)==0 .AND. LBOUND(vpwp, 1)==0) THEN
                          kgen_istrue = .FALSE.
                      ELSE
                          kgen_istrue = .TRUE.
                      END IF 
                  ELSE IF (SIZE(vpwp)==0) THEN
                      kgen_istrue = .FALSE.
                  ELSE
                      kgen_istrue = .TRUE.
                  END IF 
                  WRITE (UNIT = kgen_unit) kgen_istrue
                  IF (kgen_istrue) THEN
                      kgen_array_sum = REAL(SUM(vpwp), 8)
                      WRITE (UNIT = kgen_unit) kgen_array_sum
                      WRITE (UNIT = kgen_unit) vpwp
                  END IF 
                  IF (SIZE(edsclrm)==1) THEN
                      IF (UBOUND(edsclrm, 1)<LBOUND(edsclrm, 1)) THEN
                          kgen_istrue = .FALSE.
                      ELSE IF (UBOUND(edsclrm, 1)==0 .AND. LBOUND(edsclrm, 1)==0) THEN
                          kgen_istrue = .FALSE.
                      ELSE
                          kgen_istrue = .TRUE.
                      END IF 
                  ELSE IF (SIZE(edsclrm)==0) THEN
                      kgen_istrue = .FALSE.
                  ELSE
                      kgen_istrue = .TRUE.
                  END IF 
                  WRITE (UNIT = kgen_unit) kgen_istrue
                  IF (kgen_istrue) THEN
                      kgen_array_sum = REAL(SUM(edsclrm), 8)
                      WRITE (UNIT = kgen_unit) kgen_array_sum
                      WRITE (UNIT = kgen_unit) edsclrm
                  END IF 
                  WRITE (UNIT = kgen_unit) err_code
                  ENDFILE (kgen_unit)
                  CLOSE (kgen_unit)
