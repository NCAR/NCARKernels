...
! KGEN version: 0.5.3
    ...
    USE stat_file_module, ONLY : kgen_read_mod10 => kgen_read
    USE stat_file_module, ONLY : kgen_verify_mod10 => kgen_verify
! Description:
...
! Data for GrADS / netCDF output
    ...
    SUBROUTINE kgen_read_stats(var, kgen_unit, printvar)
        ...
        END IF
        IF ( PRESENT(printvar) ) THEN
            CALL kgen_read_mod10(var%file, kgen_unit, printvar=printvar//"%file")
        ELSE
            CALL kgen_read_mod10(var%file, kgen_unit)
        END IF
    END SUBROUTINE
    RECURSIVE SUBROUTINE kgen_verify_stats(varname, check_status, var, ref_var)
        ...
        CALL kgen_verify_mod10("file", dtype_check_status, var%file, ref_var%file)
