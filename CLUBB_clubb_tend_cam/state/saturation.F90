...
!$omp threadprivate(svp_liq_lookup_table)
  ...
  PUBLIC kw_externs_in_saturation 
  PUBLIC kw_externs_out_saturation 
  contains
...
!-------------------------------------------------------------------------
  ...
  !write in state subroutine for kw_externs_in_saturation 
  SUBROUTINE kw_externs_in_saturation(kgen_unit) 
      INTEGER, INTENT(IN) :: kgen_unit 
      LOGICAL :: kgen_istrue 
      REAL(KIND=8) :: kgen_array_sum 
        
      IF (SIZE(svp_liq_lookup_table)==1) THEN 
          IF (UBOUND(svp_liq_lookup_table, 1)<LBOUND(svp_liq_lookup_table, 1)) THEN 
              kgen_istrue = .FALSE. 
          ELSE IF (UBOUND(svp_liq_lookup_table, 1)==0 .AND. LBOUND(svp_liq_lookup_table, 1)==0) THEN 
              kgen_istrue = .FALSE. 
          ELSE 
              kgen_istrue = .TRUE. 
          END IF   
      ELSE IF (SIZE(svp_liq_lookup_table)==0) THEN 
          kgen_istrue = .FALSE. 
      ELSE 
          kgen_istrue = .TRUE. 
      END IF   
      ...
      IF (kgen_istrue) THEN 
          kgen_array_sum = REAL(SUM(svp_liq_lookup_table, mask=(svp_liq_lookup_table .eq. svp_liq_lookup_table)), 8) 
          WRITE (UNIT = kgen_unit) kgen_array_sum 
          WRITE (UNIT = kgen_unit) svp_liq_lookup_table 
      END IF   
  END SUBROUTINE kw_externs_in_saturation 
    
  !write out state subroutine for kw_externs_out_saturation 
  SUBROUTINE kw_externs_out_saturation(kgen_unit) 
      INTEGER, INTENT(IN) :: kgen_unit 
      LOGICAL :: kgen_istrue 
      REAL(KIND=8) :: kgen_array_sum 
        
  END SUBROUTINE kw_externs_out_saturation 
