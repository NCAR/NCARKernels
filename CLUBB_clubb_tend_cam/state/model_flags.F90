...
!$omp threadprivate( l_const_Nc_in_cloud, l_fix_chi_eta_correlations )
  ...
  PUBLIC kw_externs_in_model_flags 
  contains
...
!-------------------------------------------------------------------------------
  ...
  !write in state subroutine for kw_externs_in_model_flags 
  SUBROUTINE kw_externs_in_model_flags(kgen_unit) 
      INTEGER, INTENT(IN) :: kgen_unit 
      LOGICAL :: kgen_istrue 
      REAL(KIND=8) :: kgen_array_sum 
        
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_diffuse_rtm_and_thlm 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_stability_correct_kh_n2_zm 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_upwind_wpxp_ta 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_upwind_xpyp_ta 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_upwind_xm_ma 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_quintic_poly_interp 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_tke_aniso 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_uv_nudge 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_rtm_nudge 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_trapezoidal_rule_zm 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_trapezoidal_rule_zt 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_single_c2_skw 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_call_pdf_closure_twice 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_standard_term_ta 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_host_applies_sfc_fluxes 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) l_use_cloud_cover 
      kgen_istrue = .TRUE. 
      WRITE (UNIT = kgen_unit) saturation_formula 
  END SUBROUTINE kw_externs_in_model_flags 
