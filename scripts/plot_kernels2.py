    ...
    for platform, test in tests.items():
        ...
        for casename, case in test['cases'].items():
            if casename in ref_etime:
                ...
                plot_etime[casename][platform_labels.index(platform)] =  avg_etime
...
#                plot_diff[casename][platform_labels.index(platform)] =  avg_diff
    ...
    j = -3 
    for platform, test in tests.items():
        ax.text(1, j, platform, fontsize=SUBTITLE_SIZE)
        j -= 1
    ...
    ax.text(0.5, 0.5, 'Execution-time Comparison between', fontsize=TITLE_SIZE, \
        horizontalalignment='center', verticalalignment='center')
    ax.text(0.5, 0.6, 'Intel SandyBridge, Haswell, KNC, and KNL', fontsize=TITLE_SIZE, \
        horizontalalignment='center', verticalalignment='center')
    ax.axis('off')
    fig.tight_layout()
    pdf.savefig(fig)
