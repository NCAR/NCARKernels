...
try:
    ...
    pdf = PdfPages('kgen_kernels2.pdf')
except:
...
def main():
    ...
    for in_test in in_tests:
        try:
            ...
            compiler = in_test[u'compiler'].split('\n')[0]

            csplit = compiler.split()
            if not csplit: continue
            compiler_label = csplit[0]
            compiler_labels.append(compiler_label)
            ...
            for in_testpath, in_testcase in in_test[u'cases'].items():
                ...
                for line in in_testcase[u'perf_stat'].split('\n'):
                    ...
                    try:
#                        if event in [ 'task-clock (usec)', 'context-switches', 'cpu-migrations', 'page-faults', \
        ...
        for casename, case in test['cases'].items():
            if casename in ref_etime:
                ...
                if not plot_etime.has_key(casename):
                    plot_etime[casename] = [ 0.0 ] * len(compiler_labels)
                avg_etime = sum(case['etime']) / len(case['etime'])
                #plot_etime[casename][compiler_labels.index(compiler)] =  ref_etime[casename] / avg_etime
                plot_etime[casename][compiler_labels.index(compiler)] =  avg_etime

                # perf events
                if not plot_perf.has_key(casename):
                    #plot_perf[casename] = [ {} ] * len(compiler_labels)
                    plot_perf[casename] = []
                    for _ in range(len(compiler_labels)): 
                        plot_perf[casename].append({})
                for event, evals in case['perf'].items():
                    plot_perf[casename][compiler_labels.index(compiler)][event] = evals
                    if not plot_perf_minmax.has_key(compiler):
                        plot_perf_minmax[compiler] = {}
                    if not plot_perf_minmax[compiler].has_key(event):
                        plot_perf_minmax[compiler][event] = [ float("inf"), float("-inf"),float("inf"),float("-inf") ]
                    plot_perf_minmax[compiler][event][0] = min(plot_perf_minmax[compiler][event][0], evals[0])
                    plot_perf_minmax[compiler][event][1] = max(plot_perf_minmax[compiler][event][1], evals[0])
                    plot_perf_minmax[compiler][event][2] = min(plot_perf_minmax[compiler][event][2], evals[1])
                    plot_perf_minmax[compiler][event][3] = max(plot_perf_minmax[compiler][event][3], evals[1])

#            if not plot_diff.has_key(casename):
#                plot_diff[casename] = [ math.float('nan') ] * len(compiler_labels)
...
#                plot_diff[casename][compiler_labels.index(compiler)] =  avg_diff
    ...
    ax.axis([0, 1, 1, 0])
    ax.text(0.5, 0.3, 'KGen Kernels Performance Evaluation', fontsize=TITLE_SIZE, \
        horizontalalignment='center', verticalalignment='center')
    ax.text(0.5, 0.4, 'using the latest PGI and Intel Compilers', fontsize=TITLE_SIZE, \
        horizontalalignment='center', verticalalignment='center')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.axis([0, 1, 1, 0])
    ax.text(0.5, 0.5, 'Execution-time Comparison between', fontsize=TITLE_SIZE, \
        horizontalalignment='center', verticalalignment='center')
    ...
    for i, compiler in enumerate(compiler_labels):
        ...
        y = plot_sumetime[i]
        ylim = max(ylim, y)
        barplot = plt.bar(i + interval/2 - width/2, y, width, color=colors[compiler])
        if y<100:
            plt.text(i + interval/2, y, "{:5.2f}".format(y), ha='center', va='bottom')
        else:
            plt.text(i + interval/2, y, "{:5.0f}".format(y), ha='center', va='bottom')
        xticks.append(i + interval/2)
    #plt.title('KGen kernels(%d) - combined'%used_kernels, fontsize=TITLE_SIZE)
    plt.title('Total sum of kernel execution time', fontsize=TITLE_SIZE)
    plt.ylim([ 0.0, ylim*1.1 ])
    plt.xlabel('compiler', fontsize=LABEL_SIZE)
    plt.ylabel('Total sum of execution time(usec)', fontsize=LABEL_SIZE)
...
#    pdf.savefig(fig)
    ...
    for cidx, (casename, etime) in enumerate(plot_etime_list):
        ...
        for i in range(len(compiler_labels)):
            barplot = plt.bar(i + interval/2 - width/2, etime[i], width, color=colors[compiler_labels[i]])
            if etime[i]>0:
                if etime[i]<100:
                    plt.text(i + interval/2, etime[i],  "{:5.2f}".format(etime[i]), ha='center', va='bottom')
                else:
                    plt.text(i + interval/2, etime[i],  "{:5.0f}".format(etime[i]), ha='center', va='bottom')
            xticks.append(i + interval/2)
        #plt.legend(barplots, compiler_labels)
        if len(casename)>40:
            plt.title('(%d) ...%s'%(cidx,casename[-40:]), fontsize=TITLE_SIZE)
        else:
            plt.title('(%d) %s'%(cidx,casename), fontsize=TITLE_SIZE)
        plt.ylim([ 0.0, max(etime)*1.1 ])
        plt.xlabel('compiler', fontsize=LABEL_SIZE)
        plt.ylabel('Execution time(usec)', fontsize=LABEL_SIZE)
        plt.xticks(xticks, compiler_labels)
        pdf.savefig(fig)
        #plt.show()
