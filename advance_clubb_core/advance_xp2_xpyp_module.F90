...
!
! Filename    : advance_xp2_xpyp_module.F90
...
!-----------------------------------------------------------------------
        ...
        PRIVATE xp2_xpyp_lhs, term_ta_lhs, term_ta_lhs_upwind, term_dp1_lhs, xp2_xpyp_rhs, term_ta_rhs, term_tp, term_dp1_rhs, &
        xp2_xpyp_solve, xp2_xpyp_implicit_stats, xp2_xpyp_uv_rhs, term_pr1, term_pr2
...
! Named constant for rtp2 solves
! Named constant for thlp2 solves
! Named constant for rtpthlp solves
! Named constant for up2_vp2 solves
! Named constant for up2 solves
! Named constant for vp2 solves
! Named constant for scalar solves
! Named constant for sclrp2 solves
! Named constant for sclrprtp solves
! Named constant for sclrpthlp solves
        CONTAINS
...
!=============================================================================
        ...
        SUBROUTINE advance_xp2_xpyp(tau_zm, wm_zm, rtm, wprtp, thlm, wpthlp, wpthvp, um, vm, wp2, wp2_zt, wp3, upwp, vpwp, &
        sigma_sqd_w, skw_zm, kh_zt, rtp2_forcing, thlp2_forcing, rtpthlp_forcing, rho_ds_zm, rho_ds_zt, invrs_rho_ds_zm, &
        thv_ds_zm, lscale, wp3_on_wp2, wp3_on_wp2_zt, l_iter, dt, sclrm, wpsclrp, rtp2, thlp2, rtpthlp, up2, vp2, err_code, &
...
! logical constants
            USE parameters_tunable, ONLY: c5
            USE parameters_tunable, ONLY: c2
            USE parameters_tunable, ONLY: c2b
            USE parameters_tunable, ONLY: c2c
            USE parameters_tunable, ONLY: c2rt
            USE parameters_tunable, ONLY: c2thl
            USE parameters_tunable, ONLY: c2rtthl
            USE parameters_tunable, ONLY: c14
            USE parameters_tunable, ONLY: c4
            USE parameters_tunable, ONLY: c_k2
            USE parameters_tunable, ONLY: c_k9
            USE parameters_tunable, ONLY: nu2_vert_res_dep
            USE parameters_tunable, ONLY: beta
            USE parameters_tunable, ONLY: nu9_vert_res_dep
! Variable(s)
...
! Variable(s)
            ...
            USE clip_explicit, ONLY: clip_sclrp2
            USE clip_explicit, ONLY: clip_sclrprtp
            USE clip_explicit, ONLY: clip_sclrpthlp
! Procedure(s)
...
! Procedure(s)
            ...
            IMPLICIT NONE
! Intrinsic functions
            INTRINSIC exp, sqrt, min
! Constant parameters
            LOGICAL, parameter :: l_clip_large_rtp2 = .true.
! Clip rtp2 to be < rtm^2 * coef
            REAL(KIND=core_rknd), parameter :: rtp2_clip_coef = one_half
! Coefficient appled the clipping threshold on rtp2 [-]
! Input variables
            ...
            REAL(KIND=core_rknd), intent(in), dimension(gr%nz) :: tau_zm
            ...
            REAL(KIND=core_rknd), intent(in), dimension(gr%nz) :: wpthvp
            ...
            REAL(KIND=core_rknd), intent(in), dimension(gr%nz) :: wp2_zt
            REAL(KIND=core_rknd), intent(in), dimension(gr%nz) :: wp3_on_wp2_zt
! Time-scale tau on momentum levels     [s]
! w-wind component on momentum levels   [m/s]
! Total water mixing ratio (t-levs)     [kg/kg]
! <w'r_t'> (momentum levels)            [(m/s)(kg/kg)]
! Liquid potential temp. (t-levs)       [K]
! <w'th_l'> (momentum levels)           [(m K)/s]
! <w'th_v'> (momentum levels)           [(m K)/s]
! u wind (thermodynamic levels)         [m/s]
! v wind (thermodynamic levels)         [m/s]
! <w'^2> (momentum levels)              [m^2/s^2]
! <w'^2> interpolated to thermo. levels [m^2/s^2]
! <w'^3> (thermodynamic levels)         [m^3/s^3]
! <u'w'> (momentum levels)              [m^2/s^2]
! <v'w'> (momentum levels)              [m^2/s^2]
! sigma_sqd_w (momentum levels)         [-]
...
! <r_t'th_l'> forcing (momentum levels) [(kg/kg)K/s]
! Dry, static density on momentum levs. [kg/m^3]
! Dry, static density on thermo. levels [kg/m^3]
! Inv. dry, static density @ mom. levs. [m^3/kg]
! Dry, base-state theta_v on mom. levs. [K]
! Mixing length                         [m]
! Smoothed version of <w'^3>/<w'^2> zm  [m/s]
! Smoothed version of <w'^3>/<w'^2> zt  [m/s]
...
! Local Variables
            REAL(KIND=core_rknd), dimension(gr%nz) :: c2rt_1d
            REAL(KIND=core_rknd), dimension(gr%nz) :: c2thl_1d
            REAL(KIND=core_rknd), dimension(gr%nz) :: c2rtthl_1d
            REAL(KIND=core_rknd), dimension(gr%nz) :: c2sclr_1d
            REAL(KIND=core_rknd), dimension(gr%nz) :: c4_c14_1d
! Parameters C4 and C14 combined for simplicity
            REAL(KIND=core_rknd), dimension(gr%nz) :: a1
! a_1 (momentum levels); See eqn. 24 in `Equations for CLUBB' [-]
            REAL(KIND=core_rknd), dimension(gr%nz) :: upwp_zt
            REAL(KIND=core_rknd), dimension(gr%nz) :: vpwp_zt
            REAL(KIND=core_rknd), dimension(gr%nz) :: wpsclrp_zt
! <u'w'> interpolated to thermodynamic levels    [m^2/s^2]
! <v'w'> interpolated to thermodynamic levels    [m^2/s^2]
! <w'sclr'> interp. to thermo. levels  [m/s {sclrm units}]
            REAL(KIND=core_rknd) :: threshold
! Minimum value for variances                   [units vary]
            REAL(KIND=core_rknd), dimension(3,gr%nz) :: lhs
! Tridiagonal matrix
            REAL(KIND=core_rknd), dimension(gr%nz,1) :: rhs
! RHS vector of tridiagonal matrix
            REAL(KIND=core_rknd), dimension(gr%nz,2) :: uv_rhs
            REAL(KIND=core_rknd), dimension(gr%nz,2) :: uv_solution
! RHS vectors of tridiagonal system for up2/vp2
! Solution to the tridiagonal system for up2/vp2
            REAL(KIND=core_rknd), dimension(gr%nz,sclr_dim*3) :: sclr_rhs
            REAL(KIND=core_rknd), dimension(gr%nz,sclr_dim*3) :: sclr_solution
! RHS vectors of tridiagonal system for the passive scalars
! Solution to tridiagonal system for the passive scalars
            INTEGER, dimension(5+1) :: err_code_array
! Array containing the error codes for each variable
! Eddy Diffusion for Variances and Covariances.
            REAL(KIND=core_rknd), dimension(gr%nz) :: kw2
            REAL(KIND=core_rknd), dimension(gr%nz) :: kw9
! For rtp2, thlp2, rtpthlp, and passive scalars  [m^2/s]
! For up2 and vp2                                [m^2/s]
            REAL(KIND=core_rknd), dimension(gr%nz) :: a1_zt
            REAL(KIND=core_rknd), dimension(gr%nz) :: wprtp_zt
            REAL(KIND=core_rknd), dimension(gr%nz) :: wpthlp_zt
! a_1 interpolated to thermodynamic levels       [-]
! w'r_t' interpolated to thermodynamic levels    [(kg/kg) m/s]
! w'th_l' interpolated to thermodyamnic levels   [K m/s]
            REAL(KIND=core_rknd), dimension(gr%nz) :: rtpthlp_chnge
! Net change in r_t'th_l' due to clipping [(kg/kg) K]
            REAL(KIND=core_rknd), dimension(gr%nz,sclr_dim) :: sclrprtp_chnge
            REAL(KIND=core_rknd), dimension(gr%nz,sclr_dim) :: sclrpthlp_chnge
! Net change in sclr'r_t' due to clipping  [{units vary}]
! Net change in sclr'th_l' due to clipping [{units vary}]
            REAL(KIND=core_rknd), dimension(gr%nz) :: sclrp2_forcing
            REAL(KIND=core_rknd), dimension(gr%nz) :: sclrprtp_forcing
            REAL(KIND=core_rknd), dimension(gr%nz) :: sclrpthlp_forcing
! <sclr'^2> forcing (momentum levels)    [units vary]
! <sclr'r_t'> forcing (momentum levels)  [units vary]
! <sclr'th_l'> forcing (momentum levels) [units vary]
...
! Use 3 different values of C2 for rtp2, thlp2, rtpthlp.
      ...
      C2sclr_1d(1:gr%nz)  = C2rt  ! Use rt value for now ! Use rt value for now
    end if
...
! Solve the tridiagonal system
    call xp2_xpyp_solve( xp2_xpyp_rtp2, 1, & ! Intent(in)
                         ...
                         err_code_array(1) ) ! Intent(out) ! Intent(in)
! Intent(inout)
! Intent(out)
    if ( l_stats_samp ) then
      call xp2_xpyp_implicit_stats( xp2_xpyp_rtp2, rtp2 ) ! Intent(in) ! Intent(in)
    end if
!!!!!***** th_l'^2 *****!!!!!
...
! Solve the tridiagonal system
    call xp2_xpyp_solve( xp2_xpyp_thlp2, 1, & ! Intent(in)
                         ...
                         err_code_array(2) )  ! Intent(out) ! Intent(in)
! Intent(inout)
! Intent(out)
    if ( l_stats_samp ) then
      call xp2_xpyp_implicit_stats( xp2_xpyp_thlp2, thlp2 ) ! Intent(in) ! Intent(in)
    end if
!!!!!***** r_t'th_l' *****!!!!!
...
! Solve the tridiagonal system
    call xp2_xpyp_solve( xp2_xpyp_rtpthlp, 1, & ! Intent(in)
                         ...
                         err_code_array(3) )    ! Intent(out) ! Intent(in)
! Intent(inout)
! Intent(out)
    if ( l_stats_samp ) then
      call xp2_xpyp_implicit_stats( xp2_xpyp_rtpthlp, rtpthlp ) ! Intent(in) ! Intent(in)
    end if
!!!!!***** u'^2 / v'^2 *****!!!!!
...
! Solve the tridiagonal system
    call xp2_xpyp_solve( xp2_xpyp_up2_vp2, 2,       & ! Intent(in)
                         ...
                         uv_solution, err_code_array(4) )  ! Intent(out) ! Intent(in)
! Intent(inout)
! Intent(out)
    up2(1:gr%nz) = uv_solution(1:gr%nz,1)
    vp2(1:gr%nz) = uv_solution(1:gr%nz,2)
    if ( l_stats_samp ) then
      call xp2_xpyp_implicit_stats( xp2_xpyp_up2, up2 ) ! Intent(in) ! Intent(in)
      call xp2_xpyp_implicit_stats( xp2_xpyp_vp2, vp2 ) ! Intent(in) ! Intent(in)
    end if
...
! This overwrites stats clipping data from clip_variance
    ...
    end if ! l_clip_large_rtp2 ! l_clip_large_rtp2
! Clipping for th_l'^2
!threshold = zero_threshold
!
!where ( wp2 >= w_tol_sqd ) &
...
! same place, clipping for r_t'th_l' only has to be done once.
    ...
    call clip_covar( xp2_xpyp_rtpthlp, l_first_clip_ts,  & ! Intent(in)
                     ...
                     rtpthlp, rtpthlp_chnge )     ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
...
! Solve the tridiagonal system
      call xp2_xpyp_solve( xp2_xpyp_scalars, 3*sclr_dim, &   ! Intent(in)
                           ...
                           err_code_array(6) )                   ! Intent(out) ! Intent(in)
! Intent(inout)
! Intent(out)
      ...
      if ( l_hole_fill ) then
        do i = 1, sclr_dim, 1
          call pos_definite_variances( xp2_xpyp_sclrp2, dt, sclr_tol(i)**2, & ! Intent(in)
                                       rho_ds_zm, rho_ds_zt, &                ! Intent(in)
                                       sclrp2(:,i) )                          ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
          if ( i == iisclr_rt ) then
! Here again, we do this kluge here to make sclr'rt' == rt'^2
            call pos_definite_variances( xp2_xpyp_sclrprtp, dt, sclr_tol(i)**2, & ! Intent(in)
                                         rho_ds_zm, rho_ds_zt, &                  ! Intent(in)
                                         sclrprtp(:,i) )                          ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
          end if
          if ( i == iisclr_thl ) then
! As with sclr'rt' above, but for sclr'thl'
            call pos_definite_variances( xp2_xpyp_sclrpthlp, dt, sclr_tol(i)**2, & ! Intent(in)
                                         rho_ds_zm, rho_ds_zt, &                   ! Intent(in)
                                         sclrpthlp(:,i) )                          ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
          end if
        enddo
...
! Intent(inout)
          ...
          call clip_covar( clip_sclrprtp, l_first_clip_ts,  &            ! Intent(in)
                           ...
                           sclrprtp(:,i), sclrprtp_chnge(:,i) ) ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
        end if
      enddo
...
! Intent(inout)
          ...
          call clip_covar( clip_sclrpthlp, l_first_clip_ts,  &            ! Intent(in)
                           ...
                           sclrpthlp(:,i), sclrpthlp_chnge(:,i) ) ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
        end if
      enddo
    endif ! l_scalar_calc ! l_scalar_calc
...
!=============================================================================
        ...
        SUBROUTINE xp2_xpyp_lhs(dt, l_iter, wp3_on_wp2_zt, wp3_on_wp2, a1, a1_zt, tau_zm, wm_zm, kw, rho_ds_zt, rho_ds_zm, &
        invrs_rho_ds_zm, cn, nu, beta, lhs)
! Description:
...
! Variable(s)
            ...
            USE constants_clubb, ONLY: one
! Constant(s)
            USE model_flags, ONLY: l_upwind_xpyp_ta
! Constant(s)
            USE clubb_precision, ONLY: core_rknd
! Variable(s)
...
! Procedure(s)
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: irtp2_dp1
            USE stats_variables, ONLY: ithlp2_dp1
            USE stats_variables, ONLY: irtpthlp_dp1
            USE stats_variables, ONLY: zmscr01
            USE stats_variables, ONLY: irtp2_dp2
            USE stats_variables, ONLY: ithlp2_dp2
            USE stats_variables, ONLY: iup2_dp2
            USE stats_variables, ONLY: ivp2_dp2
            USE stats_variables, ONLY: irtpthlp_dp2
            USE stats_variables, ONLY: zmscr02
            USE stats_variables, ONLY: zmscr03
            USE stats_variables, ONLY: zmscr04
            USE stats_variables, ONLY: irtp2_ta
            USE stats_variables, ONLY: ithlp2_ta
            USE stats_variables, ONLY: irtpthlp_ta
            USE stats_variables, ONLY: ivp2_ta
            USE stats_variables, ONLY: iup2_ta
            USE stats_variables, ONLY: zmscr05
            USE stats_variables, ONLY: zmscr06
            USE stats_variables, ONLY: zmscr07
            USE stats_variables, ONLY: irtp2_ma
            USE stats_variables, ONLY: ithlp2_ma
            USE stats_variables, ONLY: ivp2_ma
            USE stats_variables, ONLY: iup2_ma
            USE stats_variables, ONLY: irtpthlp_ma
...
! Whether the variances are prognostic (T/F)
...
! Smoothed w'^3 / w'^2 (moment. levels)       [m/s]
! Smoothed w'^3 / w'^2 (thermo. levels)       [m/s]
! sigma_sqd_w term a_1 (momentum levels)      [-]
! a_1 interpolated to thermodynamic levels    [-]
! Time-scale tau on momentum levels           [s]
! w wind component on momentum levels         [m/s]
! Coefficient of eddy diffusivity (all vars.) [m^2/s]
! Dry, static density on thermodynamic levels [kg/m^3]
! Dry, static density on momentum levels      [kg/m^3]
! Inv. dry, static density on momentum levs.  [m^3/kg]
! Coefficient C_n                             [-]
            REAL(KIND=core_rknd), intent(in), dimension(gr%nz) :: nu
! Background constant coef. of eddy diff.        [-]
            REAL(KIND=core_rknd), intent(in) :: beta
! Constant model parameter beta                  [-]
! Output Variables
            REAL(KIND=core_rknd), dimension(3,gr%nz), intent(out) :: lhs
! Implicit contributions to the term
! Local Variables
! Array indices
            INTEGER :: k
            INTEGER :: km1
...
!-----------------------------------------------------------------------
            USE constants_clubb, ONLY: one
! Constant(s)
            USE lapack_wrap, ONLY: tridag_solvex
...
! Procedure(s)
            USE stats_variables, ONLY: irtp2_matrix_condt_num
            USE stats_variables, ONLY: ithlp2_matrix_condt_num
            USE stats_variables, ONLY: irtpthlp_matrix_condt_num
            USE stats_variables, ONLY: iup2_vp2_matrix_condt_num
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: stats_sfc
! Derived type
! Stat index Variables
! Logical
            USE error_code, ONLY: clubb_no_error
! Constant
            USE clubb_precision, ONLY: core_rknd
...
! Explicit contributions to x variance/covariance term [units vary]
            REAL(KIND=core_rknd), dimension(3,gr%nz), intent(inout) :: lhs
! Implicit contributions to x variance/covariance term [units vary]
! Output Variables
            REAL(KIND=core_rknd), dimension(gr%nz,nrhs), intent(out) :: xapxbp
! Computed value of the variable(s) at <t+1> [units vary]
...
! solve_type in string format for debug output purposes
! --- Begin Code ---
...
! Procedure(s)
            ...
            USE stats_variables, ONLY: irtpthlp_ta
            USE stats_variables, ONLY: irtpthlp_ma
            USE stats_variables, ONLY: iup2_dp1
            USE stats_variables, ONLY: iup2_dp2
            USE stats_variables, ONLY: iup2_ta
            USE stats_variables, ONLY: iup2_ma
            USE stats_variables, ONLY: iup2_pr1
            USE stats_variables, ONLY: ivp2_dp1
            USE stats_variables, ONLY: stats_zm
! Variable(s)
            USE stats_variables, ONLY: ivp2_dp2
            USE stats_variables, ONLY: ivp2_ta
            USE stats_variables, ONLY: ivp2_ma
            USE stats_variables, ONLY: ivp2_pr1
            USE stats_variables, ONLY: zmscr01
            USE stats_variables, ONLY: zmscr03
            USE stats_variables, ONLY: zmscr04
            USE stats_variables, ONLY: zmscr02
...
! Computed value of the variable at <t+1> [units vary]
! Local variables
            INTEGER :: k
            INTEGER :: km1
            INTEGER :: kp1 ! Array indices
! Budget indices
            INTEGER :: ixapxbp_dp1
            INTEGER :: ixapxbp_dp2
            INTEGER :: ixapxbp_ta
...
!=============================================================================
        ...
        SUBROUTINE xp2_xpyp_uv_rhs(solve_type, dt, l_iter, a1, a1_zt, wp2, wp2_zt, wpthvp, lscale, wp3_on_wp2_zt, wp3_on_wp2, &
        c4_c14_1d, tau_zm, xam, xbm, wpxap, wpxap_zt, wpxbp, wpxbp_zt, xap2, xbp2, rho_ds_zt, invrs_rho_ds_zm, rho_ds_zm, &
        thv_ds_zm, c4, c5, c14, beta, rhs)
! Description:
...
! Variable(s)
            ...
            USE stats_type_utilities, ONLY: stat_modify_pt
            USE stats_type_utilities, ONLY: stat_update_var_pt
! Procedure(s)
            USE stats_variables, ONLY: ivp2_ta
            USE stats_variables, ONLY: ivp2_tp
            USE stats_variables, ONLY: ivp2_dp1
            USE stats_variables, ONLY: ivp2_pr1
            USE stats_variables, ONLY: ivp2_pr2
            USE stats_variables, ONLY: iup2_ta
            USE stats_variables, ONLY: iup2_tp
            USE stats_variables, ONLY: iup2_dp1
            USE stats_variables, ONLY: iup2_pr1
            USE stats_variables, ONLY: iup2_pr2
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: stats_zm
            USE stats_variables, ONLY: zmscr01
            USE stats_variables, ONLY: zmscr11
...
! a_1 interpolated to thermodynamic levels    [-]
! w'^2 (momentum levels)                      [m^2/s^2]
! w'^2 interpolated to thermodynamic levels   [m^2/s^2]
! w'th_v' (momentum levels)                   [K m/s]
! Mixing Length                               [m]
! Smoothed w'^3 / w'^2 on momentum levels     [m/s]
! Smoothed w'^3 / w'^2 on thermo. levels      [m/s]
! Combination of model params. C_4 and C_14   [-]
! Time-scale tau on momentum levels           [s]
! x_am (thermodynamic levels)                 [m/s]
! x_bm (thermodynamic levels)                 [m/s]
! w'x_a' (momentum levels)                    [m^2/s^2]
! w'x_a' interpolated to thermodynamic levels [m^2/s^2]
! w'x_b' (momentum levels)                    [m^2/s^2]
! w'x_b' interpolated to thermodynamic levels [m^2/s^2]
! x_a'^2 (momentum levels)                    [m^2/s^2]
! x_b'^2 (momentum levels)                    [m^2/s^2]
! Dry, static density on thermodynamic levels [kg/m^3]
! Dry, static density on momentum levels      [kg/m^3]
! Inv. dry, static density on momentum levs.  [m^3/kg]
! Dry, base-state theta_v on momentum levels  [K]
...
! Model parameter C_4                         [-]
! Model parameter C_5                         [-]
! Model parameter C_{14}                      [-]
! Model parameter beta                        [-]
! Output Variable
            REAL(KIND=core_rknd), dimension(gr%nz,1), intent(out) :: rhs
! Explicit contributions to x variance/covariance terms
! Local Variables
! Array indices
            INTEGER :: k
            INTEGER :: km1
...
!        advection (ta) term).
        if ( .not. l_upwind_xpyp_ta ) then
          ...
          = term_ta_lhs( wp3_on_wp2_zt(kp1), wp3_on_wp2_zt(k), &
                         rho_ds_zt(kp1), rho_ds_zt(k), invrs_rho_ds_zm(k),  &
                         a1_zt(kp1), a1(k), a1_zt(k), gr%invrs_dzm(k), beta, k )
        else ! turbulent advection is using an upwind discretization ! turbulent advection is using an upwind discretization
          lhs_fnc_output(1:3)  &
          = term_ta_lhs_upwind( a1(k), a1(kp1), a1(km1), &
                                wp3_on_wp2(kp1), wp3_on_wp2(k), wp3_on_wp2(km1), &
                                gr%invrs_dzt(k), gr%invrs_dzt(kp1), &
                                invrs_rho_ds_zm(k), &
                                rho_ds_zm(kp1), rho_ds_zm(k), rho_ds_zm(km1), beta )
        end if ! ~l_upwind_xpyp_ta ! ~l_upwind_xpyp_ta
...
!=============================================================================
        ...
        SUBROUTINE xp2_xpyp_rhs(solve_type, dt, l_iter, a1, a1_zt, wp2_zt, wpxap, wpxap_zt, wp3_on_wp2, wp3_on_wp2_zt, wpxbp, &
        wpxbp_zt, xam, xbm, xapxbp, xapxbp_forcing, rho_ds_zm, rho_ds_zt, invrs_rho_ds_zm, cn, tau_zm, threshold, beta, rhs)
! Description:
...
! Procedure(s)
            USE stats_variables, ONLY: irtp2_ta
            USE stats_variables, ONLY: irtp2_tp
            USE stats_variables, ONLY: irtp2_dp1
            USE stats_variables, ONLY: irtp2_forcing
            USE stats_variables, ONLY: ithlp2_ta
            USE stats_variables, ONLY: ithlp2_tp
            USE stats_variables, ONLY: ithlp2_dp1
            USE stats_variables, ONLY: ithlp2_forcing
            USE stats_variables, ONLY: irtpthlp_ta
            USE stats_variables, ONLY: irtpthlp_tp1
            USE stats_variables, ONLY: irtpthlp_tp2
            USE stats_variables, ONLY: irtpthlp_dp1
            USE stats_variables, ONLY: irtpthlp_forcing
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: stats_zm
! Variable(s)
...
! a_1 interpolated to thermodynamic levels    [-]
! w'^2 interpolated to thermodynamic levels   [m^2/s^2]
! w'x_a' (momentum levels)                    [m/s {x_am units}]
! w'x_a' interpolated to thermodynamic levels [m/s {x_am units}]
! w'^3 / w'^2 on momentum levels              [m/s]
! w'^3 / w'^2 on thermodynamic levels         [m/s]
! w'x_b' (momentum levels)                    [m/s {x_bm units}]
! w'x_b' interpolated to thermodynamic levels [m/s {x_bm units}]
! x_am (thermodynamic levels)                 [{x_am units}]
! x_bm (thermodynamic levels)                 [{x_bm units}]
! x_a'x_b' (momentum levels)                  [{x_am units}*{x_bm units}]
! x_a'x_b' forcing (momentum levels)          [{x_am units}*{x_bm units}/s]
...
! Coefficient C_n                             [-]
            ...
            REAL(KIND=core_rknd), intent(in) :: beta
! Smallest allowable mag. value for x_a'x_b'  [{x_am units}
!                                              *{x_bm units}]
! Model parameter beta                        [-]
! Output Variable
            REAL(KIND=core_rknd), dimension(gr%nz,1), intent(out) :: rhs
...
!=============================================================================
        ...
        pure FUNCTION term_ta_lhs(wp3_on_wp2_ztp1, wp3_on_wp2_zt, rho_ds_ztp1, rho_ds_zt, invrs_rho_ds_zm, a1_ztp1, a1, a1_zt, &
        invrs_dzm, beta, level) RESULT ( lhs )
! Description:
...
! References:
!-----------------------------------------------------------------------
            USE grid_class, ONLY: gr ! gr%weights_zm2zt
! Variable(s)
            USE constants_clubb, ONLY: one_third
! Constant(s)
...
!-----------------------------------------------------------------------------
        ...
        pure FUNCTION term_ta_lhs_upwind(a1_zm, a1_zm_p1, a1_zm_m1, wp3_on_wp2_p1, wp3_on_wp2, wp3_on_wp2_m1, invrs_dzt, &
        invrs_dzt_p1, invrs_rho_ds_zm, rho_ds_zm_p1, rho_ds_zm, rho_ds_zm_m1, beta) RESULT ( lhs )
! Description:
...
!=============================================================================
        ...
        pure FUNCTION term_ta_rhs(wp2_ztp1, wp2_zt, wp3_on_wp2_ztp1, wp3_on_wp2_zt, rho_ds_ztp1, rho_ds_zt, invrs_rho_ds_zm, &
        a1_ztp1, a1, a1_zt, wpxbp_ztp1, wpxbp_zt, wpxap_ztp1, wpxap_zt, invrs_dzm, beta) RESULT ( rhs )
! Description:
...
! Input variables
            ...
            REAL(KIND=core_rknd), intent(in) :: a1
            REAL(KIND=core_rknd), intent(in) :: rho_ds_ztp1
! w'^2 interpolated to thermo. level (k+1)   [m^2/s^2]
! w'^2 interpolated to thermo. level (k)     [m^2/s^2]
! Smoothed w'^3 / w'^2 on thermo. level (k+1)[m^2/s^2]
! Smoothed w'^3 / w'^2 on thermo. level (k)  [m^2/s^2]
! Dry, static density at thermo. level (k+1) [kg/m^3]
! Dry, static density at thermo. level (k)   [kg/m^3]
! Inv. dry, static density @ mome. lev (k)   [m^3/kg]
! a_1 interpolated to thermo. level (k+1)    [-]
! a_1(k)                                     [-]
! a_1 interpolated to thermo. level (k)      [-]
! w'x_b' interpolated to thermo. level (k+1) [m/s {x_bm units}]
! w'x_b' interpolated to thermo. level (k)   [m/s {x_bm units}]
! w'x_a' interpolated to thermo. level (k+1) [m/s {x_am units}]
! w'x_a' interpolated to thermo. level (k)   [m/s {x_am units}]
! Inverse of grid spacing                    [1/m]
! Model parameter                            [-]
! Return Variable
...
!=============================================================================
        ...
        pure FUNCTION term_tp(xamp1, xam, xbmp1, xbm, wpxbp, wpxap, invrs_dzm) RESULT ( rhs )
! Description:
...
! Input variables
            ...
            REAL(KIND=core_rknd), intent(in) :: xamp1
...
! x_bm(k)                     [{x_bm units}]
! x_bm(k+1)                   [{x_bm units}]
! w'x_b'(k)                   [m/s {x_bm units}]
! w'x_a'(k)                   [m/s {x_am units}]
! Inverse of grid spacing (k) [1/m]
! Return Variable
            REAL(KIND=core_rknd) :: rhs
    rhs & 
...
!=============================================================================
        ...
        pure FUNCTION term_dp1_lhs(cn, tau_zm) RESULT ( lhs )
! Description:
...
! Variable(s)
            IMPLICIT NONE
! Input Variables
            REAL(KIND=core_rknd), intent(in) :: cn
            REAL(KIND=core_rknd), intent(in) :: tau_zm
! Coefficient C_n                       [-]
! Time-scale tau at momentum levels (k) [s]
! Return Variable
...
!=============================================================================
        ...
        pure FUNCTION term_dp1_rhs(cn, tau_zm, threshold) RESULT ( rhs )
! Description:
...
! Coefficient C_n                               [-]
! Time-scale tau at momentum levels (k)         [s]
! Minimum allowable magnitude value of x_a'x_b' [units vary]
! Return Variable
            REAL(KIND=core_rknd) :: rhs
    rhs  & 
    = + ( Cn / tau_zm ) * threshold
...
!=============================================================================
        ...
        pure FUNCTION term_pr1(c4, c14, xbp2, wp2, tau_zm) RESULT ( rhs )
! Description:
...
! Model parameter C_4                         [-]
! Model parameter C_14                        [-]
! v'^2(k) (if solving for u'^2) or vice versa [m^2/s^2]
! w'^2(k)                                     [m^2/s^2]
! Time-scale tau at momentum levels (k)       [s]
! Return Variable
            REAL(KIND=core_rknd) :: rhs
...
!=============================================================================
        ...
        FUNCTION term_pr2(c5, thv_ds_zm, wpthvp, upwp, vpwp, um, vm, invrs_dzm, kp1, k, lscalep1, lscale, wp2_ztp1, wp2_zt) &
        RESULT ( rhs )
! Description:
...
!-----------------------------------------------------------------------
            ...
            USE constants_clubb, ONLY: zero
            USE constants_clubb, ONLY: zero_threshold ! Constants
...
! Input Variables
            ...
            REAL(KIND=core_rknd), intent(in) :: wp2_ztp1
! Model parameter C_5                            [-]
! Dry, base-state theta_v at momentum level (k)  [K]
! w'th_v'(k)                                     [m/K/s]
! u'w'(k)                                        [m^2/s^2]
! v'w'(k)                                        [m^2/s^2]
! Inverse of the grid spacing (k)                [1/m]
! Mixing length (k+1)                            [m]
! Mixing length (k)                              [m]
! w'^2(k+1) (thermo. levels)                     [m^2/s^2]
! w'^2(k)   (thermo. levels)                     [m^2/s^2]
...
! mean meridional wind  [m/s]
            ...
            INTEGER, intent(in) :: k
! current level+1 in xp2_xpyp_uv_rhs loop
! current level in xp2_xpyp_uv_rhs loop
! Return Variable
            REAL(KIND=core_rknd) :: rhs
! Local Variable(s)  --ldgrant, March 2010
            REAL(KIND=core_rknd), parameter :: vert_avg_depth = 200.0_core_rknd
            REAL(KIND=core_rknd), parameter :: constant2 = 1000.0_core_rknd
            REAL(KIND=core_rknd), parameter :: constant1 = one
! Constants empirically determined for experimental version of term_pr2
! ldgrant March 2010
! [m/s]
! [m]
! Depth over which to average d(um)/dz and d(vm)/dz [m]
            REAL(KIND=core_rknd) :: zt_high
            REAL(KIND=core_rknd) :: um_low
            REAL(KIND=core_rknd) :: um_high
            REAL(KIND=core_rknd) :: vm_low
            REAL(KIND=core_rknd) :: vm_high
            REAL(KIND=core_rknd) :: zt_low
! altitude above current altitude zt(k)         [m]
! um at altitude zt_high                        [m/s]
! vm at altitude zt_high                        [m/s]
! altitude below (or at) current altitude zt(k) [m]
! um at altitude zt_low                         [m/s]
! vm at altitude zt_low                         [m/s]
            LOGICAL, parameter :: l_use_experimental_term_pr2 = .false.
            LOGICAL, parameter :: l_use_vert_avg_winds = .true.
! If true, use experimental version
! of term_pr2 calculation
! If true, use vert_avg_depth average
...
! As applied to w'2
    ...
    else ! use experimental version of term_pr2 --ldgrant March 2010 ! use experimental version of term_pr2 --ldgrant March 2010
...
! intent(out)
! intent(out)
      else ! Do not use a vertical average calculation for d(um)/dz and d(vm)/dz ! Do not use a vertical average calculation for d(um)/dz and d(vm)/dz
        zt_high = gr%zt(kp1)
        um_high = um(kp1)
        vm_high = vm(kp1)
        zt_low  = gr%zt(k)
        um_low  = um(k)
        vm_low  = vm(k)
      end if ! l_use_vert_avg_winds ! l_use_vert_avg_winds
...
! warning that Lscalep1/Lscale are not
! used. -meyern
    end if ! .not. l_use_experimental_term_pr2 ! .not. l_use_experimental_term_pr2
...
! Variable(s)
            IMPLICIT NONE
! Input Variables
            REAL(KIND=core_rknd), intent(in) :: vert_avg_depth
! Depth over which to average d(um)/dz
...
! In this case, no interpolation is necessary.
      ...
      um_high = um(k_high)
      vm_high = vm(k_high)
    else ! Do an interpolation to find um & vm at current altitude + depth. ! Do an interpolation to find um & vm at current altitude + depth.
...
! In this case, no interpolation is necessary.
      ...
      um_low = um(k_low)
      vm_low = vm(k_low)
    else ! Do an interpolation to find um at current altitude - depth. ! Do an interpolation to find um at current altitude - depth.
...
!-----------------------------------------------------------------------
            ...
            USE grid_class, ONLY: gr
            USE clubb_precision, ONLY: core_rknd
            USE stats_variables, ONLY: irtp2_pd
            USE stats_variables, ONLY: ithlp2_pd
            USE stats_variables, ONLY: iup2_pd
            USE stats_variables, ONLY: ivp2_pd
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: stats_zm
! variables
...
! Threshold for xp2_np1       [units vary]
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: rho_ds_zm
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: rho_ds_zt
! Dry, static density on momentum levels         [kg/m^3]
! Dry, static density on thermodynamic levels    [kg/m^3]
! Input/Output variables
            REAL(KIND=core_rknd), intent(inout), dimension(gr%nz) :: xp2_np1
! Variance for <n+1>          [units vary]
! Local variables
            INTEGER :: ixp2_pd
    select case( solve_type )
                CASE ( xp2_xpyp_rtp2 )
      ...
      ixp2_pd = ivp2_pd
                CASE DEFAULT
      ixp2_pd = 0 ! This includes the passive scalars ! This includes the passive scalars
    end select
    if ( l_stats_samp ) then
...
! of the hole.
      call fill_holes_vertical( 2, tolerance, "zm",   & ! Intent(in)
                              ...
                              xp2_np1 )               ! Intent(inout) ! Intent(in)
! Intent(in)
! Intent(inout)
    endif
    if ( l_stats_samp ) then
