...
!=============================================================================
        ...
        SUBROUTINE compute_length(thvm, thlm, rtm, em, lscale_max, p_in_pa, exner, thv_ds, mu, l_implemented, err_code, lscale, &
        lscale_up, lscale_down)
! Description:
...
! (1-ep)/ep                                  [-]
! 1/ep                                       [-]
! Latent heat of vaporiztion                 [J/kg/K]
! Gravitational acceleration                 [m/s^2]
            USE parameters_tunable, ONLY: lmin ! Variable(s)
! Minimum value for Lscale                         [m]
...
! Procedure(s)
            USE saturation, ONLY: sat_mixrat_liq_lookup
...
! Constant
            USE model_flags, ONLY: l_sat_mixrat_lookup
! Variable(s)
...
! Constant Parameters
            REAL(KIND=core_rknd), parameter :: zlmin = 0.1_core_rknd
            REAL(KIND=core_rknd), parameter :: lscale_sfclyr_depth = 500._core_rknd
! Minimum value for Lscale [m]
! [m]
! Input Variables
            ...
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: thvm
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: em
! Virtual potential temp. on themodynamic level  [K]
! Liquid potential temp. on themodynamic level   [K]
! Total water mixing ratio on themodynamic level [kg/kg]
! em = 3/2 * w'^2; on momentum level             [m^2/s^2]
! Exner function on thermodynamic level          [-]
! Pressure on thermodynamic level                [Pa]
! Dry, base-state theta_v on thermodynamic level [K]
...
! mu Fractional extrainment rate per unit altitude      [1/m]
...
! Mixing length      [m]
! Mixing length up   [m]
! Mixing length down [m]
! Local Variables
            INTEGER :: err_code_lscale
            INTEGER :: i
            INTEGER :: j
            REAL(KIND=core_rknd) :: tke_i
            REAL(KIND=core_rknd) :: cape_incr
            REAL(KIND=core_rknd) :: dcape_dz_j_minus_1
            REAL(KIND=core_rknd) :: dcape_dz_j
            REAL(KIND=core_rknd) :: dcape_dz_j_plus_1
! Temporary arrays to store calculations to speed runtime
            REAL(KIND=core_rknd), dimension(gr%nz) :: exp_mu_dzm
            REAL(KIND=core_rknd), dimension(gr%nz) :: invrs_dzm_on_mu
! Minimum value for Lscale that will taper off with height
            REAL(KIND=core_rknd) :: lminh
! Parcel quantities at grid level j
            REAL(KIND=core_rknd) :: thl_par_j
            REAL(KIND=core_rknd) :: rt_par_j
            REAL(KIND=core_rknd) :: rc_par_j
            REAL(KIND=core_rknd) :: thv_par_j
! Used in latent heating calculation
            REAL(KIND=core_rknd) :: tl_par_j
            REAL(KIND=core_rknd) :: rsatl_par_j
            REAL(KIND=core_rknd) :: beta_par_j
            REAL(KIND=core_rknd) :: s_par_j
! Parcel quantities at grid level j-1
            REAL(KIND=core_rknd) :: thl_par_j_minus_1
            REAL(KIND=core_rknd) :: rt_par_j_minus_1
! Parcel quantities at grid level j+1
            REAL(KIND=core_rknd) :: thl_par_j_plus_1
            REAL(KIND=core_rknd) :: rt_par_j_plus_1
! Variables to make L nonlocal
            REAL(KIND=core_rknd) :: lscale_up_max_alt
            REAL(KIND=core_rknd) :: lscale_down_min_alt
! ---- Begin Code ----
...
! Upwards loop
    ...
    do i = 2, gr%nz, 1
      tke_i = zm2zt( em, i )   ! TKE interpolated to thermodynamic level ! TKE interpolated to thermodynamic level
      Lscale_up(i) = zlmin
      j = i + 1
...
! Overwrite the last error code with this new fatal error
      ...
      endif ! Fatal error ! Fatal error
    endif ! clubb_debug_level ! clubb_debug_level
    return
