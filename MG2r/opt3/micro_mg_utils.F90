...
! KGEN version: 0.5.2
  ...
  if (present(n0)) n0 = nic * lam
        ...
        SUBROUTINE size_dist_param_basic_vect(props, qic, nic, lam, mgncol, n0)
            TYPE(mghydrometeorprops), intent(in) :: props
            INTEGER,                          intent(in) :: mgncol
            REAL(KIND=r8), dimension(mgncol), intent(in) :: qic
            REAL(KIND=r8), dimension(mgncol), intent(inout) :: nic
            REAL(KIND=r8), dimension(mgncol), intent(out) :: lam
            REAL(KIND=r8), dimension(mgncol), intent(out), optional :: n0
            INTEGER :: i
  do i=1,mgncol
