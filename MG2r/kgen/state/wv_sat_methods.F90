...
! KGEN version: 0.5.2
    ...
    MODULE wv_sat_methods
        ...
        REAL(KIND=r8) :: ttrice ! Ice-water transition range
        ...
        PUBLIC kgen_write_externs_wv_sat_methods
        CONTAINS
        ...
        SUBROUTINE kgen_write_externs_wv_sat_methods(kgen_unit)
            INTEGER, INTENT(IN) :: kgen_unit
            WRITE(UNIT=kgen_unit) default_idx
            WRITE(UNIT=kgen_unit) tboil
            WRITE(UNIT=kgen_unit) tmelt
            WRITE(UNIT=kgen_unit) epsilo
            WRITE(UNIT=kgen_unit) omeps
            WRITE(UNIT=kgen_unit) h2otrip
        END SUBROUTINE kgen_write_externs_wv_sat_methods
        ...
        SUBROUTINE wv_sat_methods_init(kind, tmelt_in, h2otrip_in, tboil_in, ttrice_in, epsilo_in, errstring)
            INTEGER, intent(in) :: kind
            REAL(KIND=r8), intent(in) :: tmelt_in
            REAL(KIND=r8), intent(in) :: h2otrip_in
            REAL(KIND=r8), intent(in) :: tboil_in
            REAL(KIND=r8), intent(in) :: ttrice_in
            REAL(KIND=r8), intent(in) :: epsilo_in
  ...
  select case (use_idx)
     ...
     es = Bolton_svp_water(t)
        ...
        elemental FUNCTION wv_sat_svp_trans(t, idx) RESULT ( es )
            ...
            INTEGER, intent(in), optional :: idx
            REAL(KIND=r8) :: es
            REAL(KIND=r8) :: esice ! Saturation vapor pressure over ice
  ...
  es = exp(9.550426_r8 - (5723.265_r8 / t) + (3.53068_r8 * log(t)) &
       - (0.00728332_r8 * t))
        ...
        elemental FUNCTION oldgoffgratch_svp_water(t) RESULT ( es )
            REAL(KIND=r8), intent(in) :: t
            REAL(KIND=r8) :: es
            REAL(KIND=r8) :: ps, e1, e2, f1, f2, f3, f4, f5, f
  ps = 1013.246_r8
