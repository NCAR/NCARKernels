...
! KGEN version: 0.5.2
    ...
    MODULE micro_mg2_0
        ...
        USE micro_mg_utils, ONLY: r8, pi, omsm, qsmall, mincld, rhosn, rhoi, rhow, rhows, ac, bc, ai, bi, ar, br, as, bs, mi0, rising_factorial
        IMPLICIT NONE
        PRIVATE
        SAVE
        PUBLIC micro_mg_init, micro_mg_get_cols, micro_mg_tend
        ...
        !===============================================================================
        PUBLIC kgen_write_externs_micro_mg2_0
        CONTAINS
        ...
        SUBROUTINE kgen_write_externs_micro_mg2_0(kgen_unit)
            INTEGER, INTENT(IN) :: kgen_unit
            WRITE(UNIT=kgen_unit) use_hetfrz_classnuc
            WRITE(UNIT=kgen_unit) do_cldice
            WRITE(UNIT=kgen_unit) microp_uniform
            WRITE(UNIT=kgen_unit) r
            WRITE(UNIT=kgen_unit) rhosu
            WRITE(UNIT=kgen_unit) g
            WRITE(UNIT=kgen_unit) tmelt
            WRITE(UNIT=kgen_unit) icenuct
            WRITE(UNIT=kgen_unit) rhmini
            WRITE(UNIT=kgen_unit) snowmelt
            WRITE(UNIT=kgen_unit) xlf
            WRITE(UNIT=kgen_unit) cpp
            WRITE(UNIT=kgen_unit) rainfrze
            WRITE(UNIT=kgen_unit) micro_mg_precip_frac_method
            WRITE(UNIT=kgen_unit) dcs
            WRITE(UNIT=kgen_unit) gamma_br_plus1
            WRITE(UNIT=kgen_unit) gamma_br_plus4
            WRITE(UNIT=kgen_unit) gamma_bs_plus4
            WRITE(UNIT=kgen_unit) gamma_bs_plus1
            WRITE(UNIT=kgen_unit) micro_mg_berg_eff_factor
            WRITE(UNIT=kgen_unit) xxls_squared
            WRITE(UNIT=kgen_unit) rv
            WRITE(UNIT=kgen_unit) xxlv
            WRITE(UNIT=kgen_unit) xxls
            WRITE(UNIT=kgen_unit) xxlv_squared
            WRITE(UNIT=kgen_unit) gamma_bi_plus4
            WRITE(UNIT=kgen_unit) gamma_bi_plus1
            WRITE(UNIT=kgen_unit) allow_sed_supersat
        END SUBROUTINE kgen_write_externs_micro_mg2_0

        !===============================================================================

        SUBROUTINE micro_mg_init(kind, gravit, rair, rh2o, cpair, tmelt_in, latvap, latice, rhmini_in, micro_mg_dcs, &
        microp_uniform_in, do_cldice_in, use_hetfrz_classnuc_in, micro_mg_precip_frac_method_in, micro_mg_berg_eff_factor_in, &
        allow_sed_supersat_in, errstring)
            ...
            REAL(KIND=r8), intent(in) :: gravit
            REAL(KIND=r8), intent(in) :: rair
            REAL(KIND=r8), intent(in) :: rh2o
            REAL(KIND=r8), intent(in) :: cpair
            REAL(KIND=r8), intent(in) :: tmelt_in ! Freezing point of water (K)
            REAL(KIND=r8), intent(in) :: latvap
            REAL(KIND=r8), intent(in) :: latice
            REAL(KIND=r8), intent(in) :: rhmini_in ! Minimum rh for ice cloud fraction > 0.
            REAL(KIND=r8), intent(in) :: micro_mg_dcs
            ...
            REAL(KIND=r8), intent(in) :: micro_mg_berg_eff_factor_in ! berg efficiency factor
  ...
  g= gravit                 ! gravity ! gravity
  r= rair                   ! dry air gas constant: note units(phys_constants are in J/K/kmol) ! dry air gas constant: note units(phys_constants are in J/K/kmol)
  rv= rh2o                  ! water vapor gas constant ! water vapor gas constant
  cpp = cpair               ! specific heat of dry air ! specific heat of dry air
  tmelt = tmelt_in
  rhmini = rhmini_in
  ...
  allow_sed_supersat          = allow_sed_supersat_in
            ! latent heats
  xxlv = latvap         ! latent heat vaporization ! latent heat vaporization
  xlf  = latice         ! latent heat freezing ! latent heat freezing
  xxls = xxlv + xlf     ! latent heat of sublimation ! latent heat of sublimation
            ! flags
  ...
  xxls_squared=xxls**2
        ...
        drout2, dsout2, freqs, freqr, nfice, qcrat, errstring, tnd_qsnow, tnd_nsnow, re_ice, prer_evap, frzimm, frzcnt, frzdep)
            ...
            USE micro_mg_utils, ONLY: ice_deposition_sublimation, kk2000_liq_autoconversion, ice_autoconversion, &
            immersion_freezing, contact_freezing, snow_self_aggregation, accrete_cloud_water_snow, secondary_ice_production, &
            accrete_rain_snow, heterogeneous_rain_freezing, accrete_cloud_water_rain, self_collection_rain, &
            accrete_cloud_ice_snow, evaporate_sublimate_precip, bergeron_process_snow
  ...
  sed_col_loop: do i=1,mgncol
     ...
     end do       !!! vertical loop !!! vertical loop
     ...
     end do ! vertical k loop ! vertical k loop
     do k=1,nlev
  ...
  end do sed_col_loop! i loop ! i loop
            ! DO STUFF FOR OUTPUT:
            !==================================================
  ...
  if (Atmp > 0._r8) then
     rercld = rercld + 3._r8 *(qric + qcic) / (4._r8 * rhow * Atmp)
        ...
        pure SUBROUTINE micro_mg_get_cols(ncol, nlev, top_lev, qcn, qin, qrn, qsn, mgncol, mgcols)
