...
! KGEN version: 0.5.2
    ...
    MODULE micro_mg_utils
        ...
        PRIVATE
        SAVE
        PUBLIC micro_mg_utils_init, size_dist_param_liq, size_dist_param_basic, avg_diameter, rising_factorial, &
        ice_deposition_sublimation, kk2000_liq_autoconversion, ice_autoconversion, immersion_freezing, contact_freezing, &
        ...
        ! droplets
        REAL(KIND=r8), parameter, public :: ac = 3.e7_r8
        ...
        ! m = cD^d
        ! exponent
        REAL(KIND=r8), parameter :: dsph = 3._r8
        ! Bounds for mean diameter for different constituents.
        REAL(KIND=r8), parameter :: lam_bnd_rain(2) = 1._r8/[500.e-6_r8, 20.e-6_r8]
        REAL(KIND=r8), parameter :: lam_bnd_snow(2) = 1._r8/[2000.e-6_r8, 10.e-6_r8]
        ! Minimum average mass of particles.
        REAL(KIND=r8), parameter :: min_mean_mass_liq = 1.e-20_r8
        REAL(KIND=r8), parameter :: min_mean_mass_ice = 1.e-20_r8
        ...
        INTERFACE kgen_write
            MODULE PROCEDURE kgen_write_mghydrometeorprops
        END INTERFACE kgen_write

        PUBLIC kgen_write_externs_micro_mg_utils
        CONTAINS

        ! write subroutines
        SUBROUTINE kgen_write_mghydrometeorprops(var, kgen_unit, printvar)
            INTEGER, INTENT(IN) :: kgen_unit
            CHARACTER(*), INTENT(IN), OPTIONAL :: printvar
            TYPE(mghydrometeorprops), INTENT(in) :: var
            WRITE(UNIT=kgen_unit) var%rho
            IF ( PRESENT(printvar) ) THEN
                print *, "** KGEN DEBUG: " // printvar // "%rho **", var%rho
            END IF
            WRITE(UNIT=kgen_unit) var%eff_dim
            IF ( PRESENT(printvar) ) THEN
                print *, "** KGEN DEBUG: " // printvar // "%eff_dim **", var%eff_dim
            END IF
            WRITE(UNIT=kgen_unit) var%shape_coef
            IF ( PRESENT(printvar) ) THEN
                print *, "** KGEN DEBUG: " // printvar // "%shape_coef **", var%shape_coef
            END IF
            WRITE(UNIT=kgen_unit) var%lambda_bounds
            IF ( PRESENT(printvar) ) THEN
                print *, "** KGEN DEBUG: " // printvar // "%lambda_bounds **", var%lambda_bounds
            END IF
            WRITE(UNIT=kgen_unit) var%min_mean_mass
            IF ( PRESENT(printvar) ) THEN
        ...
        SUBROUTINE kgen_write_externs_micro_mg_utils(kgen_unit)
            INTEGER, INTENT(IN) :: kgen_unit
            WRITE(UNIT=kgen_unit) tmelt
            WRITE(UNIT=kgen_unit) gamma_bs_plus3
            WRITE(UNIT=kgen_unit) xxlv
            WRITE(UNIT=kgen_unit) rv
            WRITE(UNIT=kgen_unit) cpp
            WRITE(UNIT=kgen_unit) gamma_half_br_plus5
            WRITE(UNIT=kgen_unit) xxls
            WRITE(UNIT=kgen_unit) gamma_half_bs_plus5
            CALL kgen_write_mghydrometeorprops(mg_liq_props, kgen_unit)
            CALL kgen_write_mghydrometeorprops(mg_ice_props, kgen_unit)
            CALL kgen_write_mghydrometeorprops(mg_rain_props, kgen_unit)
            CALL kgen_write_mghydrometeorprops(mg_snow_props, kgen_unit)
        END SUBROUTINE kgen_write_externs_micro_mg_utils
  ...
  mg_snow_props = MGHydrometeorProps(rhosn, dsph, lam_bnd_snow)
        ...
        FUNCTION newmghydrometeorprops(rho, eff_dim, lambda_bounds, min_mean_mass) RESULT ( res )
            REAL(KIND=r8), intent(in) :: rho, eff_dim
            REAL(KIND=r8), intent(in), optional :: lambda_bounds(2), min_mean_mass
