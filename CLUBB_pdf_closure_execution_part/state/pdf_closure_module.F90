    ...
    DO WHILE( kgen_cur_rank < kgen_mpi_size )
        IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
            IF (ANY(kgen_counter == kgen_counter_at)) THEN
                ...
                kgen_filepath = "/glade/u/home/youngsun/trepo/temp/kgen_system_test/branches/initial/test_cases/CLUBB_pdf_closure_part3/kernel/pdf_closure_part3." // TRIM(ADJUSTL(kgen_counter_conv)) // "." // TRIM(ADJUSTL(kgen_mpi_rank_conv))
                ...
                WRITE (UNIT = kgen_unit) wp2
                WRITE (UNIT = kgen_unit) wm
                WRITE (UNIT = kgen_unit) rtm
                WRITE (UNIT = kgen_unit) thlm
                WRITE (UNIT = kgen_unit) skw
                WRITE (UNIT = kgen_unit) sigma_sqd_w
                WRITE (UNIT = kgen_unit) thlp2
                WRITE (UNIT = kgen_unit) wpthlp
                WRITE (UNIT = kgen_unit) rtp2
                WRITE (UNIT = kgen_unit) wprtp
                WRITE (UNIT = kgen_unit) rtpthlp
                WRITE (UNIT = kgen_unit) exner
                WRITE (UNIT = kgen_unit) p_in_pa
                WRITE (UNIT = kgen_unit) thv_ds
                WRITE (UNIT = kgen_unit) wp3
                CALL kw_pdf_closure_real__core_rknd_dim1(sclrm, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(sclrp2, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(wpsclrp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(sclrpthlp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(sclrprtp, kgen_unit)
                WRITE (UNIT = kgen_unit) level
                CALL kw_pdf_closure_real__core_rknd_dim1(wp2hmp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(wphydrometp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(thlphmp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(rtphmp, kgen_unit)
                ...
                CALL kw_pdf_closure_real__core_rknd_dim1(sclrprcp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(sclrpthvp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(wpsclrp2, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(wpsclrprtp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(wpsclrpthlp, kgen_unit)
                CALL kw_pdf_closure_real__core_rknd_dim1(wp2sclrp, kgen_unit)
                ...
                WRITE (UNIT = kgen_unit) alpha_rt
                WRITE (UNIT = kgen_unit) varnce_rt_1
                WRITE (UNIT = kgen_unit) varnce_thl_1
                WRITE (UNIT = kgen_unit) varnce_rt_2
                WRITE (UNIT = kgen_unit) varnce_thl_2
                WRITE (UNIT = kgen_unit) rt_1
                WRITE (UNIT = kgen_unit) thl_1
                WRITE (UNIT = kgen_unit) rt_2
                WRITE (UNIT = kgen_unit) thl_2
                WRITE (UNIT = kgen_unit) rrtthl
                WRITE (UNIT = kgen_unit) w_1
                WRITE (UNIT = kgen_unit) varnce_w_1
                WRITE (UNIT = kgen_unit) w_2
                WRITE (UNIT = kgen_unit) varnce_w_2
                WRITE (UNIT = kgen_unit) crt_1
                WRITE (UNIT = kgen_unit) cthl_1
                WRITE (UNIT = kgen_unit) crt_2
                WRITE (UNIT = kgen_unit) cthl_2
                WRITE (UNIT = kgen_unit) mixt_frac
                WRITE (UNIT = kgen_unit) rsatl_1
                WRITE (UNIT = kgen_unit) rsatl_2
                WRITE (UNIT = kgen_unit) stdev_chi_1
                WRITE (UNIT = kgen_unit) stdev_chi_2
                WRITE (UNIT = kgen_unit) stdev_eta_1
                WRITE (UNIT = kgen_unit) covar_chi_eta_1
                WRITE (UNIT = kgen_unit) stdev_eta_2
                WRITE (UNIT = kgen_unit) covar_chi_eta_2
                WRITE (UNIT = kgen_unit) chi_1
                WRITE (UNIT = kgen_unit) cloud_frac_1
                WRITE (UNIT = kgen_unit) rc_1
                WRITE (UNIT = kgen_unit) chi_2
                WRITE (UNIT = kgen_unit) cloud_frac_2
                WRITE (UNIT = kgen_unit) rc_2
                WRITE (UNIT = kgen_unit) corr_chi_eta_1
                WRITE (UNIT = kgen_unit) corr_chi_eta_2
                IF (SIZE(alpha_sclr)==1) THEN
                    IF (UBOUND(alpha_sclr, 1)<LBOUND(alpha_sclr, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(alpha_sclr, 1)==0 .AND. LBOUND(alpha_sclr, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(alpha_sclr)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) alpha_sclr
                END IF 
                IF (SIZE(varnce_sclr1)==1) THEN
                    IF (UBOUND(varnce_sclr1, 1)<LBOUND(varnce_sclr1, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(varnce_sclr1, 1)==0 .AND. LBOUND(varnce_sclr1, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(varnce_sclr1)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) varnce_sclr1
                END IF 
                IF (SIZE(varnce_sclr2)==1) THEN
                    IF (UBOUND(varnce_sclr2, 1)<LBOUND(varnce_sclr2, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(varnce_sclr2, 1)==0 .AND. LBOUND(varnce_sclr2, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(varnce_sclr2)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) varnce_sclr2
                END IF 
                IF (SIZE(sclr1)==1) THEN
                    IF (UBOUND(sclr1, 1)<LBOUND(sclr1, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(sclr1, 1)==0 .AND. LBOUND(sclr1, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(sclr1)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) sclr1
                END IF 
                IF (SIZE(sclr2)==1) THEN
                    IF (UBOUND(sclr2, 1)<LBOUND(sclr2, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(sclr2, 1)==0 .AND. LBOUND(sclr2, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(sclr2)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) sclr2
                END IF 
                IF (SIZE(rsclrthl)==1) THEN
                    IF (UBOUND(rsclrthl, 1)<LBOUND(rsclrthl, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(rsclrthl, 1)==0 .AND. LBOUND(rsclrthl, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(rsclrthl)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) rsclrthl
                END IF 
                IF (SIZE(rsclrrt)==1) THEN
                    IF (UBOUND(rsclrrt, 1)<LBOUND(rsclrrt, 1)) THEN
                        kgen_istrue = .FALSE.
                    ELSE IF (UBOUND(rsclrrt, 1)==0 .AND. LBOUND(rsclrrt, 1)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                ELSE IF (SIZE(rsclrrt)==0) THEN
                    kgen_istrue = .FALSE.
                ELSE
                    kgen_istrue = .TRUE.
                END IF 
                WRITE (UNIT = kgen_unit) kgen_istrue
                IF (kgen_istrue) THEN
                    WRITE (UNIT = kgen_unit) rsclrrt
                END IF 
                WRITE (UNIT = kgen_unit) l_scalar_calc
                WRITE (UNIT = kgen_unit) l_calc_ice_supersat_frac
                WRITE (UNIT = kgen_unit) tl1
                WRITE (UNIT = kgen_unit) tl2
                ...
                WRITE (UNIT = kgen_unit) rtprxp
                WRITE (UNIT = kgen_unit) width_factor_1
                WRITE (UNIT = kgen_unit) width_factor_2
                WRITE (UNIT = kgen_unit) rt_at_ice_sat1
                WRITE (UNIT = kgen_unit) rt_at_ice_sat2
                WRITE (UNIT = kgen_unit) chi_at_ice_sat1
                WRITE (UNIT = kgen_unit) ice_supersat_frac1
                WRITE (UNIT = kgen_unit) rc_1_ice
                WRITE (UNIT = kgen_unit) chi_at_ice_sat2
                WRITE (UNIT = kgen_unit) ice_supersat_frac2
                WRITE (UNIT = kgen_unit) rc_2_ice
                WRITE (UNIT = kgen_unit) i
                WRITE (UNIT = kgen_unit) hm_idx
            END IF 
        END IF 
...
!$kgen end_callsite
    ...
    !write state subroutine for kw_pdf_closure_real__core_rknd_dim1
    SUBROUTINE kw_pdf_closure_real__core_rknd_dim1(var, kgen_unit, printvar)
    ...
    END SUBROUTINE kw_pdf_closure_real__core_rknd_dim1
