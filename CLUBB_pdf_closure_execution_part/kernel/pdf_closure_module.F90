...
!------------------------------------------------------------------------
  ...
  SUBROUTINE pdf_closure(kgen_unit, kgen_total_time, hydromet_dim, wp2, wm, rtm, thlm, skw, sigma_sqd_w, thlp2, wpthlp, rtp2, wprtp, rtpthlp, exner, p_in_pa, thv_ds, wp3, sclrm, sclrp2, wpsclrp, sclrpthlp, sclrprtp, level, wp2hmp, wphydrometp, thlphmp, rtphmp, wp2thlp, wp2rtp, wp2rcp, wprcp, thlprcp, rtprcp, rcm, rcp2, wp4, wprtp2, wpthlp2, cloud_frac, wpthvp, wp2thvp, rtpthvp, thlpthvp, wprtpthlp, ice_supersat_frac, pdf_params, err_code, sclrprcp, sclrpthvp, wpsclrp2, wpsclrprtp, wpsclrpthlp, wp2sclrp, rc_coef)
    ...
    ! Output Variables
      REAL(KIND=core_rknd) :: wp4, wprtp2, wp2rtp, wpthlp2, wp2thlp, cloud_frac, ice_supersat_frac, rcm, wpthvp, wp2thvp, rtpthvp, thlpthvp, wprcp, wp2rcp, rtprcp, thlprcp, rcp2, wprtpthlp
    ...
    ! Output (passive scalar variables)
      ...
      REAL(KIND=core_rknd), dimension(sclr_dim) :: sclrpthvp, sclrprcp, wpsclrp2, wpsclrprtp, wpsclrpthlp, wp2sclrp
...
!$kgen begin_callsite pdf_closure_part3
    ...
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) wp2sclrp
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) wpsclrp2
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) wpsclrprtp
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) wpsclrpthlp
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) sclrprcp
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) sclrpthvp
    END IF 
