...
!      use parkind, only : jpim, jprb 
      ...
      use dSFMT_interface, only : dSFMT_init, dSFMT_t, get_rand_arr_close_open
...
! Variables related to random number and seed 
      ...
      integer, dimension(ncol*nsubcol*nlay) :: seed1_str, seed2_str, seed3_str, seed4_str  ! seed to create random number
      real(kind=r8), dimension(ncol) :: rand_num                    ! random number (kissvec)
      real(kind=r8), dimension(ncol*nsubcol*nlay) :: rand_str       ! random number (kissvec)
          ...
          do isubcol = 1,nsubcol
            do i = 1,ncol
              if (pmid(i,nlay).lt.pmid(i,nlay-1)) then
                call endrun('MCICA_SUBCOL: KISSVEC SEaD GENERATOR REQUIRES PMID FROM BOTTOM FOUR LAYERS.')
              endif
              n = n + 1
              seed1_str(n) = (pmid(i,nlay)   - int(pmid(i,nlay)))    * 1000000000
              seed2_str(n) = (pmid(i,nlay-1) - int(pmid(i,nlay-1)))  * 1000000000
              seed3_str(n) = (pmid(i,nlay-2) - int(pmid(i,nlay-2)))  * 1000000000
              seed4_str(n) = (pmid(i,nlay-3) - int(pmid(i,nlay-3)))  * 1000000000
            enddo
          enddo
        enddo
        do i=1,changeSeed
          call kissvec(seed1_str, seed2_str, seed3_str, seed4_str, rand_str)
        enddo
      elseif (irnd == 1) then
!$OMP CRITICAL 
        call dSFMT_init(iseed, SIZE(rand_str), rng )
!$OMP END CRITICAL 
      endif 

      call system_clock(c2, cr, cm); dt1 = dble(c2-c1)/dble(cr) + dt1
      if ( masterproc) then
        print *, 'Total time RANDOM: SW_OPTS:dt1 =  ', dt1, changeSeed
      endif
...
! i) pick a random value at every level
         ...
         elseif (irnd == 1) then
!$OMP CRITICAL 
           call get_rand_arr_close_open( rng, rand_str, SIZE(rand_str) )
!$OMP END CRITICAL 
         endif
         ...
         do ilev = 1,nlay
           do isubcol = 1,nsubcol
             do i = 1,ncol
               n = n + 1
               CDF(isubcol,i,ilev) = rand_str(n)
             enddo
           enddo
         enddo
           
         call system_clock(c2, cr, cm); dt2 = dble(c2-c1)/dble(cr) + dt2
         if ( masterproc) then
           print *, 'Total time RANDOM: SW_OPTS:dt2 =  ', dt2
         endif
...
!    - if the layer above is clear, we use a new random number 
         ...
         if (irnd == 0) then 
           call kissvec(seed1_str, seed2_str, seed3_str, seed4_str, rand_str)
           call system_clock(c2, cr, cm); dt3 = dble(c2-c1)/dble(cr) + dt3
         elseif (irnd == 1) then
!$OMP CRITICAL 
           call get_rand_arr_close_open( rng, rand_str, SIZE(rand_str) )
!$OMP END CRITICAL 
         ...
         call system_clock(c2, cr, cm); dt3 = dble(c2-c1)/dble(cr) + dt3
         if ( masterproc) then
            print *, 'Total time RANDOM: SW_OPTS:dt3 =  ', dt3
         endif
...
!$OMP END CRITICAL 
         ...
         call system_clock(c2, cr, cm); dt4 = dble(c2-c1)/dble(cr) + dt4
         if ( masterproc) then
           print *, 'Total time RANDOM: SW_OPTS:dt4 =  ', dt4
         endif
