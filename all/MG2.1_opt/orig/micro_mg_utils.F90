...
subroutine size_dist_param_liq_line(props, qcic, ncic, rho, pgam, lamc)
  ...
  if (qcic > qsmall) then
     ...
     call size_dist_param_basic_line(props_loc, qcic, ncic, lamc)
...
!$OMP SIMD ALIGNED (qcic,ncic,pgam,rho)
  ...
  if (qcic(i) > qsmall) then
     ...
     call size_dist_param_basic_line(props_loc, qcic(i), ncic(i), lamc(i))
  endif
  enddo
...
subroutine ice_deposition_sublimation(t, qv, qi, ni, &
 icldm, rho, dv,qvl, qvi, berg, vap_dep, ice_sublim, mgncol)
  ...
  if (qi(i)>=qsmall) then
     ...
     call size_dist_param_basic_line(mg_ice_props, qiic, niic, lami, n0i)
...
subroutine contact_freezing(microp_uniform, t, p, rndst, nacon, &
  ...
  integer, intent(in) :: mgncol, mdust
  logical, intent(in) :: microp_uniform
  ...
  real(r8) :: nslip(mdust)  ! slip correction factors
  real(r8) :: ndfaer(mdust) ! aerosol diffusivities (m^2/sec)
...
subroutine heterogeneous_rain_freezing(t, qric, nric, lamr, &
                                      mnuccr, nnuccr, mgncol)
...
end subroutine accrete_cloud_ice_snow_orig
