...
echo 1
mpiexec.hydra -f hostfile.txt  -n 1 -env LD_LIBRARY_PATH $LD_LIBRARY_PATH ./kernel.avx2.exe >& hsw_v03.txt

for i in {2..72..2}; do
##  echo $i
##  echo ""
  mpiexec.hydra -f hostfile.txt  -n $i -env LD_LIBRARY_PATH $LD_LIBRARY_PATH ./kernel.avx2.exe >> hsw_v03.txt
done
