...
! rtp2, thlp2, rtpthlp, up2, vp2, wpsclrp, sclrprtp, sclrpthlp, and sclrp2.
        ...
        PUBLIC term_ma_zt_lhs, term_ma_zm_lhs
        CONTAINS
...
!=============================================================================
        ...
        pure FUNCTION term_ma_zt_lhs(wm_zt, invrs_dzt, level, invrs_dzm_k, invrs_dzm_km1) RESULT ( lhs )
! Description:
...
! Constant(s)
            USE model_flags, ONLY: l_upwind_xm_ma
! Variable(s)
...
! Input Variables
            ...
            REAL(KIND=core_rknd), intent(in) :: invrs_dzm_km1
            REAL(KIND=core_rknd), intent(in) :: wm_zt
            REAL(KIND=core_rknd), intent(in) :: invrs_dzm_k
! wm_zt(k)                        [m/s]
...
! Local Variables
            LOGICAL, parameter :: l_ub_const_deriv = .true.
! Flag to use the "one-sided" upper boundary.
            INTEGER :: mk
            INTEGER :: mkm1
...
! Most of the interior model; normal conditions.
       if( .not. l_upwind_xm_ma ) then  ! Use centered differencing ! Use centered differencing
! Thermodynamic superdiagonal: [ x var_zt(k+1,<t+1>) ]
          lhs(kp1_tdiag) & 
...
! Thermodynamic subdiagonal: [ x var_zt(k-1,<t+1>) ]
       ...
       else ! l_upwind_xm_ma == .true.; use "upwind" differencing ! l_upwind_xm_ma == .true.; use "upwind" differencing
          if ( wm_zt >= zero ) then  ! Mean wind is in upward direction ! Mean wind is in upward direction
! Thermodynamic superdiagonal: [ x var_zt(k+1,<t+1>) ]
             lhs(kp1_tdiag) &
             = zero
! Thermodynamic main diagonal: [ x var_zt(k,<t+1>) ]
...
! Thermodynamic subdiagonal: [ x var_zt(k-1,<t+1>) ]
             lhs(km1_tdiag) &
             = - wm_zt * invrs_dzm_km1
          else  ! wm_zt < 0; Mean wind is in downward direction ! wm_zt < 0; Mean wind is in downward direction
! Thermodynamic superdiagonal: [ x var_zt(k+1,<t+1>) ]
             lhs(kp1_tdiag) &
             = + wm_zt * invrs_dzm_k
! Thermodynamic main diagonal: [ x var_zt(k,<t+1>) ]
...
! Thermodynamic subdiagonal: [ x var_zt(k-1,<t+1>) ]
             lhs(km1_tdiag) &
             = zero
          endif ! wm_zt > 0 ! wm_zt > 0
       endif ! l_upwind_xm_ma ! l_upwind_xm_ma
...
! Thermodynamic subdiagonal: [ x var_zt(k-1,<t+1>) ]
             lhs(km1_tdiag) & 
             = - wm_zt * invrs_dzt * gr%weights_zt2zm(t_below,mkm1)
          endif ! l_ub_const_deriv ! l_ub_const_deriv
       else ! l_upwind_xm_ma == .true.; use "upwind" differencing ! l_upwind_xm_ma == .true.; use "upwind" differencing
          if ( wm_zt >= zero ) then  ! Mean wind is in upward direction ! Mean wind is in upward direction
! Thermodynamic superdiagonal: [ x var_zt(k+1,<t+1>) ]
             lhs(kp1_tdiag) &
...
! Thermodynamic subdiagonal: [ x var_zt(k-1,<t+1>) ]
    ...
    endif ! level = gr%nz ! level = gr%nz
    return
        END FUNCTION term_ma_zt_lhs
!=============================================================================

        pure FUNCTION term_ma_zm_lhs(wm_zm, invrs_dzm, level) RESULT ( lhs )
! Description:
