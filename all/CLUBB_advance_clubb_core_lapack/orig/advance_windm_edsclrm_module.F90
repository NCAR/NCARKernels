...
!
! Filename    : advance_windm_edsclrm_module.F90
...
! KGEN version: 0.5.3
    ...
    MODULE advance_windm_edsclrm_module
        ...
        PRIVATE compute_uv_tndcy, windm_edsclrm_rhs, windm_edsclrm_lhs, windm_edsclrm_solve
...
!=============================================================================
        ...
        SUBROUTINE advance_windm_edsclrm(dt, wm_zt, km_zm, ug, vg, um_ref, vm_ref, wp2, up2, vp2, um_forcing, vm_forcing, &
        edsclrm_forcing, rho_ds_zm, invrs_rho_ds_zt, fcor, l_implemented, um, vm, edsclrm, upwp, vpwp, wpedsclrp, err_code)
! Description:
...
! Variable(s)
            USE parameters_tunable, ONLY: nu10_vert_res_dep
! Constant
            USE model_flags, ONLY: l_uv_nudge
...
! Subroutines
            USE stats_variables, ONLY: iwindm_matrix_condt_num
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: ium_sdmp
            USE stats_variables, ONLY: stats_zt
            USE stats_variables, ONLY: ivm_sdmp
            USE stats_variables, ONLY: ium_ndg
            USE stats_variables, ONLY: ivm_ndg
            USE stats_variables, ONLY: ium_ref
            USE stats_variables, ONLY: ivm_ref
! Variables
...
! Constant(s)
            USE sponge_layer_damping, ONLY: uv_sponge_damp_settings
            USE sponge_layer_damping, ONLY: uv_sponge_damp_profile
...
! External
            INTRINSIC real
! Constant Parameters
            REAL(KIND=core_rknd), dimension(gr%nz) :: dummy_nu
! Used to feed zero values into function calls
! Input Variables
...
! Eddy diffusivity of winds on momentum levels [m^2/s]
! u (west-to-east) geostrophic wind comp.      [m/s]
! v (south-to-north) geostrophic wind comp.    [m/s]
! Reference u wind component for nudging       [m/s]
! Reference v wind component for nudging       [m/s]
...
! Inv. dry, static density at thermo. levels   [m^3/kg]
            REAL(KIND=core_rknd), dimension(gr%nz,edsclr_dim), intent(in) :: edsclrm_forcing
! Eddy scalar large-scale forcing             [{units vary}/s]
...
! Mean u (west-to-east) wind component          [m/s]
! Mean v (south-to-north) wind component        [m/s]
! Input/Output Variable for eddy-scalars
            REAL(KIND=core_rknd), dimension(gr%nz,edsclr_dim), intent(inout) :: edsclrm
! Mean eddy scalar quantity                     [units vary]
! Output Variables
...
! w'edsclr' (momentum levels)                   [units vary]
            INTEGER, intent(inout) :: err_code
! clubb_singular_matrix when matrix is singular
! Local Variables
            REAL(KIND=core_rknd), dimension(gr%nz) :: um_tndcy
            REAL(KIND=core_rknd), dimension(gr%nz) :: vm_tndcy
! u wind component tendency                     [m/s^2]
! v wind component tendency                     [m/s^2]
            REAL(KIND=core_rknd), dimension(gr%nz) :: upwp_chnge
            REAL(KIND=core_rknd), dimension(gr%nz) :: vpwp_chnge
! Net change of u'w' due to clipping            [m^2/s^2]
! Net change of v'w' due to clipping            [m^2/s^2]
            REAL(KIND=core_rknd), dimension(3,gr%nz) :: lhs
! The implicit part of the tridiagonal matrix              [units vary]
            REAL(KIND=core_rknd), dimension(gr%nz,max(2,edsclr_dim)) :: rhs
            REAL(KIND=core_rknd), dimension(gr%nz,max(2,edsclr_dim)) :: solution
! The explicit part of the tridiagonal matrix        [units vary]
! The solution to the tridiagonal matrix             [units vary]
            REAL(KIND=core_rknd), dimension(gr%nz) :: wind_speed
! wind speed; sqrt(u^2 + v^2)                      [m/s]
            REAL(KIND=core_rknd) :: u_star_sqd
! Surface friction velocity, u_star, squared       [m/s]
            LOGICAL :: l_imp_sfc_momentum_flux
! Flag for implicit momentum surface fluxes.
            INTEGER :: err_code_windm
            INTEGER :: err_code_edsclrm
            INTEGER :: nrhs
! Error code for each LAPACK solve
! Number of right hand side terms
            INTEGER :: i ! Array index
            LOGICAL :: l_first_clip_ts
            LOGICAL :: l_last_clip_ts ! flags for clip_covar
!--------------------------- Begin Code ------------------------------------
...
! Decompose and back substitute for um and vm
    ...
    call windm_edsclrm_solve( nrhs, iwindm_matrix_condt_num, & ! in
                              lhs, rhs, &                      ! in/out
                              solution, err_code_windm )       ! out ! in
! in/out
! out
!----------------------------------------------------------------
...
! Implicit contributions to um and vm
      call windm_edsclrm_implicit_stats( windm_edsclrm_um, um ) ! in ! in
      call windm_edsclrm_implicit_stats( windm_edsclrm_vm, vm ) ! in ! in
...
! Decompose and back substitute for all eddy-scalar variables
      call windm_edsclrm_solve( edsclr_dim, 0, &     ! in
                                lhs, rhs, &          ! in/out
                                solution, err_code_edsclrm ) ! out ! in
! in/out
! out
!----------------------------------------------------------------
...
!=============================================================================
        ...
        SUBROUTINE windm_edsclrm_solve(nrhs, ixm_matrix_condt_num, lhs, rhs, solution, err_code)
...
! Number of right-hand side (explicit) vectors & Number of solution vectors.
            INTEGER, intent(in) :: ixm_matrix_condt_num
! Stats index of the condition numbers
            REAL(KIND=core_rknd), dimension(3,gr%nz), intent(inout) :: lhs
! Implicit contributions to um, vm, and eddy scalars  [units vary]
            REAL(KIND=core_rknd), dimension(gr%nz,nrhs), intent(inout) :: rhs
! Right-hand side (explicit) contributions.
            REAL(KIND=core_rknd), dimension(gr%nz,nrhs), intent(out) :: solution
! Solution to the system of equations    [units vary]
            INTEGER, intent(out) :: err_code
! clubb_singular_matrix when matrix is singular
! Local variables
            REAL(KIND=core_rknd) :: rcond
! Estimate of the reciprocal of the condition number on the LHS matrix
...
! None
!-----------------------------------------------------------------------
            USE stats_variables, ONLY: ium_ma
            USE stats_variables, ONLY: ium_ta
            USE stats_variables, ONLY: ivm_ma
            USE stats_variables, ONLY: ivm_ta
            USE stats_variables, ONLY: stats_zt
...
! Desc. of what is being solved for
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: xm
!  Computed value um or vm at <t+1>    [m/s]
! Local variables
            INTEGER :: k
            INTEGER :: km1
...
!=============================================================================
        ...
        SUBROUTINE compute_uv_tndcy(solve_type, fcor, perp_wind_m, perp_wind_g, xm_forcing, l_implemented, xm_tndcy)
! Description:
...
!-----------------------------------------------------------------------
            USE grid_class, ONLY: gr
            USE stats_type_utilities, ONLY: stat_update_var
            USE stats_variables, ONLY: ium_gf
            USE stats_variables, ONLY: ium_cf
            USE stats_variables, ONLY: ium_f
            USE stats_variables, ONLY: ivm_gf
            USE stats_variables, ONLY: ivm_cf
            USE stats_variables, ONLY: ivm_f
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: stats_zt
            USE clubb_precision, ONLY: core_rknd
! Variable(s)
...
! Input Variables
            INTEGER, intent(in) :: solve_type
! Description of what is being solved for
            REAL(KIND=core_rknd), intent(in) :: fcor
! Coriolis parameter     [s^-1]
...
! Perpendicular component of the mean wind (e.g. v, for the u-eqn) [m/s]
! Perpendicular component of the geostropic wind (e.g. vg)         [m/s]
! Prescribed wind forcing                                          [m/s/s]
            LOGICAL, intent(in) :: l_implemented
! Flag for CLUBB being implemented in a larger model.
! Output Variables
...
! Local Variables
            ...
            INTEGER :: ixm_cf
            INTEGER :: ixm_f
            REAL(KIND=core_rknd), dimension(gr%nz) :: xm_gf
            REAL(KIND=core_rknd), dimension(gr%nz) :: xm_cf
! --- Begin Code ---
...
! xm term F
    ...
    else   ! implemented in a host model. ! implemented in a host model.
...
!===============================================================================
        ...
        SUBROUTINE windm_edsclrm_lhs(dt, nu, wm_zt, km_zm, wind_speed, u_star_sqd, rho_ds_zm, invrs_rho_ds_zt, l_implemented, &
        l_imp_sfc_momentum_flux, lhs)
! Description:
...
! Surface friction velocity, u_*, squared  [m/s]
...
! Implicit contributions to xm (tridiagonal matrix)
! Local Variables
            INTEGER :: k
...
! Modify zmscr05 accordingly.
    ...
    endif ! l_imp_sfc_momentum_flux ! l_imp_sfc_momentum_flux
    return
        END SUBROUTINE windm_edsclrm_lhs
!=============================================================================

        FUNCTION windm_edsclrm_rhs(solve_type, dt, nu, km_zm, xm, xm_tndcy, rho_ds_zm, invrs_rho_ds_zt, l_imp_sfc_momentum_flux, &
        xpwp_sfc) RESULT ( rhs )
! Description:
...
! Background constant coef. of eddy diffusivity        [m^2/s]
            ...
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: rho_ds_zm
! Eddy diffusivity on momentum levels        [m^2/s]
! Eddy-scalar variable, xm (thermo. levels)  [units vary]
! The explicit time-tendency acting on xm    [units vary]
! Dry, static density on momentum levels     [kg/m^3]
! Inv. dry, static density at thermo. levels [m^3/kg]
            REAL(KIND=core_rknd), intent(in) :: xpwp_sfc
! x'w' at the surface                              [units vary]
            LOGICAL, intent(in) :: l_imp_sfc_momentum_flux
! Flag for implicit momentum surface fluxes.
! Output Variable
            REAL(KIND=core_rknd), dimension(gr%nz) :: rhs
! Right-hand side (explicit) contributions.
! Local Variables
            INTEGER :: k
            INTEGER :: km1
...
! Variable(s)
            IMPLICIT NONE
! Input variables
            REAL(KIND=core_rknd), intent(in) :: km_zm
            REAL(KIND=core_rknd), intent(in) :: xm
            REAL(KIND=core_rknd), intent(in) :: xmp1
            REAL(KIND=core_rknd), intent(in) :: invrs_dzm
! Eddy diff. (k momentum level)                 [m^2/s]
! x (k thermo level)                            [units vary]
! x (k+1 thermo level)                          [units vary]
! Inverse of the grid spacing (k thermo level)  [1/m]
! Output variable
            REAL(KIND=core_rknd) :: xpwp_fnc
! x'w'   [(units vary)(m/s)]
!-----------------------------------------------------------------------
! --- Begin Code ---
! Solve for x'w' at all intermediate model levels.
