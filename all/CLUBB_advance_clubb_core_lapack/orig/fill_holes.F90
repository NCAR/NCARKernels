...
! KGEN version: 0.5.3
    ...
    MODULE fill_holes
        ...
        IMPLICIT NONE
        PUBLIC vertical_integral, fill_holes_vertical, vertical_avg
...
!=============================================================================
        ...
        SUBROUTINE fill_holes_vertical(num_draw_pts, threshold, field_grid, rho_ds, rho_ds_zm, field)
! Description:
...
! Variable(s)
            IMPLICIT NONE
! Input variables
            INTEGER, intent(in) :: num_draw_pts
! The number of points on either side of the hole;
! Mass is drawn from these points to fill the hole.  []
            REAL(KIND=core_rknd), intent(in) :: threshold
! A threshold (e.g. w_tol*w_tol) below which field must not
...
! The field (e.g. wp2) that contains holes [Units same as threshold]
! Local Variables
            INTEGER :: upper_hf_level
            INTEGER :: k
            INTEGER :: begin_idx
            INTEGER :: end_idx
! Loop index for absolute grid level              []
! Lower grid level of local hole-filling range    []
! Upper grid level of local hole-filling range    []
! Upper grid level of global hole-filling range   []
!-----------------------------------------------------------------------
...
! 'field' is on the zm (momentum level) grid
    ...
    endif  ! End overall check for existence of holes ! End overall check for existence of holes
    return
        END SUBROUTINE fill_holes_vertical
!=============================================================================

        SUBROUTINE fill_holes_multiplicative(begin_idx, end_idx, threshold, rho, invrs_dz, field)
! Description:
! This subroutine clips values of 'field' that are below 'threshold' as much
! as possible (i.e. "fills holes"), but conserves the total integrated mass
! of 'field'.  This prevents clipping from acting as a spurious source.
...
! Input variables
            ...
            INTEGER, intent(in) :: begin_idx
! The beginning index (e.g. k=2) of the range of hole-filling
! The end index (e.g. k=gr%nz) of the range of hole-filling
            REAL(KIND=core_rknd), intent(in) :: threshold
! A threshold (e.g. w_tol*w_tol) below which field must not fall
...
! Dry, static density on either thermodynamic or momentum levels   [kg/m^3]
! Reciprocal of thermodynamic or momentum level thickness depending on whether
! we're on zt or zm grid.
! Input/Output variable
            REAL(KIND=core_rknd), dimension(end_idx-begin_idx+1), intent(inout) :: field
! The field (e.g. wp2) that contains holes
!                                  [Units same as threshold]
! Local Variables
            REAL(KIND=core_rknd), dimension(end_idx-begin_idx+1) :: field_clipped
! The raw field (e.g. wp2) that contains no holes
!                          [Units same as threshold]
            REAL(KIND=core_rknd) :: field_avg
            REAL(KIND=core_rknd) :: field_clipped_avg
            REAL(KIND=core_rknd) :: mass_fraction
! Vertical average of field [Units of field]
! Vertical average of clipped field [Units of field]
! Coefficient that multiplies clipped field
...
!=============================================================================
        ...
        FUNCTION vertical_avg(total_idx, rho_ds, field, invrs_dz)
! Description:
...
! The total numer of indices within the range of averaging
...
! Dry, static density on either thermodynamic or momentum levels    [kg/m^3]
! The field (e.g. wp2) to be vertically averaged                    [Units vary]
! Reciprocal of thermodynamic or momentum level thickness           [1/m]
...
! Output variable
            REAL(KIND=core_rknd) :: vertical_avg
! Vertical average of field    [Units of field]
! Local variables
            REAL(KIND=core_rknd) :: numer_integral
            REAL(KIND=core_rknd) :: denom_integral
! Integral in the numerator (see description)
! Integral in the denominator (see description)
            REAL(KIND=core_rknd), dimension(total_idx) :: denom_field
! When computing the vertical integral in the denominator
...
!=============================================================================
        ...
        pure FUNCTION vertical_integral(total_idx, rho_ds, field, invrs_dz)
! Description:
...
! The total numer of indices within the range of averaging
            ...
            REAL(KIND=core_rknd), dimension(total_idx), intent(in) :: invrs_dz
            REAL(KIND=core_rknd), dimension(total_idx), intent(in) :: field
! Dry, static density                   [kg/m^3]
! The field to be vertically averaged   [Units vary]
! Level thickness                       [1/m]
! Note:  The rho_ds and field points need to be arranged from
!        lowest to highest in altitude, with rho_ds(1) and
!        field(1) actually their respective values at level k = begin_idx.
! Local variables
            REAL(KIND=core_rknd) :: vertical_integral
! Integral in the numerator (see description)
!-----------------------------------------------------------------------
