...
!
! Filename    : mono_flux_limiter.F90
...
! KGEN version: 0.5.3
    ...
    MODULE mono_flux_limiter
        ...
        PUBLIC calc_turb_adv_range, monotonic_turbulent_flux_limit
        PRIVATE mean_vert_vel_up_down, mfl_xm_lhs, mfl_xm_rhs, mfl_xm_solve
...
! Named constant for thlm mono_flux calls
! Named constant for rtm mono_flux calls
        CONTAINS
...
!=============================================================================
        ...
        SUBROUTINE monotonic_turbulent_flux_limit(solve_type, dt, xm_old, xp2, wm_zt, xm_forcing, rho_ds_zm, rho_ds_zt, &
        invrs_rho_ds_zm, invrs_rho_ds_zt, xp2_threshold, l_implemented, low_lev_effect, high_lev_effect, xm, xm_tol, wpxp, &
        err_code)
! Description:
...
! Procedure(s)
            USE stats_variables, ONLY: iwprtp_mfl
            USE stats_variables, ONLY: irtm_mfl
            USE stats_variables, ONLY: iwpthlp_mfl
            USE stats_variables, ONLY: ithlm_mfl
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: stats_zm
            USE stats_variables, ONLY: stats_zt
            USE stats_variables, ONLY: ithlm_enter_mfl
            USE stats_variables, ONLY: ithlm_old
            USE stats_variables, ONLY: iwpthlp_entermfl
            USE stats_variables, ONLY: irtm_enter_mfl
            USE stats_variables, ONLY: irtm_old
            USE stats_variables, ONLY: iwprtp_enter_mfl
            USE stats_variables, ONLY: ithlm_without_ta
            USE stats_variables, ONLY: ithlm_mfl_min
            USE stats_variables, ONLY: ithlm_mfl_max
            USE stats_variables, ONLY: iwpthlp_mfl_min
            USE stats_variables, ONLY: iwpthlp_mfl_max
            USE stats_variables, ONLY: irtm_without_ta
            USE stats_variables, ONLY: irtm_mfl_min
            USE stats_variables, ONLY: irtm_mfl_max
            USE stats_variables, ONLY: iwprtp_mfl_min
            USE stats_variables, ONLY: iwprtp_mfl_max
            USE stats_variables, ONLY: ithlm_exit_mfl
            USE stats_variables, ONLY: iwpthlp_exit_mfl
            USE stats_variables, ONLY: irtm_exit_mfl
            USE stats_variables, ONLY: iwprtp_exit_mfl
! Variable(s)
            IMPLICIT NONE
! Constant Parameters
...
! xm at previous time step (thermo. levs.) [units vary]
! x'^2 (momentum levels)                   [units vary]
...
! Lower limit of x'^2                      [units vary]
! Lower limit of maxdev                    [units vary]
...
! Index of lowest level that has an effect (for lev. k)
! Index of highest level that has an effect (for lev. k)
! Input/Output Variables
            REAL(KIND=core_rknd), dimension(gr%nz), intent(inout) :: wpxp
            REAL(KIND=core_rknd), dimension(gr%nz), intent(inout) :: xm
! xm at current time step (thermodynamic levels)  [units vary]
! w'x' (momentum levels)                          [units vary]
! Output Variable
            INTEGER, intent(out) :: err_code
! Returns an error code in the event of a singular matrix
! Local Variables
            REAL(KIND=core_rknd), dimension(gr%nz) :: wpxp_net_adjust
            REAL(KIND=core_rknd), dimension(gr%nz) :: dxm_dt_mfl_adjust
            REAL(KIND=core_rknd), dimension(gr%nz) :: xm_enter_mfl
            REAL(KIND=core_rknd), dimension(gr%nz) :: xp2_zt
            REAL(KIND=core_rknd), dimension(gr%nz) :: xm_without_ta
! x'^2 interpolated to thermodynamic levels  [units vary]
! xm as it enters the MFL                    [units vary]
! Value of xm without turb. adv. contrib.    [units vary]
! Net amount of adjustment needed on w'x'    [units vary]
! Rate of change of adjustment to xm         [units vary]
            REAL(KIND=core_rknd), dimension(gr%nz) :: min_x_allowable_lev
            REAL(KIND=core_rknd), dimension(gr%nz) :: max_x_allowable_lev
            REAL(KIND=core_rknd), dimension(gr%nz) :: min_x_allowable
            REAL(KIND=core_rknd), dimension(gr%nz) :: max_x_allowable
            REAL(KIND=core_rknd), dimension(gr%nz) :: wpxp_mfl_max
            REAL(KIND=core_rknd), dimension(gr%nz) :: wpxp_mfl_min
! Smallest usuable value of x at lev k [units vary]
! Largest usuable value of x at lev k  [units vary]
! Smallest usuable x within k +/- num_levs [units vary]
! Largest usuable x within k +/- num_levs  [units vary]
! Upper limit on w'x'(k)                [units vary]
! Lower limit on w'x'(k)                [units vary]
            REAL(KIND=core_rknd) :: max_xp2
            REAL(KIND=core_rknd) :: dz
            REAL(KIND=core_rknd) :: stnd_dev_x
            REAL(KIND=core_rknd) :: max_dev
            REAL(KIND=core_rknd) :: m_adv_term
            REAL(KIND=core_rknd) :: xm_density_weighted
            REAL(KIND=core_rknd) :: xm_vert_integral
            REAL(KIND=core_rknd) :: xm_adj_coef
! Maximum allowable x'^2                        [units vary]
! Standard deviation of x                       [units vary]
! Determines approximate upper/lower limit of x [units vary]
! Contribution of mean advection to d(xm)/dt    [units vary]
! Density weighted xm at domain top             [units vary]
! Coeffecient to eliminate spikes at domain top [units vary]
! Vertical integral of xm                       [units_vary]
! zm grid spacing at top of domain              [m]
            REAL(KIND=core_rknd), dimension(3,gr%nz) :: lhs_mfl_xm
! Left hand side of tridiagonal matrix
            REAL(KIND=core_rknd), dimension(gr%nz) :: rhs_mfl_xm
! Right hand side of tridiagonal matrix equation
            INTEGER :: k
            INTEGER :: km1
! Array indices
!    integer, parameter :: &
...
!                     ! maxima and minima of variable x.
            INTEGER :: low_lev
            INTEGER :: high_lev
! Lowest level (from level k) to look for x minima and maxima
! Highest level (from level k) to look for x minima and maxima
            INTEGER :: iwpxp_mfl
            INTEGER :: ixm_mfl
!--- Begin Code ---
...
! Check for errors
       ...
       else  ! l_mfl_xm_imp_adj = .false. ! l_mfl_xm_imp_adj = .false.
...
! Boundary condition on xm
          xm(1) = xm(2)
       endif  ! l_mfl_xm_imp_adj ! l_mfl_xm_imp_adj
...
!endif
          endif ! xm_vert_integral < eps ! xm_vert_integral < eps
       endif ! spike at domain top ! spike at domain top
    endif ! any( wpxp_net_adjust(:) /= 0.0_core_rknd ) ! any( wpxp_net_adjust(:) /= 0.0_core_rknd )
    if ( l_stats_samp ) then
...
!=============================================================================
        ...
        SUBROUTINE mfl_xm_lhs(dt, wm_zt, l_implemented, lhs)
! Description:
...
!=============================================================================
        ...
        SUBROUTINE mfl_xm_rhs(dt, xm_old, wpxp, xm_forcing, rho_ds_zm, invrs_rho_ds_zt, rhs)
! Description:
! This subroutine is part of the process of re-solving for xm at timestep
! index (t+1).  This is done because the original solving process produced
! values outside of what is deemed acceptable by the monotonic flux limiter.
...
! Model timestep length                    [s]
            ...
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: rho_ds_zm
            REAL(KIND=core_rknd), dimension(gr%nz), intent(in) :: invrs_rho_ds_zt
! xm; timestep (t) (thermodynamic levels)  [units vary]
! w'x'; timestep (t+1); limited (m-levs.)  [units vary]
! xm forcings (thermodynamic levels)       [units vary]
! Dry, static density on momentum levels   [kg/m^3]
! Inv. dry, static density @ thermo. levs. [m^3/kg]
! Output Variable
...
!=============================================================================
        ...
        SUBROUTINE mfl_xm_solve(solve_type, lhs, rhs, xm, err_code)
! Description:
! This subroutine is part of the process of re-solving for xm at timestep
! index (t+1).  This is done because the original solving process produced
! values outside of what is deemed acceptable by the monotonic flux limiter.
...
! Output Variables
            REAL(KIND=core_rknd), dimension(gr%nz), intent(inout) :: xm
! Value of variable being solved for at timestep (t+1)   [units vary]
            INTEGER, intent(out) :: err_code
! Returns an error code in the event of a singular matrix
! Local variable
...
!=============================================================================
        ...
        SUBROUTINE calc_turb_adv_range(dt, w_1_zm, w_2_zm, varnce_w_1_zm, varnce_w_2_zm, mixt_frac_zm, low_lev_effect, &
        high_lev_effect)
! Description:
...
! Variable(s)
            IMPLICIT NONE
! Constant parameters
            LOGICAL, parameter :: l_constant_thickness = .false.
! Toggle constant or variable thickness.
            REAL(KIND=core_rknd), parameter :: const_thick = 150.0_core_rknd
! Constant thickness value               [m]
! Input Variables
            REAL(KIND=core_rknd), intent(in) :: dt
! Model timestep length                       [s]
...
! Index of highest level that has an effect (for lev. k)
! Local Variables
            REAL(KIND=core_rknd), dimension(gr%nz) :: vert_vel_up
            REAL(KIND=core_rknd), dimension(gr%nz) :: vert_vel_down
! Average upwards vertical velocity component   [m/s]
! Average downwards vertical velocity component [m/s]
            REAL(KIND=core_rknd) :: dt_all_grid_levs
            REAL(KIND=core_rknd) :: dt_one_grid_lev
! Amount of time to travel one grid box           [s]
! Running count of amount of time taken to travel [s]
            INTEGER :: k
            INTEGER :: j
! ---- Begin Code ----
    if ( l_constant_thickness ) then ! thickness is a constant value. ! thickness is a constant value.
...
! the central thermodynamic level.
          j = k - 1
          do ! loop downwards until answer is found. ! loop downwards until answer is found.
...
! Increment to the next vertical level down.
          ...
          enddo ! downwards loop ! downwards loop
! Compute the number of levels that effect the central thermodynamic
...
! the central thermodynamic level.
          j = k + 1
          do ! loop upwards until answer is found. ! loop upwards until answer is found.
...
! Increment to the next vertical level up.
          ...
          enddo ! upwards loop ! upwards loop
       enddo ! k = 3, gr%nz-2 ! k = 3, gr%nz-2
    else ! thickness based on vertical velocity and time step length. ! thickness based on vertical velocity and time step length.
...
! that can be considered is one-level-below the current level.
          ...
          enddo  ! upwards loop ! upwards loop
       enddo ! k = 3, gr%nz-2 ! k = 3, gr%nz-2
    endif ! l_constant_thickness ! l_constant_thickness
...
!=============================================================================
        ...
        SUBROUTINE mean_vert_vel_up_down(w_1_zm, w_2_zm, varnce_w_1_zm, varnce_w_2_zm, mixt_frac_zm, w_ref, mean_w_down, &
        mean_w_up)
! Description
...
! Procedure(s)
            USE stats_variables, ONLY: l_stats_samp
            USE stats_variables, ONLY: imean_w_up
            USE stats_variables, ONLY: stats_zm
            USE stats_variables, ONLY: imean_w_down
! Variable(s)
            USE clubb_precision, ONLY: core_rknd
! Variable(s)
            IMPLICIT NONE
...
! Reference velocity, w|_ref (normally = 0)   [m/s]
! Output Variables
            REAL(KIND=core_rknd), dimension(gr%nz), intent(out) :: mean_w_up
            REAL(KIND=core_rknd), dimension(gr%nz), intent(out) :: mean_w_down
! Overall mean w (<= w|_ref)                  [m/s]
! Overall mean w (>= w|_ref)                  [m/s]
! Local Variables
            REAL(KIND=core_rknd) :: sigma_w_1
            REAL(KIND=core_rknd) :: sigma_w_2
            REAL(KIND=core_rknd) :: mean_w_down_1st
            REAL(KIND=core_rknd) :: mean_w_up_1st
            REAL(KIND=core_rknd) :: exp_cache
            REAL(KIND=core_rknd) :: erf_cache
            REAL(KIND=core_rknd) :: mean_w_down_2nd
            REAL(KIND=core_rknd) :: mean_w_up_2nd
! Standard deviation of w for 1st normal distribution    [m/s]
! Standard deviation of w for 2nd normal distribution    [m/s]
! Mean w (<= w|_ref) from 1st normal distribution [m/s]
! Mean w (<= w|_ref) from 2nd normal distribution [m/s]
! Mean w (>= w|_ref) from 1st normal distribution [m/s]
! Mean w (>= w|_ref) from 2nd normal distribution [m/s]
! Cache of exponential calculations to reduce runtime
! Cache of error function calculations to reduce runtime
