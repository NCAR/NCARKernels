...
!
! Filename    : clip_semi_implicit.F90
...
! Variable(s)
        IMPLICIT NONE
        PRIVATE
        PUBLIC clip_semi_imp_lhs, clip_semi_imp_rhs
        PRIVATE compute_fncts_a_b, compute_clip_lhs
! Constant parameters.
...
!=============================================================================
        ...
        FUNCTION clip_semi_imp_lhs(dt, f_unclipped, l_upper_thresh, upper_threshold, l_lower_thresh, lower_threshold) RESULT ( &
        lhs )
! Description:
...
! The unclipped value of variable f at timestep (t). [f units]
! Greatest allowable value of variable f.            [f units]
! Smallest allowable value of variable f.            [f units]
            LOGICAL, intent(in) :: l_lower_thresh
            LOGICAL, intent(in) :: l_upper_thresh
! Flag for having an upper threshold value.
! Flag for having a lower threshold value.
! Return Variable
            REAL(KIND=core_rknd) :: lhs
! Local Variables
            REAL(KIND=core_rknd) :: dt_clip
! Time scale for semi-implicit clipping term.                [s]
            REAL(KIND=core_rknd) :: f_diff
            REAL(KIND=core_rknd) :: a_fnc
            REAL(KIND=core_rknd) :: b_fnc
            REAL(KIND=core_rknd) :: lhs_upper
            REAL(KIND=core_rknd) :: lhs_lower
! Difference between the threshold value and f_unclipped.    [f units]
! Function that approximates { f_diff * [ 1 - H(f_diff) ] }. [f units]
! Derivative w/ respect to f_diff of function A_fnc.         []
! Contribution of upper threshold to implicit portion (LHS). [s^-1]
! Contribution of lower threshold to implicit portion (LHS). [s^-1]
...
!=============================================================================
        ...
        pure FUNCTION compute_clip_lhs(dt_clip, b_fnc) RESULT ( lhs_contribution )
! Description:
...
! Variable(s)
            IMPLICIT NONE
! Input Variables
            REAL(KIND=core_rknd), intent(in) :: dt_clip
! Time scale for semi-implicit clipping term.        [s]
...
!=============================================================================
        ...
        FUNCTION clip_semi_imp_rhs(dt, f_unclipped, l_upper_thresh, upper_threshold, l_lower_thresh, lower_threshold) RESULT ( &
        rhs )
! Description:
...
! Time scale for semi-implicit clipping term.                [s]
            ...
            REAL(KIND=core_rknd) :: b_fnc
            REAL(KIND=core_rknd) :: a_fnc
            REAL(KIND=core_rknd) :: rhs_upper
            REAL(KIND=core_rknd) :: rhs_lower
! Difference between the threshold value and f_unclipped.    [f units]
! Function that approximates { f_diff * [ 1 - H(f_diff) ] }. [f units]
! Derivative w/ respect to f_diff of function A_fnc.         []
! Contribution of upper threshold to explicit portion (RHS). [s^-1]
! Contribution of lower threshold to explicit portion (RHS). [s^-1]
! Compute the clipping time scale, dt_clip.
    dt_clip = dt_clip_coef * dt
! Upper Threshold
    if ( l_upper_thresh ) then
...
!=============================================================================
        ...
        SUBROUTINE compute_fncts_a_b(l_upper_thresh, upper_threshold, l_lower_thresh, lower_threshold, f_diff, a_fnc, b_fnc)
! Description:
...
! Derivative w/ respect to f_diff of function A_fnc.         []
! Local Variables
            REAL(KIND=core_rknd) :: thresh_avg_mag
            REAL(KIND=core_rknd) :: sigma_val ! Value of parameter sigma. [f units]
! Average magnitude of threshold(s).         [f units]
    thresh_avg_mag = 0.0_core_rknd ! Default Initialization ! Default Initialization
...
! at all points.  It is evaluated for f_diff at timestep index (t).
    ...
    elseif ( f_diff >= sigma_val ) then
      A_fnc = 0.0_core_rknd
    else ! -sigma_val < f_diff < sigma_val ! -sigma_val < f_diff < sigma_val
