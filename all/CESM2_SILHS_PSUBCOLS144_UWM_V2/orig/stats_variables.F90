...
!$omp   icorr_chi_Ncn_2_n, icorr_eta_Ncn_1_n, icorr_eta_Ncn_2_n )
    ...
    INTEGER, dimension(:), allocatable, public :: ilh_samp_frac_category 
...
!$omp   threadprivate(ilh_Vrr,  ilh_VNr)
    ...
    INTEGER, public :: ilh_precip_frac = 0, ilh_mixt_frac = 0 
...
!$omp   ilh_chi, ilh_eta, ilh_precip_frac, ilh_mixt_frac )
    ...
    INTEGER, public :: ilh_precip_frac_unweighted = 0, ilh_mixt_frac_unweighted     = 0 

!$omp threadprivate( ilh_cloud_frac_unweighted, ilh_precip_frac_unweighted, &
!$omp                ilh_mixt_frac_unweighted )

  ! None covariance estimate indicies
...
!$omp threadprivate( irtp2_from_chi )
    ...
    TYPE(stats), target, public :: stats_lh_zt, stats_lh_sfc 
  ! Scratch space
...
!$omp   threadprivate(zmscr16, zmscr17)
    ...
    SUBROUTINE kr_externs_in_stats_variables(kgen_unit) 
        ...
        CALL kr_stats_variables_integer___dim1(ilh_samp_frac_category, kgen_unit, "ilh_samp_frac_category", .FALSE.) 
        READ (UNIT = kgen_unit) ik_lh_start 
        READ (UNIT = kgen_unit) ilh_mixt_frac 
        READ (UNIT = kgen_unit) ilh_precip_frac 
        READ (UNIT = kgen_unit) ilh_precip_frac_unweighted 
        READ (UNIT = kgen_unit) ilh_mixt_frac_unweighted 
        CALL kr_stats_type_stats(stats_lh_zt, kgen_unit, "stats_lh_zt", .FALSE.) 
        CALL kr_stats_type_stats(stats_lh_sfc, kgen_unit, "stats_lh_sfc", .FALSE.) 
