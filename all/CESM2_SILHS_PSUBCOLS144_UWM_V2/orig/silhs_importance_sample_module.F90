...
module silhs_importance_sample_module
  ...
  PUBLIC importance_category_type, importance_sampling_driver, define_importance_categories, compute_category_real_probs, &
  &generate_strat_uniform_variate, pick_sample_categories, cloud_weighted_sampling_driver 
  ...
  PUBLIC kr_kgen_silhs_importance_sample_module_typesubp1 
  PUBLIC kv_kgen_silhs_importance_sample_module_typesubp1 
...
!-----------------------------------------------------------------------
  ...
  ! 
      ...
      USE parameters_silhs, ONLY: eight_cluster_allocation_opt, four_cluster_allocation_opt, two_cluster_cp_nocp_opt, &
      &l_lh_clustered_sampling, l_lh_limit_weights, cluster_allocation_strategy, l_lh_normalize_weights 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
      ...
      USE constants_clubb, ONLY: zero, unused_var 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ! Included Modules
      ...
      USE constants_clubb, ONLY: zero, fstderr 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ! Included Modules
      ...
      USE parameters_silhs, ONLY: eight_cluster_presc_probs 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ! Included Modules
      ...
      USE parameters_silhs, ONLY: l_lh_var_frac 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ! Included Modules
      ...
      USE constants_clubb, ONLY: zero, eps 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ! Included Modules
      ...
      USE parameters_silhs, ONLY: importance_prob_thresh 
...
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ! Included Modules
      ...
      USE constants_clubb, ONLY: one, two 
...
!-------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
      ...
      USE parameters_silhs, ONLY: l_lh_normalize_weights 
      USE pdf_parameter_module, ONLY: kr_pdf_parameter_module_pdf_parameter 
      USE hydromet_pdf_parameter_module, ONLY: kr_kgen_hydromet_pdf_parameter_module_typesubp0 
      USE pdf_parameter_module, ONLY: kv_pdf_parameter_module_pdf_parameter 
      USE hydromet_pdf_parameter_module, ONLY: kv_kgen_hydromet_pdf_parameter_module_typesubp0 
...
!-------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
      ...
      USE generate_uniform_sample_module, ONLY: choose_permuted_random 
    ...
    real( kind = core_rknd ), dimension(num_samples), intent(out) :: &
      lh_sample_point_weights             ! Weight of None sample points (these must be applied
...
!-------------------------------------------------------------------------------
  ...
  !-----------------------------------
      ...
      USE generate_uniform_sample_module, ONLY: rand_permute 
...
!----------------------------------------------------------------------
  ...
  !read state subroutine for kr_kgen_silhs_importance_sample_module_typesubp1 
  RECURSIVE SUBROUTINE kr_kgen_silhs_importance_sample_module_typesubp1(var, kgen_unit, printname, printvar) 
      TYPE(importance_category_type), INTENT(INOUT) :: var 
      ...
      READ (UNIT = kgen_unit) var%l_in_cloud 
      IF (PRESENT( printvar ) .AND. printvar) THEN 
          WRITE (*, *) "KGEN DEBUG: " // printname // "%l_in_cloud = ", var%l_in_cloud 
      END IF   
      READ (UNIT = kgen_unit) var%l_in_precip 
      IF (PRESENT( printvar ) .AND. printvar) THEN 
          WRITE (*, *) "KGEN DEBUG: " // printname // "%l_in_precip = ", var%l_in_precip 
      END IF   
      READ (UNIT = kgen_unit) var%l_in_component_1 
      IF (PRESENT( printvar ) .AND. printvar) THEN 
          WRITE (*, *) "KGEN DEBUG: " // printname // "%l_in_component_1 = ", var%l_in_component_1 
      END IF   
        
  END SUBROUTINE kr_kgen_silhs_importance_sample_module_typesubp1 
    
  !verify state subroutine for kv_kgen_silhs_importance_sample_module_typesubp1 
  RECURSIVE SUBROUTINE kv_kgen_silhs_importance_sample_module_typesubp1(varname, check_status, var, kgenref_var) 
      CHARACTER(LEN=*), INTENT(IN) :: varname 
      TYPE(check_t), INTENT(INOUT) :: check_status 
      TYPE(importance_category_type), INTENT(IN) :: var, kgenref_var 
      TYPE(check_t) :: dtype_check_status, comp_check_status 
      INTEGER :: check_result 
      LOGICAL :: is_print = .FALSE. 
        
      logical :: diff_l_in_cloud 
      logical :: diff_l_in_precip 
      logical :: diff_l_in_component_1 
      ...
      IF (var%l_in_cloud .EQV. kgenref_var%l_in_cloud) THEN 
          dtype_check_status%numIdentical = dtype_check_status%numIdentical + 1 
          IF (kgen_verboseLevel > 2) THEN 
              IF (check_status%rank == 0) THEN 
                  WRITE (*, *) trim(adjustl(varname)), "%l_in_cloud is IDENTICAL." 
              END IF   
          END IF   
      ...
      ELSE 
          ...
          IF (kgen_verboseLevel > 1) THEN 
              IF (check_status%rank == 0) THEN 
                  WRITE (*, *) trim(adjustl(varname)), "%l_in_cloud is NOT IDENTICAL." 
              END IF   
          END IF   
      ...
      END IF   
      dtype_check_status%numTotal = dtype_check_status%numTotal + 1 
      IF (var%l_in_precip .EQV. kgenref_var%l_in_precip) THEN 
          dtype_check_status%numIdentical = dtype_check_status%numIdentical + 1 
          IF (kgen_verboseLevel > 2) THEN 
              IF (check_status%rank == 0) THEN 
                  WRITE (*, *) trim(adjustl(varname)), "%l_in_precip is IDENTICAL." 
              END IF   
          END IF   
          check_result = CHECK_IDENTICAL 
      ELSE 
          ...
          IF (kgen_verboseLevel > 1) THEN 
              IF (check_status%rank == 0) THEN 
                  WRITE (*, *) trim(adjustl(varname)), "%l_in_precip is NOT IDENTICAL." 
              END IF   
          END IF   
          check_result = CHECK_OUT_TOL 
      END IF   
      ...
      END IF   
      dtype_check_status%numTotal = dtype_check_status%numTotal + 1 
      IF (var%l_in_component_1 .EQV. kgenref_var%l_in_component_1) THEN 
          dtype_check_status%numIdentical = dtype_check_status%numIdentical + 1 
          IF (kgen_verboseLevel > 2) THEN 
              IF (check_status%rank == 0) THEN 
                  WRITE (*, *) trim(adjustl(varname)), "%l_in_component_1 is IDENTICAL." 
              END IF   
          END IF   
          check_result = CHECK_IDENTICAL 
      ELSE 
          ...
          IF (kgen_verboseLevel > 1) THEN 
              IF (check_status%rank == 0) THEN 
                  WRITE (*, *) trim(adjustl(varname)), "%l_in_component_1 is NOT IDENTICAL." 
              END IF   
          END IF   
          check_result = CHECK_OUT_TOL 
      END IF   
      ...
      END IF   
  END SUBROUTINE kv_kgen_silhs_importance_sample_module_typesubp1 
