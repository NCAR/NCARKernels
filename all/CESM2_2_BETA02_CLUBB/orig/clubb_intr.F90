...
!Generated at : 2019-06-20 14:46:33 
!KGEN version : 0.8.1 
...
module clubb_intr
  ...
  !----------------------------------------------------------------------------------------------------- !
    ...
    USE physconst, ONLY: cpairv, cpair, gravit, latvap, latice 
...
!   
      ...
      USE clubb_api_module, ONLY: nparams, read_parameters_api, setup_parameters_api, advance_clubb_core_api, zt2zm_api, &
      &zm2zt_api, setup_grid_heights_api, w_tol_sqd, rt_tol, thl_tol, l_stats, stats_tsamp, stats_tout, pdf_parameter, &
      &num_pdf_params, pack_pdf_params_api, unpack_pdf_params_api, stats_begin_timestep_api, hydromet_dim, &
      &calculate_thlp2_rad_api, update_xp2_mc_api, fstderr 

      USE clubb_api_module, ONLY: clubb_fatal_error 
      ...
      USE time_manager, ONLY: kr_externs_out_time_manager 
      ...
      USE error_code, ONLY: kv_externs_error_code 
   ...
   ! --------------- !
      ...
      REAL(KIND=r8) :: wpthvp_in(pverp+1-top_lev) 
      REAL(KIND=r8) :: wp2thvp_in(pverp+1-top_lev) 
      REAL(KIND=r8) :: rtpthvp_in(pverp+1-top_lev) 
      REAL(KIND=r8) :: thlpthvp_in(pverp+1-top_lev) 
      ...
      REAL(KIND=r8) :: rcm_inout(pverp+1-top_lev) 
      REAL(KIND=r8) :: rcm_out_zm(pverp+1-top_lev) 
      REAL(KIND=r8) :: wprcp_out(pverp+1-top_lev) 
      REAL(KIND=r8) :: cloud_frac_inout(pverp+1-top_lev) 
      ...
      REAL(KIND=r8) :: ice_supersat_frac_out(pverp+1-top_lev) 
      ...
      REAL(KIND=r8) :: sclrpthvp_inout(pverp,sclr_dim) 
   ...
   ! input to CLUBB using l_input_fields 
      ...
      REAL(KIND=r8) :: thlp3_dummy(pverp) 
      REAL(KIND=r8) :: thlp3_dummy_in(pverp) 
      REAL(KIND=r8) :: rtp3_dummy(pverp) 
      REAL(KIND=r8) :: rtp3_dummy_in(pverp) 
   ! Variables below are needed to compute energy integrals for conservation
      ...
      REAL(KIND=r8) :: pdfp_rtp2(pcols, pverp) 
      REAL(KIND=r8) :: rtp2_zt(pverp+1-top_lev) 
      REAL(KIND=r8) :: rtp2_zt_out(pcols, pverp) 
      REAL(KIND=r8) :: thl2_zt(pverp+1-top_lev) 
      REAL(KIND=r8) :: thl2_zt_out(pcols, pverp) 
      REAL(KIND=r8) :: wp2_zt(pverp+1-top_lev) 
      REAL(KIND=r8) :: wp2_zt_out(pcols, pverp) 
      REAL(KIND=r8) :: wm_zt_out(pcols, pverp) 
      REAL(KIND=r8) :: mean_rt 
      REAL(KIND=r8) :: dum1 
      ...
      TYPE(pdf_parameter), dimension(pverp) :: pdf_params_zm 
      REAL(KIND=r8), dimension(pverp,num_pdf_params) :: pdf_params_packed 
      REAL(KIND=r8), dimension(pverp,num_pdf_params) :: pdf_params_zm_packed 
      INTEGER :: stats_nsamp, stats_nout 

      REAL(KIND=r8) :: rtm_integral_vtend, rtm_integral_ltend 
    ! --------------- !
    ! Pointers        !
    ! --------------- !
      ...
      REAL(KIND=r8), pointer, dimension(:,:) :: wpthvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: wp2thvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: rtpthvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: thlpthvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: cloud_frac 
      REAL(KIND=r8), pointer, dimension(:,:) :: thlm 
      REAL(KIND=r8), pointer, dimension(:,:) :: rtm 
      REAL(KIND=r8), pointer, dimension(:,:) :: rcm 
      REAL(KIND=r8), pointer, dimension(:,:) :: um 
      REAL(KIND=r8), pointer, dimension(:,:) :: vm 
      REAL(KIND=r8), pointer, dimension(:,:) :: khzm 
      REAL(KIND=r8), pointer, dimension(:,:) :: ice_supersat_frac 
      REAL(KIND=r8), pointer, dimension(:,:,:) :: pdf_params_ptr 
      REAL(KIND=r8), pointer, dimension(:,:,:) :: pdf_params_zm_ptr 
   ...
   ! SILHS covariance contributions
      ...
      REAL(KIND=r8), pointer, dimension(:,:) :: rtp2_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: thlp2_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: wprtp_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: wpthlp_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: rtpthlp_mc_zt 
  ...
  ! ZM microphysics
      ...
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_wpthvp_in 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_wp2thvp_in 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_rtpthvp_in 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_thlpthvp_in 
      ...
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_rcm_inout 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_rcm_out_zm 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_wprcp_out 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_cloud_frac_inout 
      ...
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_ice_supersat_frac_out 
      ...
      REAL(KIND=r8), dimension(pverp,sclr_dim) :: kgenref_sclrpthvp_inout 
      ...
      REAL(KIND=r8), dimension(pverp) :: kgenref_thlp3_dummy_in 
      REAL(KIND=r8), dimension(pverp) :: kgenref_rtp3_dummy_in 
      ...
      REAL(KIND=r8), dimension(pcols,pverp) :: kgenref_pdfp_rtp2 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_rtp2_zt 
      REAL(KIND=r8), dimension(pcols,pverp) :: kgenref_rtp2_zt_out 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_thl2_zt 
      REAL(KIND=r8), dimension(pcols,pverp) :: kgenref_thl2_zt_out 
      REAL(KIND=r8), dimension(pverp+1-top_lev) :: kgenref_wp2_zt 
      REAL(KIND=r8), dimension(pcols,pverp) :: kgenref_wp2_zt_out 
      REAL(KIND=r8), dimension(pcols,pverp) :: kgenref_wm_zt_out 
      REAL(KIND=r8) :: kgenref_mean_rt 
      ...
      TYPE(pdf_parameter), dimension(pverp) :: kgenref_pdf_params_zm 
      REAL(KIND=r8), dimension(pverp,num_pdf_params) :: kgenref_pdf_params_packed 
      REAL(KIND=r8), dimension(pverp,num_pdf_params) :: kgenref_pdf_params_zm_packed 
      INTEGER :: kgenref_stats_nsamp 
      INTEGER :: kgenref_stats_nout 
      REAL(KIND=r8) :: kgenref_rtm_integral_vtend 
      REAL(KIND=r8) :: kgenref_rtm_integral_ltend 
      ...
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_wpthvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_wp2thvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_rtpthvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_thlpthvp 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_cloud_frac 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_thlm 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_rtm 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_rcm 
      ...
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_ice_supersat_frac 
      REAL(KIND=r8), pointer, dimension(:,:,:) :: kgenref_pdf_params_ptr 
      REAL(KIND=r8), pointer, dimension(:,:,:) :: kgenref_pdf_params_zm_ptr 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_rtp2_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_thlp2_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_wprtp_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_wpthlp_mc_zt 
      REAL(KIND=r8), pointer, dimension(:,:) :: kgenref_rtpthlp_mc_zt 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) wpthvp_in 
          CALL kgen_array_sumcheck("wpthvp_in", kgen_array_sum, DBLE(SUM(wpthvp_in, mask=(wpthvp_in .eq. wpthvp_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) wp2thvp_in 
          CALL kgen_array_sumcheck("wp2thvp_in", kgen_array_sum, DBLE(SUM(wp2thvp_in, mask=(wp2thvp_in .eq. wp2thvp_in))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rtpthvp_in 
          CALL kgen_array_sumcheck("rtpthvp_in", kgen_array_sum, DBLE(SUM(rtpthvp_in, mask=(rtpthvp_in .eq. rtpthvp_in))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) thlpthvp_in 
          CALL kgen_array_sumcheck("thlpthvp_in", kgen_array_sum, DBLE(SUM(thlpthvp_in, mask=(thlpthvp_in .eq. thlpthvp_in))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rcm_inout 
          CALL kgen_array_sumcheck("rcm_inout", kgen_array_sum, DBLE(SUM(rcm_inout, mask=(rcm_inout .eq. rcm_inout))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) cloud_frac_inout 
          CALL kgen_array_sumcheck("cloud_frac_inout", kgen_array_sum, DBLE(SUM(cloud_frac_inout, mask=(cloud_frac_inout .eq. &
          &cloud_frac_inout))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) ice_supersat_frac_out 
          CALL kgen_array_sumcheck("ice_supersat_frac_out", kgen_array_sum, DBLE(SUM(ice_supersat_frac_out, &
          &mask=(ice_supersat_frac_out .eq. ice_supersat_frac_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          ...
          CALL kgen_array_sumcheck("hydromet", kgen_array_sum, DBLE(SUM(hydromet, mask=(hydromet .eq. hydromet))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) wphydrometp 
          CALL kgen_array_sumcheck("wphydrometp", kgen_array_sum, DBLE(SUM(wphydrometp, mask=(wphydrometp .eq. wphydrometp))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          ...
          CALL kgen_array_sumcheck("wp2hmp", kgen_array_sum, DBLE(SUM(wp2hmp, mask=(wp2hmp .eq. wp2hmp))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rtphmp_zt 
          CALL kgen_array_sumcheck("rtphmp_zt", kgen_array_sum, DBLE(SUM(rtphmp_zt, mask=(rtphmp_zt .eq. rtphmp_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) thlphmp_zt 
          CALL kgen_array_sumcheck("thlphmp_zt", kgen_array_sum, DBLE(SUM(thlphmp_zt, mask=(thlphmp_zt .eq. thlphmp_zt))), &
          &.TRUE.) 
      END IF   
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) thlp3_dummy 
          CALL kgen_array_sumcheck("thlp3_dummy", kgen_array_sum, DBLE(SUM(thlp3_dummy, mask=(thlp3_dummy .eq. thlp3_dummy))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) thlp3_dummy_in 
          CALL kgen_array_sumcheck("thlp3_dummy_in", kgen_array_sum, DBLE(SUM(thlp3_dummy_in, mask=(thlp3_dummy_in .eq. &
          &thlp3_dummy_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rtp3_dummy 
          CALL kgen_array_sumcheck("rtp3_dummy", kgen_array_sum, DBLE(SUM(rtp3_dummy, mask=(rtp3_dummy .eq. rtp3_dummy))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rtp3_dummy_in 
          CALL kgen_array_sumcheck("rtp3_dummy_in", kgen_array_sum, DBLE(SUM(rtp3_dummy_in, mask=(rtp3_dummy_in .eq. &
          &rtp3_dummy_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
      ...
      IF (kgen_istrue) THEN 
          ...
          CALL kgen_array_sumcheck("cloud_cover", kgen_array_sum, DBLE(SUM(cloud_cover, mask=(cloud_cover .eq. cloud_cover))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) pdfp_rtp2 
          CALL kgen_array_sumcheck("pdfp_rtp2", kgen_array_sum, DBLE(SUM(pdfp_rtp2, mask=(pdfp_rtp2 .eq. pdfp_rtp2))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rtp2_zt 
          CALL kgen_array_sumcheck("rtp2_zt", kgen_array_sum, DBLE(SUM(rtp2_zt, mask=(rtp2_zt .eq. rtp2_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) rtp2_zt_out 
          CALL kgen_array_sumcheck("rtp2_zt_out", kgen_array_sum, DBLE(SUM(rtp2_zt_out, mask=(rtp2_zt_out .eq. rtp2_zt_out))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) thl2_zt 
          CALL kgen_array_sumcheck("thl2_zt", kgen_array_sum, DBLE(SUM(thl2_zt, mask=(thl2_zt .eq. thl2_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) thl2_zt_out 
          CALL kgen_array_sumcheck("thl2_zt_out", kgen_array_sum, DBLE(SUM(thl2_zt_out, mask=(thl2_zt_out .eq. thl2_zt_out))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) wp2_zt 
          CALL kgen_array_sumcheck("wp2_zt", kgen_array_sum, DBLE(SUM(wp2_zt, mask=(wp2_zt .eq. wp2_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) wp2_zt_out 
          CALL kgen_array_sumcheck("wp2_zt_out", kgen_array_sum, DBLE(SUM(wp2_zt_out, mask=(wp2_zt_out .eq. wp2_zt_out))), &
          &.TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) wm_zt_out 
          CALL kgen_array_sumcheck("wm_zt_out", kgen_array_sum, DBLE(SUM(wm_zt_out, mask=(wm_zt_out .eq. wm_zt_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) mean_rt 
      ...
      CALL kr_kgen_clubb_tend_cam_subp2(pdf_params_zm, kgen_unit, "pdf_params_zm", .FALSE.) 
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) pdf_params_packed 
          CALL kgen_array_sumcheck("pdf_params_packed", kgen_array_sum, DBLE(SUM(pdf_params_packed, mask=(pdf_params_packed .eq. &
          &pdf_params_packed))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) pdf_params_zm_packed 
          CALL kgen_array_sumcheck("pdf_params_zm_packed", kgen_array_sum, DBLE(SUM(pdf_params_zm_packed, &
          &mask=(pdf_params_zm_packed .eq. pdf_params_zm_packed))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) stats_nsamp 
      READ (UNIT = kgen_unit) stats_nout 
      READ (UNIT = kgen_unit) rtm_integral_ltend 
      READ (UNIT = kgen_unit) rtm_integral_vtend 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(wpthvp, kgen_unit, "wpthvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(wp2thvp, kgen_unit, "wp2thvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(rtpthvp, kgen_unit, "rtpthvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(thlpthvp, kgen_unit, "thlpthvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(cloud_frac, kgen_unit, "cloud_frac", .FALSE.) 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(rcm, kgen_unit, "rcm", .FALSE.) 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(ice_supersat_frac, kgen_unit, "ice_supersat_frac", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim3_ptr(pdf_params_ptr, kgen_unit, "pdf_params_ptr", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim3_ptr(pdf_params_zm_ptr, kgen_unit, "pdf_params_zm_ptr", .FALSE.) 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(rtp2_mc_zt, kgen_unit, "rtp2_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(thlp2_mc_zt, kgen_unit, "thlp2_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(wprtp_mc_zt, kgen_unit, "wprtp_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(wpthlp_mc_zt, kgen_unit, "wpthlp_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(rtpthlp_mc_zt, kgen_unit, "rtpthlp_mc_zt", .FALSE.) 
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      CALL kr_externs_out_time_manager(kgen_unit) 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wpthvp_in 
          CALL kgen_array_sumcheck("kgenref_wpthvp_in", kgen_array_sum, DBLE(SUM(kgenref_wpthvp_in, mask=(kgenref_wpthvp_in .eq. &
          &kgenref_wpthvp_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wp2thvp_in 
          CALL kgen_array_sumcheck("kgenref_wp2thvp_in", kgen_array_sum, DBLE(SUM(kgenref_wp2thvp_in, mask=(kgenref_wp2thvp_in &
          &.eq. kgenref_wp2thvp_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_rtpthvp_in 
          CALL kgen_array_sumcheck("kgenref_rtpthvp_in", kgen_array_sum, DBLE(SUM(kgenref_rtpthvp_in, mask=(kgenref_rtpthvp_in &
          &.eq. kgenref_rtpthvp_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_thlpthvp_in 
          CALL kgen_array_sumcheck("kgenref_thlpthvp_in", kgen_array_sum, DBLE(SUM(kgenref_thlpthvp_in, mask=(kgenref_thlpthvp_in &
          &.eq. kgenref_thlpthvp_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_rcm_inout 
          CALL kgen_array_sumcheck("kgenref_rcm_inout", kgen_array_sum, DBLE(SUM(kgenref_rcm_inout, mask=(kgenref_rcm_inout .eq. &
          &kgenref_rcm_inout))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_cloud_frac_inout 
          CALL kgen_array_sumcheck("kgenref_cloud_frac_inout", kgen_array_sum, DBLE(SUM(kgenref_cloud_frac_inout, &
          &mask=(kgenref_cloud_frac_inout .eq. kgenref_cloud_frac_inout))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_ice_supersat_frac_out 
          CALL kgen_array_sumcheck("kgenref_ice_supersat_frac_out", kgen_array_sum, DBLE(SUM(kgenref_ice_supersat_frac_out, &
          &mask=(kgenref_ice_supersat_frac_out .eq. kgenref_ice_supersat_frac_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_hydromet 
          CALL kgen_array_sumcheck("kgenref_hydromet", kgen_array_sum, DBLE(SUM(kgenref_hydromet, mask=(kgenref_hydromet .eq. &
          &kgenref_hydromet))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wphydrometp 
          CALL kgen_array_sumcheck("kgenref_wphydrometp", kgen_array_sum, DBLE(SUM(kgenref_wphydrometp, mask=(kgenref_wphydrometp &
          &.eq. kgenref_wphydrometp))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wp2hmp 
          CALL kgen_array_sumcheck("kgenref_wp2hmp", kgen_array_sum, DBLE(SUM(kgenref_wp2hmp, mask=(kgenref_wp2hmp .eq. &
          &kgenref_wp2hmp))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_rtphmp_zt 
          CALL kgen_array_sumcheck("kgenref_rtphmp_zt", kgen_array_sum, DBLE(SUM(kgenref_rtphmp_zt, mask=(kgenref_rtphmp_zt .eq. &
          &kgenref_rtphmp_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_thlphmp_zt 
          CALL kgen_array_sumcheck("kgenref_thlphmp_zt", kgen_array_sum, DBLE(SUM(kgenref_thlphmp_zt, mask=(kgenref_thlphmp_zt &
          &.eq. kgenref_thlphmp_zt))), .TRUE.) 
      END IF   
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_thlp3_dummy_in 
          CALL kgen_array_sumcheck("kgenref_thlp3_dummy_in", kgen_array_sum, DBLE(SUM(kgenref_thlp3_dummy_in, &
          &mask=(kgenref_thlp3_dummy_in .eq. kgenref_thlp3_dummy_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_rtp3_dummy_in 
          CALL kgen_array_sumcheck("kgenref_rtp3_dummy_in", kgen_array_sum, DBLE(SUM(kgenref_rtp3_dummy_in, &
          &mask=(kgenref_rtp3_dummy_in .eq. kgenref_rtp3_dummy_in))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          ...
          CALL kgen_array_sumcheck("kgenref_cloud_cover", kgen_array_sum, DBLE(SUM(kgenref_cloud_cover, mask=(kgenref_cloud_cover &
          &.eq. kgenref_cloud_cover))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
      ...
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_pdfp_rtp2 
          CALL kgen_array_sumcheck("kgenref_pdfp_rtp2", kgen_array_sum, DBLE(SUM(kgenref_pdfp_rtp2, mask=(kgenref_pdfp_rtp2 .eq. &
          &kgenref_pdfp_rtp2))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_rtp2_zt 
          CALL kgen_array_sumcheck("kgenref_rtp2_zt", kgen_array_sum, DBLE(SUM(kgenref_rtp2_zt, mask=(kgenref_rtp2_zt .eq. &
          &kgenref_rtp2_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_rtp2_zt_out 
          CALL kgen_array_sumcheck("kgenref_rtp2_zt_out", kgen_array_sum, DBLE(SUM(kgenref_rtp2_zt_out, mask=(kgenref_rtp2_zt_out &
          &.eq. kgenref_rtp2_zt_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_thl2_zt 
          CALL kgen_array_sumcheck("kgenref_thl2_zt", kgen_array_sum, DBLE(SUM(kgenref_thl2_zt, mask=(kgenref_thl2_zt .eq. &
          &kgenref_thl2_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_thl2_zt_out 
          CALL kgen_array_sumcheck("kgenref_thl2_zt_out", kgen_array_sum, DBLE(SUM(kgenref_thl2_zt_out, mask=(kgenref_thl2_zt_out &
          &.eq. kgenref_thl2_zt_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wp2_zt 
          CALL kgen_array_sumcheck("kgenref_wp2_zt", kgen_array_sum, DBLE(SUM(kgenref_wp2_zt, mask=(kgenref_wp2_zt .eq. &
          &kgenref_wp2_zt))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wp2_zt_out 
          CALL kgen_array_sumcheck("kgenref_wp2_zt_out", kgen_array_sum, DBLE(SUM(kgenref_wp2_zt_out, mask=(kgenref_wp2_zt_out &
          &.eq. kgenref_wp2_zt_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_wm_zt_out 
          CALL kgen_array_sumcheck("kgenref_wm_zt_out", kgen_array_sum, DBLE(SUM(kgenref_wm_zt_out, mask=(kgenref_wm_zt_out .eq. &
          &kgenref_wm_zt_out))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgenref_mean_rt 
      ...
      CALL kr_kgen_clubb_tend_cam_subp2(kgenref_pdf_params_zm, kgen_unit, "kgenref_pdf_params_zm", .FALSE.) 
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_pdf_params_packed 
          CALL kgen_array_sumcheck("kgenref_pdf_params_packed", kgen_array_sum, DBLE(SUM(kgenref_pdf_params_packed, &
          &mask=(kgenref_pdf_params_packed .eq. kgenref_pdf_params_packed))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgen_istrue 
      IF (kgen_istrue) THEN 
          READ (UNIT = kgen_unit) kgen_array_sum 
          READ (UNIT = kgen_unit) kgenref_pdf_params_zm_packed 
          CALL kgen_array_sumcheck("kgenref_pdf_params_zm_packed", kgen_array_sum, DBLE(SUM(kgenref_pdf_params_zm_packed, &
          &mask=(kgenref_pdf_params_zm_packed .eq. kgenref_pdf_params_zm_packed))), .TRUE.) 
      END IF   
      READ (UNIT = kgen_unit) kgenref_stats_nsamp 
      READ (UNIT = kgen_unit) kgenref_stats_nout 
      READ (UNIT = kgen_unit) kgenref_rtm_integral_vtend 
      READ (UNIT = kgen_unit) kgenref_rtm_integral_ltend 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_wpthvp, kgen_unit, "kgenref_wpthvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_wp2thvp, kgen_unit, "kgenref_wp2thvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_rtpthvp, kgen_unit, "kgenref_rtpthvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_thlpthvp, kgen_unit, "kgenref_thlpthvp", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_cloud_frac, kgen_unit, "kgenref_cloud_frac", .FALSE.) 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_rcm, kgen_unit, "kgenref_rcm", .FALSE.) 
      ...
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_ice_supersat_frac, kgen_unit, "kgenref_ice_supersat_frac", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim3_ptr(kgenref_pdf_params_ptr, kgen_unit, "kgenref_pdf_params_ptr", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim3_ptr(kgenref_pdf_params_zm_ptr, kgen_unit, "kgenref_pdf_params_zm_ptr", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_rtp2_mc_zt, kgen_unit, "kgenref_rtp2_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_thlp2_mc_zt, kgen_unit, "kgenref_thlp2_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_wprtp_mc_zt, kgen_unit, "kgenref_wprtp_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_wpthlp_mc_zt, kgen_unit, "kgenref_wpthlp_mc_zt", .FALSE.) 
      CALL kr_clubb_tend_cam_real__r8_dim2_ptr(kgenref_rtpthlp_mc_zt, kgen_unit, "kgenref_rtpthlp_mc_zt", .FALSE.) 
...
!$kgen begin_callsite clubb
   ...
   IF (kgen_mainstage) THEN 
       ...
       CALL kv_externs_error_code(check_status) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim1("wpthvp_in", check_status, wpthvp_in, kgenref_wpthvp_in) 
       CALL kv_clubb_tend_cam_real__r8_dim1("wp2thvp_in", check_status, wp2thvp_in, kgenref_wp2thvp_in) 
       CALL kv_clubb_tend_cam_real__r8_dim1("rtpthvp_in", check_status, rtpthvp_in, kgenref_rtpthvp_in) 
       CALL kv_clubb_tend_cam_real__r8_dim1("thlpthvp_in", check_status, thlpthvp_in, kgenref_thlpthvp_in) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim1("rcm_inout", check_status, rcm_inout, kgenref_rcm_inout) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim1("cloud_frac_inout", check_status, cloud_frac_inout, kgenref_cloud_frac_inout) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim1("ice_supersat_frac_out", check_status, ice_supersat_frac_out, &
       &kgenref_ice_supersat_frac_out) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim2("hydromet", check_status, hydromet, kgenref_hydromet) 
       CALL kv_clubb_tend_cam_real__r8_dim2("wphydrometp", check_status, wphydrometp, kgenref_wphydrometp) 
       CALL kv_clubb_tend_cam_real__r8_dim2("wp2hmp", check_status, wp2hmp, kgenref_wp2hmp) 
       CALL kv_clubb_tend_cam_real__r8_dim2("rtphmp_zt", check_status, rtphmp_zt, kgenref_rtphmp_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2("thlphmp_zt", check_status, thlphmp_zt, kgenref_thlphmp_zt) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim1("thlp3_dummy_in", check_status, thlp3_dummy_in, kgenref_thlp3_dummy_in) 
       CALL kv_clubb_tend_cam_real__r8_dim1("rtp3_dummy_in", check_status, rtp3_dummy_in, kgenref_rtp3_dummy_in) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim2("pdfp_rtp2", check_status, pdfp_rtp2, kgenref_pdfp_rtp2) 
       CALL kv_clubb_tend_cam_real__r8_dim1("rtp2_zt", check_status, rtp2_zt, kgenref_rtp2_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2("rtp2_zt_out", check_status, rtp2_zt_out, kgenref_rtp2_zt_out) 
       CALL kv_clubb_tend_cam_real__r8_dim1("thl2_zt", check_status, thl2_zt, kgenref_thl2_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2("thl2_zt_out", check_status, thl2_zt_out, kgenref_thl2_zt_out) 
       CALL kv_clubb_tend_cam_real__r8_dim1("wp2_zt", check_status, wp2_zt, kgenref_wp2_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2("wp2_zt_out", check_status, wp2_zt_out, kgenref_wp2_zt_out) 
       CALL kv_clubb_tend_cam_real__r8_dim2("wm_zt_out", check_status, wm_zt_out, kgenref_wm_zt_out) 
       CALL kv_clubb_tend_cam_real__r8("mean_rt", check_status, mean_rt, kgenref_mean_rt) 
       ...
       CALL kv_kgen_clubb_tend_cam_subp1("pdf_params_zm", check_status, pdf_params_zm, kgenref_pdf_params_zm) 
       CALL kv_clubb_tend_cam_real__r8_dim2("pdf_params_packed", check_status, pdf_params_packed, kgenref_pdf_params_packed) 
       CALL kv_clubb_tend_cam_real__r8_dim2("pdf_params_zm_packed", check_status, pdf_params_zm_packed, &
       &kgenref_pdf_params_zm_packed) 
       CALL kv_clubb_tend_cam_integer__("stats_nout", check_status, stats_nout, kgenref_stats_nout) 
       CALL kv_clubb_tend_cam_integer__("stats_nsamp", check_status, stats_nsamp, kgenref_stats_nsamp) 
       CALL kv_clubb_tend_cam_real__r8("rtm_integral_vtend", check_status, rtm_integral_vtend, kgenref_rtm_integral_vtend) 
       CALL kv_clubb_tend_cam_real__r8("rtm_integral_ltend", check_status, rtm_integral_ltend, kgenref_rtm_integral_ltend) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("wpthvp", check_status, wpthvp, kgenref_wpthvp) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("wp2thvp", check_status, wp2thvp, kgenref_wp2thvp) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("rtpthvp", check_status, rtpthvp, kgenref_rtpthvp) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("thlpthvp", check_status, thlpthvp, kgenref_thlpthvp) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("cloud_frac", check_status, cloud_frac, kgenref_cloud_frac) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("rcm", check_status, rcm, kgenref_rcm) 
       ...
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("ice_supersat_frac", check_status, ice_supersat_frac, kgenref_ice_supersat_frac) 
       CALL kv_clubb_tend_cam_real__r8_dim3_ptr("pdf_params_ptr", check_status, pdf_params_ptr, kgenref_pdf_params_ptr) 
       CALL kv_clubb_tend_cam_real__r8_dim3_ptr("pdf_params_zm_ptr", check_status, pdf_params_zm_ptr, kgenref_pdf_params_zm_ptr) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("rtp2_mc_zt", check_status, rtp2_mc_zt, kgenref_rtp2_mc_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("thlp2_mc_zt", check_status, thlp2_mc_zt, kgenref_thlp2_mc_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("wprtp_mc_zt", check_status, wprtp_mc_zt, kgenref_wprtp_mc_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("wpthlp_mc_zt", check_status, wpthlp_mc_zt, kgenref_wpthlp_mc_zt) 
       CALL kv_clubb_tend_cam_real__r8_dim2_ptr("rtpthlp_mc_zt", check_status, rtpthlp_mc_zt, kgenref_rtpthlp_mc_zt) 
...
#endif 
       IF (check_status%rank==0) THEN 
           WRITE (*, *) "clubb : Time per call (usec): ", kgen_measure 
       END IF   
   END IF   
   IF (kgen_warmupstage) THEN 
   END IF   
...
!$kgen end_callsite clubb
   ...
   !read state subroutine for kr_clubb_tend_cam_real__r8_dim3_ptr 
   SUBROUTINE kr_clubb_tend_cam_real__r8_dim3_ptr(var, kgen_unit, printname, printvar) 
       REAL(KIND=r8), INTENT(INOUT), POINTER, DIMENSION(:,:,:) :: var 
       INTEGER, INTENT(IN) :: kgen_unit 
       CHARACTER(LEN=*), INTENT(IN) :: printname 
       LOGICAL, INTENT(IN), OPTIONAL :: printvar 
       ...
       IF (kgen_istrue) THEN 
           ...
           END IF   
       END IF   
   END SUBROUTINE kr_clubb_tend_cam_real__r8_dim3_ptr 
   ...
   !verify state subroutine for kv_clubb_tend_cam_real__r8_dim3_ptr 
   RECURSIVE SUBROUTINE kv_clubb_tend_cam_real__r8_dim3_ptr(varname, check_status, var, kgenref_var) 
       CHARACTER(LEN=*), INTENT(IN) :: varname 
       TYPE(check_t), INTENT(INOUT) :: check_status 
       REAL(KIND=r8), pointer, INTENT(IN), DIMENSION(:,:,:) :: var, kgenref_var 
       INTEGER :: check_result 
       ...
       END IF   
   END SUBROUTINE kv_clubb_tend_cam_real__r8_dim3_ptr 
...
return
  ...
  subroutine stats_end_timestep_clubb(thecol,out_zt,out_zm,out_radzt,out_radzm,out_sfc)
    ...
    !-----------------------------------------------------------------------
      ...
      USE clubb_api_module, ONLY: stats_zt, stats_zm, stats_rad_zt, stats_rad_zm, stats_sfc, l_stats_last, l_output_rad_files 
