...
!
    ...
    PUBLIC advance_clubb_core, set_lscale_max, calculate_thlp2_rad 
  ...
  subroutine advance_clubb_core &
    ...
    !
      ...
      USE constants_clubb, ONLY: em_min, thl_tol, rt_tol, w_tol, w_tol_sqd, ep2, cp, lv, ep1, fstderr, zero_threshold, &
      &three_halves, one, unused_var, grav, eps 

      USE parameters_tunable, ONLY: taumax, c_k, mu, lscale_mu_coef, lscale_pert_coef, gamma_coef, gamma_coefb, gamma_coefc, &
      &c_k10, c_k10h, c1, c14, c5, c4, c_wp2_splat 

      USE parameters_model, ONLY: sclr_dim, edsclr_dim, t0, sclr_tol 

      USE model_flags, ONLY: l_tke_aniso, l_call_pdf_closure_twice, l_host_applies_sfc_fluxes, l_stability_correct_tau_zm, &
      &l_do_expldiff_rtm_thlm, l_lscale_plume_centered, l_use_ice_latent, l_gamma_skw, l_damp_wp2_using_em, l_advance_xp3, &
      &l_predict_upwp_vpwp, l_diag_lscale_from_tau 

      USE grid_class, ONLY: gr, zm2zt, zt2zm, ddzt 

      USE numerical_check, ONLY: parameterization_check, calculate_spurious_source 

      USE variables_diagnostic_module, ONLY: skw_zt, skw_zm, wp4, rtprcp, thlprcp, rcp2, rsat, wprtp2, wp2rtp, wpthlp2, wp2thlp, &
      &wprtpthlp, wp2rcp 

      USE variables_diagnostic_module, ONLY: thvm, em, lscale, lscale_up, lscale_down, tau_zm, tau_zt, kh_zm, kh_zt, vg, ug, &
      &um_ref, vm_ref 

      USE variables_diagnostic_module, ONLY: wp2_zt, thlp2_zt, wpthlp_zt, wprtp_zt, rtp2_zt, rtpthlp_zt, up2_zt, vp2_zt, upwp_zt, &
      &vpwp_zt, rtm_ref, thlm_ref 

      USE variables_diagnostic_module, ONLY: wpedsclrp, sclrprcp, wp2sclrp, wpsclrp2, wpsclrprtp, wpsclrpthlp, wp3_zm, &
      &skw_velocity, a3_coef, a3_coef_zt 

      USE variables_diagnostic_module, ONLY: wp3_on_wp2, wp3_on_wp2_zt 
      ...
      USE skx_module, ONLY: skx_func, xp3_lg_2005_ansatz 
      ...
      USE stats_type_utilities, ONLY: stat_update_var_pt, stat_update_var, stat_begin_update, stat_begin_update_pt, &
      &stat_end_update, stat_end_update_pt 

      USE stats_variables, ONLY: irtp2_bt, ithlp2_bt, irtpthlp_bt, iwp2_bt, iwp3_bt, ivp2_bt, iup2_bt, iwprtp_bt, iwpthlp_bt, &
      &iupwp_bt, ivpwp_bt, irtm_bt, ithlm_bt, ivm_bt, ium_bt, irvm, irel_humidity, iwpthlp_zt 

      USE stats_variables, ONLY: iwprtp_zt, iup2_zt, ivp2_zt, iupwp_zt, ivpwp_zt, ithlp2_sf, irtp2_sf, irtpthlp_sf, iup2_sf, &
      &ivp2_sf, iwp2_sf, l_stats_samp, l_stats, stats_zt, stats_zm, stats_sfc, irtm_spur_src, ithlm_spur_src 

      USE stats_variables, ONLY: irfrzm, istability_correction 

      USE stats_variables, ONLY: ilscale_pert_1, ilscale_pert_2 

      USE fill_holes, ONLY: vertical_integral, fill_holes_vertical 

      USE advance_helper_module, ONLY: calc_stability_correction, compute_cx_fnc_richardson, calc_brunt_vaisala_freq_sqd, &
...
! End Vince Larson's commenting.
  ...
  subroutine pdf_closure_driver( dt, hydromet_dim, wprtp,       & ! Intent(in)
      ...
      USE grid_class, ONLY: gr, zt2zm, zm2zt 

      USE constants_clubb, ONLY: w_tol, w_tol_sqd, rt_tol, thl_tol, cp, lv, ls, p0, kappa, fstderr, zero, zero_threshold, eps 

      USE pdf_parameter_module, ONLY: pdf_parameter, implicit_coefs_terms, init_pdf_params 

      USE parameters_model, ONLY: sclr_dim, sclr_tol, ts_nudge, rtm_min, rtm_nudge_max_altitude 

      USE parameters_tunable, ONLY: gamma_coef, gamma_coefb, gamma_coefc 

      USE pdf_closure_module, ONLY: pdf_closure, calc_vert_avg_cf_component 
      ...
      USE array_index, ONLY: iirr 

      USE model_flags, ONLY: l_gamma_skw, l_call_pdf_closure_twice, l_trapezoidal_rule_zm, l_trapezoidal_rule_zt, &
      &l_use_cloud_cover, l_use_ice_latent, l_rcm_supersat_adj, l_rtm_nudge, l_explicit_turbulent_adv_wp3 

      USE error_code, ONLY: clubb_at_least_debug_level, err_code, clubb_fatal_error 

      USE variables_diagnostic_module, ONLY: sigma_sqd_w_zt, rtm_ref 

      USE stats_type_utilities, ONLY: stat_update_var, stat_update_var_pt 

      USE stats_variables, ONLY: l_stats_samp, stats_zt, stats_zm, iskw_zm, iskw_zt, iskrt_zm, iskrt_zt, iskthl_zm, iskthl_zt, &
      &iskw_velocity, igamma_skw_fnc, if_w, if_rt, if_thl, imin_f_w, imax_f_w, imin_f_rt, imax_f_rt, imin_f_thl, imax_f_thl, &
      &iwp4, ircm_refined, icloud_frac_refined 
  ...
  !=============================================================================
    ...
    subroutine trapezoidal_rule_zt &
      ...
      !
        ...
        USE stats_variables, ONLY: iwprtp2, iwprtpthlp, iwpthlp2, iwprtp2, iwpsclrp2, iwpsclrprtp, iwpsclrpthlp, l_stats 

        USE grid_class, ONLY: gr, zt2zm 
    ...
    subroutine compute_cloud_cover &
      ...
      !
        ...
        USE constants_clubb, ONLY: rc_tol, fstderr 
...
!===============================================================================
  ...
  !----------------------------------------------------------------------
      ...
      USE constants_clubb, ONLY: two, rc_tol 

      USE parameters_tunable, ONLY: thlp2_rad_coef 
