...
module pdf_closure_module
    ...
    PUBLIC pdf_closure, calc_wp4_pdf, calc_wp2xp_pdf, calc_wpxp2_pdf, calc_wpxpyp_pdf, calc_vert_avg_cf_component 
...
!------------------------------------------------------------------------
  ...
  subroutine pdf_closure( hydromet_dim, p_in_Pa, exner, thv_ds, wm, &
    ...
    !
      ...
      USE constants_clubb, ONLY: three, two, one, one_half, zero, cp, lv, rd, ep, ep1, ep2, rt_tol, thl_tol, t_freeze_k, fstderr, &
      &zero_threshold, chi_tol, eta_tol, max_mag_correlation, eps, w_tol 

      USE parameters_model, ONLY: mixt_frac_max_mag, sclr_dim 
      ...
      USE adg1_adg2_3d_luhar_pdf, ONLY: adg1_pdf_driver, adg2_pdf_driver, luhar_3d_pdf_driver 

      USE new_tsdadg_pdf, ONLY: tsdadg_pdf_driver 

      USE ly93_pdf, ONLY: ly93_driver 

      USE pdf_utilities, ONLY: calc_comp_corrs_binormal, calc_corr_chi_x, calc_corr_eta_x 

      USE array_index, ONLY: l_mix_rat_hm 

      USE model_flags, ONLY: l_explicit_turbulent_adv_wp3, l_explicit_turbulent_adv_xpyp 
...
!------------------------ Code Begins ----------------------------------
  ...
  result( wp4 )
    ...
    !
      ...
      USE constants_clubb, ONLY: six, three, one 
  ...
  subroutine calc_cloud_frac_component( mean_chi_i, stdev_chi_i, &
    ...
    !
      ...
      USE constants_clubb, ONLY: chi_tol, sqrt_2pi, sqrt_2, one, one_half, zero, max_num_stdevs, eps 
    ...
    elsewhere
        ...
        cloud_frac_i = one_half * ( one + erf( zeta_i / sqrt_2 )  ) !!! UNRESOLVED !!! erf
  ...
  subroutine calc_xprcp_component( wm, rtm, thlm, rcm,                & ! In
    ...
    !
      ...
      USE constants_clubb, ONLY: sqrt_2pi, two, chi_tol 
