...
module mono_flux_limiter
    ...
    PUBLIC monotonic_turbulent_flux_limit, calc_turb_adv_range 

    PRIVATE mfl_xm_lhs, mfl_xm_rhs, mfl_xm_solve, mean_vert_vel_up_down 
  ...
  subroutine monotonic_turbulent_flux_limit( solve_type, dt, xm_old, &
    ...
    !
      ...
      USE constants_clubb, ONLY: zero_threshold, eps, fstderr 
      ...
      USE stats_variables, ONLY: stats_zm, stats_zt, iwprtp_mfl, irtm_mfl, iwpthlp_mfl, ithlm_mfl, iupwp_mfl, ium_mfl, ivpwp_mfl, &
      &ivm_mfl, ithlm_old, ithlm_without_ta, ithlm_mfl_min, ithlm_mfl_max, irtm_old, irtm_without_ta, irtm_mfl_min, irtm_mfl_max, &
      &ithlm_enter_mfl, ithlm_exit_mfl, irtm_enter_mfl, irtm_exit_mfl, iwpthlp_mfl_min, iwpthlp_mfl_max, iwpthlp_entermfl, &
      &iwpthlp_exit_mfl, iwprtp_mfl_min, iwprtp_mfl_max, iwprtp_enter_mfl, iwprtp_exit_mfl, l_stats_samp 
...
!                     ! maxima and minima of variable x.
  ...
  subroutine mean_vert_vel_up_down( w_1_zm, w_2_zm, varnce_w_1_zm, varnce_w_2_zm, &
    ...
    !
      ...
      USE constants_clubb, ONLY: sqrt_2pi, sqrt_2 

      USE stats_type_utilities, ONLY: stat_update_var_pt 

      USE stats_variables, ONLY: stats_zm, imean_w_up, imean_w_down, l_stats_samp 
    ...
    do k = 2, gr%nz-1, 1
       ...
       elseif ( w_1_zm(k) - 3._core_rknd*sigma_w_1 >= w_ref ) then
          ...
          erf_cache = erf( (w_ref-w_1_zm(k)) / (sqrt_2*sigma_w_1) ) !!! UNRESOLVED !!! erf
...
!          + w_1(k) * 0.5_core_rknd*( 1.0_core_rknd - erf( (w_ref-w_1(k)) / (sqrt_2*sigma_w_1) ) )
       ...
       elseif ( w_2_zm(k) - 3._core_rknd*sigma_w_2 >= w_ref ) then
          ...
          erf_cache = erf( (w_ref-w_2_zm(k)) / (sqrt_2*sigma_w_2) ) !!! UNRESOLVED !!! erf
