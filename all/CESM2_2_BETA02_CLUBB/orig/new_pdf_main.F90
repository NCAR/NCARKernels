...
module new_pdf_main
  ...
  !-------------------------------------------------------------------------
    ...
    PUBLIC new_pdf_driver 

    PRIVATE calc_responder_var, calc_f_x_zeta_x_setter, calc_f_x_responder 
  ...
  subroutine new_pdf_driver( wm, rtm, thlm, wp2, rtp2, thlp2, Skw,    & ! In
    ...
    !-----------------------------------------------------------------------
      ...
      USE constants_clubb, ONLY: four, two, one, zero, rt_tol, thl_tol, max_mag_correlation 

      USE new_pdf, ONLY: calc_setter_var_params, calc_coef_wp4_implicit, calc_coef_wpxp2_implicit, calc_coefs_wp2xp_semiimpl, &
      &calc_coefs_wpxpyp_semiimpl 

      USE pdf_parameter_module, ONLY: implicit_coefs_terms 

      USE parameters_tunable, ONLY: slope_coef_spread_dg_means_w, pdf_component_stdev_factor_w, coef_spread_dg_means_rt, &
      &coef_spread_dg_means_thl 

      USE model_flags, ONLY: l_explicit_turbulent_adv_wp3, l_explicit_turbulent_adv_wpxp, l_explicit_turbulent_adv_xpyp 
  ...
  subroutine calc_responder_var( xm, xp2, sgn_wpxp, mixt_frac, & ! In
    ...
    !-----------------------------------------------------------------------
      ...
      USE new_pdf, ONLY: calc_limits_f_x_responder, calc_responder_params 
