       ...
       use mpi
        ...
        REAL(KIND=real_kind) temp(np, np), temp2(np, np)
        ...
        real(kind=8) :: usecs, total_usecs, mpi_total_usecs
        total_usecs = 0.0
        ...
        do j=1,np
            do i=1,np
                ...
                elem%tensorVisc(i,j,1,2) = 1.1_real_kind * j
        ...
        ELSE
           n = count(KGEN_laplace/=KGEN_RESULT_laplace)
              where(abs(KGEN_laplace) > 1.0E-15)
                 temp  = ((KGEN_RESULT_laplace-KGEN_laplace)/KGEN_laplace)**2
                 temp2 = (KGEN_RESULT_laplace-KGEN_laplace)**2
              elsewhere
                 temp  = (KGEN_RESULT_laplace-KGEN_laplace)**2
                 temp2 = temp
              endwhere
              nrmsdiff = sqrt(sum(temp)/real(n))
              rmsdiff = sqrt(sum(temp2)/real(n))

              write(*,*) "Normalized RMS : ", nrmsdiff, rmsdiff
...
#ifdef _MPI
        call MPI_Reduce(dt, mpi_total_usecs, 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, iError)
        if (mpiRank == 0) then
