...
interface kgen_perturb_real
    ...
    module procedure kgen_perturb_real8_dim4
end interface
...
subroutine kgen_perturb_real8_dim4(var, pertlim)
    real*8, intent(inout), dimension(:,:,:,:) :: var
    real*8, intent(in) :: pertlim
    integer, allocatable :: rndm_seed(:)
    integer :: rndm_seed_sz
    ...
    do idx1=1,size(var, dim=1)
        do idx2=1,size(var, dim=2)
            do idx3=1,size(var, dim=3)
               do idx4=1,size(var, dim=1)
                call random_number(pertval)
                pertval = 2.0_8*pertlim*(0.5_8 - pertval)
                var(idx1,idx2,idx3,idx4) = var(idx1,idx2,idx3,idx4)*(1.0_8 + pertval)
              end do
            end do
        end do
    end do
