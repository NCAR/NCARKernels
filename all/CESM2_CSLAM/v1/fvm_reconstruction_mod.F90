...
!          gradient(6,i,j) = centroid_stretch(5,i,j)*gradient(6,i,j)
          ...
          gradient(2,i,j) = centroid_stretch(1,i,j)*(-f(i+2,j  ,in)+ 8.0_r8*f(i+1,j  ,in)                 &
                          & - 8.0_r8*f(i-1,j  ,in)+f(i-2,j  ,in))
          gradient(3,i,j) = centroid_stretch(2,i,j)*(-f(i  ,j+2,in)+ 8.0_r8*f(i  ,j+1,in)                 &
                          & - 8.0_r8*f(i  ,j-1,in)+f(i  ,j-2,in))
          gradient(4,i,j) = centroid_stretch(3,i,j)*(-f(i+2,j  ,in)+16.0_r8*f(i+1,j  ,in)-30.0_r8*f(i,j,in)&
                          & +16.0_r8*f(i-1,j  ,in)-f(i-2,j  ,in))+centroid_stretch(6,i,j)*gradient(2,i,j)
          gradient(5,i,j) = centroid_stretch(4,i,j)*(-f(i  ,j+2,in)+16.0_r8*f(i  ,j+1,in)-30.0_r8*f(i,j,in)&
                          & +16.0_r8*f(i  ,j-1,in)-f(i  ,j-2,in))+centroid_stretch(7,i,j)*gradient(3,i,j)
          gradient(6,i,j) = centroid_stretch(5,i,j)*(f(i+1,j+1,in)-       f(i+1,j-1,in)                 - &
                          & f(i-1,j+1,in)+f(i-1,j-1,in))
...
!        end do
        ...
        do j=jy(1,in),jy(2,in)
          do i=jx(1,in),jx(2,in)
            ...
            gradient(6,i,j) = centroid_stretch(5,i,j)*(sign*(f(i+1,j+1,in)-f(i+1,j-1,in)-f(i-1,j+1,in)+f(i-1,j-1,in)))
          end do
        end do
      end do
    case default
...
!    REAL(KIND=r8), dimension(1:2), intent(in) :: centroid
...
!    endif
    ...
    invval= 1.0_r8 / (value - cell_value)
    ! Check against the minimum bound on the reconstruction
    if (value - cell_value > 1.0e-12_r8 * value) then
      phi = (local_max - cell_value) * invval
      if (phi < 0.0_r8) then
        min_phi = phi
      endif
      ! Check against the maximum bound on the reconstruction
    elseif ( value - cell_value < -1.0e-12_r8 * value) then
      phi = (local_min - cell_value) * invval
      if(phi < 0.0_r8) then
        min_phi = phi
      endif
    endif
