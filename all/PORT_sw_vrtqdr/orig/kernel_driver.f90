...
PROGRAM kernel_driver
    ...
    DO kgen_repeat_counter = 0,5 
        ...
        kgen_filepath = "../data/vrtqdr_sw." // trim(adjustl(kgen_counter_conv)) // "." // trim(adjustl(kgen_mpi_rank_conv))
        ...
        WRITE (*,*) "** Verification against '" // trim(adjustl(kgen_filepath)) // "' **"
            ...
            call spcvmc_sw(nlayers, ncol, kgen_unit)

            CLOSE (UNIT=kgen_unit)
        END DO
        WRITE (*, *) ""
        WRITE (*, "(A)") "****************************************************"
        WRITE (*, "(4X,A)") "kernel execution summary: vrtqdr_sw"
        WRITE (*, "(A)") "****************************************************"
