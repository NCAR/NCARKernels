...
module stats_type
  ...
  SUBROUTINE kr_stats_real__core_rknd_dim1_ptr(var, kgen_unit, printvar) 
      ...
      IF (kgen_istrue) THEN 
          ...
          CALL kgen_array_sumcheck("z", kgen_array_sum, REAL(SUM(var, mask=(var .eq. var)), 8), .TRUE.) 
          IF (PRESENT( printvar )) THEN 
  ...
  SUBROUTINE kr_stats_real__stat_rknd_dim4_ptr(var, kgen_unit, printvar) 
      ...
      IF (kgen_istrue) THEN 
          ...
          READ (UNIT = kgen_unit) var 
          CALL kgen_array_sumcheck("accum_field_values", kgen_array_sum, REAL(SUM(var, mask=(var .eq. var)), 8), .TRUE.) 
          IF (PRESENT( printvar )) THEN 
              WRITE (*, *) "** KGEN DEBUG: REAL(SUM(" // printvar // "), 8) **", REAL(SUM(var), 8) 
          END IF   
      END IF   
  ...
  SUBROUTINE kr_stats_integer__stat_nknd_dim4_ptr(var, kgen_unit, printvar) 
      ...
      IF (kgen_istrue) THEN 
          ...
          &kgen_bound(1,4):kgen_bound(2,4))) 
          READ (UNIT = kgen_unit) var 
          CALL kgen_array_sumcheck("accum_num_samples", kgen_array_sum, REAL(SUM(var), 8), .TRUE.) 
          IF (PRESENT( printvar )) THEN 
              WRITE (*, *) "** KGEN DEBUG: REAL(SUM(" // printvar // "), 8) **", REAL(SUM(var), 8) 
          END IF   
      END IF   
