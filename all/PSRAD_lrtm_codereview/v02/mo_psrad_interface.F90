...
! KGEN version: 0.4.4
    ...
    USE mo_spec_sampling, only : read_var_mod5 => kgen_read_var
        ...
        SUBROUTINE psrad_interface(kbdim, klev, nb_sw, kproma, ktrac, tk_sfc, kgen_unit, usecs)
#ifdef _MPI
                use mpi
#endif
            integer, intent(in) :: kgen_unit
            ...
            WRITE(*,*)
            PRINT *, "Elapsed time (usec): ", 1.E6*(stop_clock - start_clock)/REAL(rate_clock*maxiter)
            usecs = 1.E6*REAL(stop_clock - start_clock)/REAL(rate_clock*maxiter)
            !
