...
#ifdef _MPI
        call MPI_Reduce(clock_total, mpi_clock_total, 1, MPI_REAL8, MPI_SUM, 0,MPI_COMM_WORLD, iError)


         if (mpiRank == 0) then
            write(*,*) "Average time per rank : ", mpi_clock_total / real(mpiSize)
         endif
