...
!  real(r8) :: faltndqce
  ...
  real(r8) :: dumni0, dumni0A(mgncol), dumni0A2D(mgncol,nlev)
  real(r8) :: dumns0, dumns0A(mgncol), dumns0A2D(mgncol,nlev)
...
subroutine UpdateTendencies_vec(mgncol,nlev,do_cldice,deltat,fx,fnx,pdelInv,qxtend,nxtend, &
   ...
   real(r8) :: faltndx(mgncol),faltndnx(mgncol)
   real(r8) :: rnstep(mgncol),faltndqxe(mgncol)
   real(r8) :: faltndqxe2(mgncol)
   ...
   present_qvlat = present(qvlat)
     ...
     do n = 1,nstepMax
        ...
        if(present(preci)) then 
          preci = preci+faloutx(:,nlev)/g*rnstep/1000._r8
        endif
