...
subroutine size_dist_param_liq_vec(props, qcic, ncic, rho, pgam, lamc, veclen)
  ...
  type(mghydrometeorprops), intent(in) :: props
  real(r8), dimension(veclen), intent(in) :: qcic
  real(r8), dimension(veclen), intent(inout) :: ncic
  real(r8), dimension(veclen), intent(in) :: rho
  real(r8), dimension(veclen), intent(out) :: pgam
  real(r8), dimension(veclen), intent(out) :: lamc
  type(mghydrometeorprops) :: props_loc
  integer :: i, cnt
  real(r8) :: tmp(veclen),pgamp1(veclen)
  real(r8) :: shapeC(veclen),lbnd(veclen),ubnd(veclen)
...
subroutine ice_deposition_sublimation(t, qv, qi, ni, &
  ...
  call size_dist_param_basic_vec(mg_ice_props, qiic, niic, lami,mgncol,n0i)
...
#endif
  ! do the calculation every then mask out the results where precip fraction > cloud fraction
  call calc_ab(t, qvl, xxlv, abr,mgncol)
  do i=1,mgncol
...
!     if (precip_frac(i) > dum(i)) then
  ...
  call calc_ab(t, qvi, xxls, abs, mgncol)
  do i=1,mgncol
        ! sublimation of snow
...
!     end if
  enddo
  ! the case where precip fraction > cloud fraction does not hold
  where(.not.cond1) 
     prds = 0._r8
     pre  = 0._r8
  end where
