...
subroutine micro_mg_tend ( &
  ...
  real(rkind_comp) :: dumni0
  real(rkind_comp) :: dumns0
     ...
     do k=1,nlev
        do i=1,mgncol
           if (dumi(i,k).ge.qsmall) then
              ...
              sadice(i,k) = 2._rkind_comp*pi*(lami(i,k)**(-3))*dumni0*rho(i,k)*1.e-2_rkind_comp  ! m2/m3 -> cm2/cm3
  ...
  do k=1,nlev
     do i=1,mgncol
        if (dums(i,k).ge.qsmall) then
           ...
           sadsnow(i,k) = 2._rkind_comp*pi*(lams(i,k)**(-3))*dumns0*rho(i,k)*1.e-2_rkind_comp  ! m2/m3 -> cm2/cm3
