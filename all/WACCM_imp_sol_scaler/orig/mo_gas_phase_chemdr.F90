...
!
    ...
    REAL(KIND=r8) :: reaction_rates(ncol,pver,max(1,rxntot))
    REAL(KIND=r8) :: het_rates(ncol,pver,max(1,gas_pcnst))

    INTEGER :: ltrop_sol(pcols)
...
!
    ...
    IF (kgen_istrue) THEN
        ...
        CALL kgen_array_sumcheck("invariants", kgen_array_sum, REAL(SUM(invariants), 8), .TRUE.)
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) kgen_array_sum
    ...
    IF (kgen_istrue) THEN
        ...
        CALL kgen_array_sumcheck("reaction_rates", kgen_array_sum, REAL(SUM(reaction_rates), 8), .TRUE.)
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) kgen_array_sum
        READ (UNIT = kgen_unit) het_rates
        CALL kgen_array_sumcheck("het_rates", kgen_array_sum, REAL(SUM(het_rates), 8), .TRUE.)
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) kgen_array_sum
        READ (UNIT = kgen_unit) ltrop_sol
        CALL kgen_array_sumcheck("ltrop_sol", kgen_array_sum, REAL(SUM(ltrop_sol), 8), .TRUE.)
    END IF 
    READ (UNIT = kgen_unit) kgen_istrue
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) kgen_array_sum
        READ (UNIT = kgen_unit) o3s_loss
        CALL kgen_array_sumcheck("o3s_loss", kgen_array_sum, REAL(SUM(o3s_loss), 8), .TRUE.)
    END IF 
    ...
    IF (kgen_istrue) THEN
        READ (UNIT = kgen_unit) kgen_array_sum
        READ (UNIT = kgen_unit) kgenref_o3s_loss
        CALL kgen_array_sumcheck("kgenref_o3s_loss", kgen_array_sum, REAL(SUM(kgenref_o3s_loss), 8), .TRUE.)
    END IF 
...
!
    ...
    CALL kv_gas_phase_chemdr_real__r8_dim2("o3s_loss", check_status, o3s_loss, kgenref_o3s_loss)
    WRITE (*, *) ""
    ...
    !verify state subroutine for kv_gas_phase_chemdr_real__r8_dim2
    RECURSIVE SUBROUTINE kv_gas_phase_chemdr_real__r8_dim2(varname, check_status, var, kgenref_var)
        CHARACTER(LEN=*), INTENT(IN) :: varname
        TYPE(check_t), INTENT(INOUT) :: check_status
        REAL(KIND=r8), INTENT(IN), DIMENSION(:,:) :: var, kgenref_var
        INTEGER :: check_result
    ...
    END SUBROUTINE kv_gas_phase_chemdr_real__r8_dim2
