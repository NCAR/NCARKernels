...
! !USES:
      ...
      USE esmf_alarmmod, ONLY: kw_esmf_alarmmod_esmf_alarm
      implicit none
!
!------------------------------------------------------------------------------
! !PRIVATE TYPES:
...
!==============================================================================
      ...
      PUBLIC kw_esmf_alarmmod_esmf_alarm
      PUBLIC kw_esmf_clockmod_esmf_clockint
      PUBLIC kw_esmf_clockmod_esmf_clock
      contains
...
!------------------------------------------------------------------------------
      ...
      !read state subroutine for kw_esmf_clockmod_esmf_clockint
      RECURSIVE SUBROUTINE kw_esmf_clockmod_esmf_clockint(var, kgen_unit, printvar)
          TYPE(esmf_clockint), INTENT(IN) :: var
          ...
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_timeintervalmod_esmf_timeinterval(var%timestep, kgen_unit, printvar // "%timestep")
          ELSE
              CALL kw_esmf_timeintervalmod_esmf_timeinterval(var%timestep, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%starttime, kgen_unit, printvar // "%starttime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%starttime, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%stoptime, kgen_unit, printvar // "%stoptime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%stoptime, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%reftime, kgen_unit, printvar // "%reftime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%reftime, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%currtime, kgen_unit, printvar // "%currtime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%currtime, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%prevtime, kgen_unit, printvar // "%prevtime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%prevtime, kgen_unit)
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%advancecount
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%advancecount **" // NEW_LINE("A"), var%advancecount
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%clockmutex
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%clockmutex **" // NEW_LINE("A"), var%clockmutex
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%numalarms
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%numalarms **" // NEW_LINE("A"), var%numalarms
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_clockint_esmf_alarm__esmf_alarm_dim1_ptr(var%alarmlist, kgen_unit, printvar // "%alarmlist")
          ELSE
              CALL kw_esmf_clockint_esmf_alarm__esmf_alarm_dim1_ptr(var%alarmlist, kgen_unit)
          END IF 
          
      END SUBROUTINE kw_esmf_clockmod_esmf_clockint
      
      !write state subroutine for kw_esmf_clockint_esmf_alarm__esmf_alarm_dim1_ptr
      SUBROUTINE kw_esmf_clockint_esmf_alarm__esmf_alarm_dim1_ptr(var, kgen_unit, printvar)
          TYPE(esmf_alarm), INTENT(IN), POINTER, DIMENSION(:) :: var
          INTEGER, INTENT(IN) :: kgen_unit
          CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
          LOGICAL :: kgen_istrue
          INTEGER :: idx1
          ...
          IF (kgen_istrue) THEN
              ...
              DO idx1=LBOUND(var,1), UBOUND(var,1)
                  IF (PRESENT( printvar )) THEN
                      CALL kw_esmf_alarmmod_esmf_alarm(var(idx1), kgen_unit, printvar // "(idx1)")
                  ELSE
                      CALL kw_esmf_alarmmod_esmf_alarm(var(idx1), kgen_unit)
                  END IF 
              END DO 
          END IF 
      END SUBROUTINE kw_esmf_clockint_esmf_alarm__esmf_alarm_dim1_ptr
      
      !read state subroutine for kw_esmf_clockmod_esmf_clock
      RECURSIVE SUBROUTINE kw_esmf_clockmod_esmf_clock(var, kgen_unit, printvar)
          TYPE(esmf_clock), INTENT(IN) :: var
          INTEGER, INTENT(IN) :: kgen_unit
          CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
          LOGICAL :: kgen_istrue
          REAL(KIND=8) :: kgen_array_sum
          ...
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_clock_esmf_clockint__esmf_clockint_ptr(var%clockint, kgen_unit, printvar // "%clockint")
          ELSE
              CALL kw_esmf_clock_esmf_clockint__esmf_clockint_ptr(var%clockint, kgen_unit)
          END IF 
          
      END SUBROUTINE kw_esmf_clockmod_esmf_clock
      
      !write state subroutine for kw_esmf_clock_esmf_clockint__esmf_clockint_ptr
      SUBROUTINE kw_esmf_clock_esmf_clockint__esmf_clockint_ptr(var, kgen_unit, printvar)
          TYPE(esmf_clockint), INTENT(IN), POINTER :: var
          INTEGER, INTENT(IN) :: kgen_unit
          CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
          LOGICAL :: kgen_istrue
          REAL(KIND=8) :: kgen_array_sum
          ...
          IF (kgen_istrue) THEN
              IF (PRESENT( printvar )) THEN
                  CALL kw_esmf_clockmod_esmf_clockint(var, kgen_unit, printvar // " clockint ")
              ELSE
                  CALL kw_esmf_clockmod_esmf_clockint(var, kgen_unit)
              END IF 
          END IF 
      END SUBROUTINE kw_esmf_clock_esmf_clockint__esmf_clockint_ptr
