...
!==============================================================================
      ...
      PUBLIC kw_esmf_alarmmod_esmf_alarmint
...
!------------------------------------------------------------------------------
      ...
      !read state subroutine for kw_esmf_alarmmod_esmf_alarmint
      RECURSIVE SUBROUTINE kw_esmf_alarmmod_esmf_alarmint(var, kgen_unit, printvar)
          TYPE(esmf_alarmint), INTENT(IN) :: var
          ...
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_timeintervalmod_esmf_timeinterval(var%ringinterval, kgen_unit, printvar // "%ringinterval")
          ELSE
              CALL kw_esmf_timeintervalmod_esmf_timeinterval(var%ringinterval, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%ringtime, kgen_unit, printvar // "%ringtime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%ringtime, kgen_unit)
          END IF 
          
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_shrtimemod_esmf_time(var%prevringtime, kgen_unit, printvar // "%prevringtime")
          ELSE
              CALL kw_esmf_shrtimemod_esmf_time(var%prevringtime, kgen_unit)
          END IF 
          ...
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%id
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%id **" // NEW_LINE("A"), var%id
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%alarmmutex
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%alarmmutex **" // NEW_LINE("A"), var%alarmmutex
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%ringing
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%ringing **" // NEW_LINE("A"), var%ringing
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%enabled
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%enabled **" // NEW_LINE("A"), var%enabled
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%ringtimeset
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%ringtimeset **" // NEW_LINE("A"), var%ringtimeset
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%ringintervalset
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%ringintervalset **" // NEW_LINE("A"), var%ringintervalset
          END IF 
          
          kgen_istrue = .TRUE.
          WRITE (UNIT = kgen_unit) var%stoptimeset
          IF (PRESENT( printvar )) THEN
              WRITE (*, *) "** KGEN DEBUG: " // printvar // "%stoptimeset **" // NEW_LINE("A"), var%stoptimeset
          END IF 
          
      END SUBROUTINE kw_esmf_alarmmod_esmf_alarmint
      
      !read state subroutine for kw_esmf_alarmmod_esmf_alarm
      RECURSIVE SUBROUTINE kw_esmf_alarmmod_esmf_alarm(var, kgen_unit, printvar)
          TYPE(esmf_alarm), INTENT(IN) :: var
          INTEGER, INTENT(IN) :: kgen_unit
          CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
          LOGICAL :: kgen_istrue
          REAL(KIND=8) :: kgen_array_sum
          ...
          IF (PRESENT( printvar )) THEN
              CALL kw_esmf_alarm_esmf_alarmint__esmf_alarmint_ptr(var%alarmint, kgen_unit, printvar // "%alarmint")
          ELSE
              CALL kw_esmf_alarm_esmf_alarmint__esmf_alarmint_ptr(var%alarmint, kgen_unit)
          END IF 
          
      END SUBROUTINE kw_esmf_alarmmod_esmf_alarm
      
      !write state subroutine for kw_esmf_alarm_esmf_alarmint__esmf_alarmint_ptr
      SUBROUTINE kw_esmf_alarm_esmf_alarmint__esmf_alarmint_ptr(var, kgen_unit, printvar)
          TYPE(esmf_alarmint), INTENT(IN), POINTER :: var
          INTEGER, INTENT(IN) :: kgen_unit
          CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
          LOGICAL :: kgen_istrue
          REAL(KIND=8) :: kgen_array_sum
          ...
          IF (kgen_istrue) THEN
              IF (PRESENT( printvar )) THEN
                  CALL kw_esmf_alarmmod_esmf_alarmint(var, kgen_unit, printvar // " alarmint ")
              ELSE
                  CALL kw_esmf_alarmmod_esmf_alarmint(var, kgen_unit)
              END IF 
          END IF 
      END SUBROUTINE kw_esmf_alarm_esmf_alarmint__esmf_alarmint_ptr
