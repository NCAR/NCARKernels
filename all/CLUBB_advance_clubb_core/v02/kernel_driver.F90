...
#ifdef _MPI
        call MPI_Reduce(kgen_total_time, mpi_total_time, 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, iError)
        if (mpiRank == 0) then
          write(*,*) "Average time per call per rank : ", mpi_total_time/REAL(mpiSize) / 9
      endif
