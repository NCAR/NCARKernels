...
module clubb_intr
  ...
  real(rkind_comp), parameter :: &
      host_dx = 100000._rkind_comp, &           ! Host model deltax [m]
      host_dy = 100000._rkind_comp              ! Host model deltay [m]
...
!-------------------------------------------------------------------------------
   ...
   ! --------------- !
      ...
      REAL(KIND=rkind_comp) :: edsclr_in(pverp,edsclr_dim)
      REAL(KIND=rkind_comp) :: wp2_in(pverp)
      REAL(KIND=rkind_comp) :: wp3_in(pverp)
      REAL(KIND=rkind_comp) :: wpthlp_in(pverp)
      REAL(KIND=rkind_comp) :: wprtp_in(pverp)
      REAL(KIND=rkind_comp) :: rtpthlp_in(pverp)
      REAL(KIND=rkind_comp) :: rtp2_in(pverp)
      REAL(KIND=rkind_comp) :: thlp2_in(pverp)
      REAL(KIND=rkind_comp) :: up2_in(pverp)
      REAL(KIND=rkind_comp) :: vp2_in(pverp)
      REAL(KIND=rkind_comp) :: upwp_in(pverp)
      REAL(KIND=rkind_comp) :: vpwp_in(pverp)
      REAL(KIND=rkind_comp) :: thlm_in(pverp)
      REAL(KIND=rkind_comp) :: rtm_in(pverp)
      REAL(KIND=rkind_comp) :: um_in(pverp)
      REAL(KIND=rkind_comp) :: vm_in(pverp)
      REAL(KIND=rkind_comp) :: rho_in(pverp)
      REAL(KIND=rkind_comp) :: rcm_out(pverp)
      REAL(KIND=rkind_comp) :: wprcp_out(pverp)
      REAL(KIND=rkind_comp) :: cloud_frac_out(pverp)
      REAL(KIND=rkind_comp) :: rcm_in_layer_out(pverp)
      REAL(KIND=rkind_comp) :: cloud_cover_out(pverp)
      REAL(KIND=rkind_comp) :: thlprcp_out(pverp)
      REAL(KIND=rkind_comp) :: rho_ds_zm(pverp)
      REAL(KIND=rkind_comp) :: rho_ds_zt(pverp)
      REAL(KIND=rkind_comp) :: invrs_rho_ds_zm(pverp)
      REAL(KIND=rkind_comp) :: invrs_rho_ds_zt(pverp)
      REAL(KIND=rkind_comp) :: thv_ds_zm(pverp)
      REAL(KIND=rkind_comp) :: thv_ds_zt(pverp)
      REAL(KIND=rkind_comp) :: rfrzm(pverp)
      REAL(KIND=rkind_comp) :: radf(pverp)
      REAL(KIND=rkind_comp) :: wprtp_forcing(pverp)
      REAL(KIND=rkind_comp) :: wpthlp_forcing(pverp)
      REAL(KIND=rkind_comp) :: rtp2_forcing(pverp)
      REAL(KIND=rkind_comp) :: thlp2_forcing(pverp)
      REAL(KIND=rkind_comp) :: rtpthlp_forcing(pverp)
      REAL(KIND=rkind_comp) :: ice_supersat_frac(pverp)
      REAL(KIND=rkind_comp) :: fcor
      REAL(KIND=rkind_comp) :: sfc_elevation
      REAL(KIND=rkind_comp) :: thlm_forcing(pverp)
      REAL(KIND=rkind_comp) :: rtm_forcing(pverp)
      REAL(KIND=rkind_comp) :: um_forcing(pverp)
      REAL(KIND=rkind_comp) :: vm_forcing(pverp)
      REAL(KIND=rkind_comp) :: wm_zm(pverp)
      REAL(KIND=rkind_comp) :: wm_zt(pverp)
      REAL(KIND=rkind_comp) :: p_in_pa(pverp)
      REAL(KIND=rkind_comp) :: rho_zm(pverp)
      REAL(KIND=rkind_comp) :: exner(pverp)
      REAL(KIND=rkind_comp) :: wpthlp_sfc
      REAL(KIND=rkind_comp) :: wprtp_sfc
      REAL(KIND=rkind_comp) :: upwp_sfc
      REAL(KIND=rkind_comp) :: vpwp_sfc
      REAL(KIND=rkind_comp) :: sclrm_forcing(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: wpsclrp_sfc(sclr_dim)
      REAL(KIND=rkind_comp) :: edsclrm_forcing(pverp,edsclr_dim)
      REAL(KIND=rkind_comp) :: wpedsclrp_sfc(edsclr_dim)
      REAL(KIND=rkind_comp) :: sclrm(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: wpsclrp(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: sclrp2(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: sclrprtp(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: sclrpthlp(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: hydromet(pverp,hydromet_dim)
      REAL(KIND=rkind_comp) :: wphydrometp(pverp,hydromet_dim)
      REAL(KIND=rkind_comp) :: wp2hmp(pverp,hydromet_dim)
      REAL(KIND=rkind_comp) :: rtphmp_zt(pverp,hydromet_dim)
      REAL(KIND=rkind_comp) :: thlphmp_zt (pverp,hydromet_dim)
      REAL(KIND=rkind_comp) :: khzm_out(pverp)
      REAL(KIND=rkind_comp) :: khzt_out(pverp)
      REAL(KIND=rkind_comp) :: qclvar_out(pverp)
   ...
   ! --------------- !
      ...
      REAL(KIND=rkind_comp) :: kgenref_edsclr_in(pverp,edsclr_dim)
      REAL(KIND=rkind_comp) :: kgenref_wp2_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_wp3_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_wpthlp_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_wprtp_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_rtpthlp_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_rtp2_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_thlp2_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_up2_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_vp2_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_upwp_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_vpwp_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_thlm_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_rtm_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_um_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_vm_in(pverp)
      REAL(KIND=rkind_comp) :: kgenref_rcm_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_wprcp_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_cloud_frac_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_rcm_in_layer_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_cloud_cover_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_thlprcp_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_ice_supersat_frac(pverp)
      REAL(KIND=rkind_comp) :: kgenref_sclrm(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: kgenref_wpsclrp(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: kgenref_sclrp2(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: kgenref_sclrprtp(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: kgenref_sclrpthlp(pverp,sclr_dim)
      REAL(KIND=rkind_comp) :: kgenref_khzm_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_khzt_out(pverp)
      REAL(KIND=rkind_comp) :: kgenref_qclvar_out(pverp)
...
!                     rtpthlp_in = rtpthlp_in + rtpthlp_mc_out * dtime
   ...
   !  Output CLUBB history here
         ...
         CALL kv_clubb_tend_cam_real__rkind_comp_dim2("edsclr_in", check_status, edsclr_in, kgenref_edsclr_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("wp2_in", check_status, wp2_in, kgenref_wp2_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("wp3_in", check_status, wp3_in, kgenref_wp3_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("wpthlp_in", check_status, wpthlp_in, kgenref_wpthlp_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("wprtp_in", check_status, wprtp_in, kgenref_wprtp_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("rtpthlp_in", check_status, rtpthlp_in, kgenref_rtpthlp_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("rtp2_in", check_status, rtp2_in, kgenref_rtp2_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("thlp2_in", check_status, thlp2_in, kgenref_thlp2_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("up2_in", check_status, up2_in, kgenref_up2_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("vp2_in", check_status, vp2_in, kgenref_vp2_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("upwp_in", check_status, upwp_in, kgenref_upwp_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("vpwp_in", check_status, vpwp_in, kgenref_vpwp_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("thlm_in", check_status, thlm_in, kgenref_thlm_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("rtm_in", check_status, rtm_in, kgenref_rtm_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("um_in", check_status, um_in, kgenref_um_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("vm_in", check_status, vm_in, kgenref_vm_in)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("rcm_out", check_status, rcm_out, kgenref_rcm_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("wprcp_out", check_status, wprcp_out, kgenref_wprcp_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("cloud_frac_out", check_status, cloud_frac_out, kgenref_cloud_frac_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("rcm_in_layer_out", check_status, rcm_in_layer_out, kgenref_rcm_in_layer_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("cloud_cover_out", check_status, cloud_cover_out, kgenref_cloud_cover_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("thlprcp_out", check_status, thlprcp_out, kgenref_thlprcp_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("ice_supersat_frac", check_status, ice_supersat_frac, kgenref_ice_supersat_frac)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("khzm_out", check_status, khzm_out, kgenref_khzm_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("khzt_out", check_status, khzt_out, kgenref_khzt_out)
         CALL kv_clubb_tend_cam_real__rkind_comp_dim1("qclvar_out", check_status, qclvar_out, kgenref_qclvar_out)
         ...
         !verify state subroutine for kv_clubb_tend_cam_real__rkind_comp_dim2
         RECURSIVE SUBROUTINE kv_clubb_tend_cam_real__rkind_comp_dim2(varname, check_status, var, kgenref_var)
             CHARACTER(LEN=*), INTENT(IN) :: varname
             TYPE(check_t), INTENT(INOUT) :: check_status
         ...
         END SUBROUTINE kv_clubb_tend_cam_real__rkind_comp_dim2
         
         !verify state subroutine for kv_clubb_tend_cam_real__rkind_comp_dim1
         RECURSIVE SUBROUTINE kv_clubb_tend_cam_real__rkind_comp_dim1(varname, check_status, var, kgenref_var)
             CHARACTER(LEN=*), INTENT(IN) :: varname
             TYPE(check_t), INTENT(INOUT) :: check_status
             REAL(KIND=rkind_comp), INTENT(IN), DIMENSION(:) :: var, kgenref_var
             INTEGER :: check_result
         ...
         END SUBROUTINE kv_clubb_tend_cam_real__rkind_comp_dim1
