...
!$omp threadprivate(rsat)
  ...
  TYPE(pdf_parameter), allocatable, dimension(:), public :: pdf_params_zm

!$omp threadprivate(pdf_params_zm, pdf_params_zm_frz)

  REAL(KIND=core_rknd), allocatable, dimension(:), public :: frad
...
!$omp threadprivate(wp3_on_wp2, wp3_on_wp2_zt)
  ...
  REAL(KIND=core_rknd), allocatable, dimension(:) :: kgenref_wp2_zt, kgenref_thlp2_zt, kgenref_wpthlp_zt, kgenref_wprtp_zt, kgenref_rtp2_zt, kgenref_rtpthlp_zt, kgenref_up2_zt, kgenref_vp2_zt, kgenref_upwp_zt, kgenref_vpwp_zt
...
!------------------------------------------------------------------------
  ...
  SUBROUTINE kr_externs_in_variables_diagnostic_module(kgen_unit)
      ...
      CALL kr_variables_diagnostic_module_real__core_rknd_dim2_unused(sclrpthvp, kgen_unit)
      CALL kr_variables_diagnostic_module_real__core_rknd_dim2_unused(wpsclrp2, kgen_unit)
      CALL kr_variables_diagnostic_module_real__core_rknd_dim2_unused(sclrprcp, kgen_unit)
      CALL kr_variables_diagnostic_module_real__core_rknd_dim2_unused(wpsclrprtp, kgen_unit)
      CALL kr_variables_diagnostic_module_real__core_rknd_dim2_unused(wpsclrpthlp, kgen_unit)
      CALL kr_variables_diagnostic_module_real__core_rknd_dim2_unused(wp2sclrp, kgen_unit)
  ...
  SUBROUTINE kr_variables_diagnostic_module_real__core_rknd_dim1(var, kgen_unit, printvar)
      ...
      LOGICAL :: kgen_istrue
      REAL(KIND=core_rknd) :: kgen_array_sum
      INTEGER :: idx1
      ...
      IF (kgen_istrue) THEN
          ...
          var = real(var_io,kind=core_rknd)
          deallocate(var_io)
          CALL kgen_array_sumcheck("var", kgen_array_sum, REAL(SUM(var), core_rkind), .TRUE.)
  ...
  !hacked up routine to allocate variables that aren't used, but still
  !passed as arguments
  SUBROUTINE kr_variables_diagnostic_module_real__core_rknd_dim2_unused(var, kgen_unit, printvar)
      ...
      IF (kgen_istrue) THEN
       ...
       ELSE
         ...
         allocate(var(gr%nz, sclr_dim))
       endif
  END SUBROUTINE kr_variables_diagnostic_module_real__core_rknd_dim2_unused
  ...
  SUBROUTINE kv_externs_variables_diagnostic_module(check_status)
      ...
      CALL kv_variables_diagnostic_module_real__core_rknd_dim1("sigma_sqd_w_zt", check_status, sigma_sqd_w_zt, kgenref_sigma_sqd_w_zt)
      ...
      CALL kv_variables_diagnostic_module_real__core_rknd_dim1("wp3_on_wp2_zt", check_status, wp3_on_wp2_zt, kgenref_wp3_on_wp2_zt)
