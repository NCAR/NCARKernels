         ...
         DO WHILE( kgen_cur_rank < kgen_mpi_size )
             IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
                 IF (ANY(kgen_counter == kgen_counter_at)) THEN
                     ...
                     kgen_filepath = "/glade/u/home/youngsun/trepo/temp/kgen_system_test/branches/initial/test_cases/CLUBB_adv_clubb_core/kernel/advance_clubb_core." // TRIM(ADJUSTL(kgen_counter_conv)) // "." // TRIM(ADJUSTL(kgen_mpi_rank_conv))
                     kgen_unit = kgen_get_newunit()
                     OPEN (UNIT=kgen_unit, FILE=kgen_filepath, STATUS="REPLACE", ACCESS="STREAM", FORM="UNFORMATTED", ACTION="WRITE", CONVERT="BIG_ENDIAN", IOSTAT=kgen_ierr)
                     IF (kgen_ierr /= 0) THEN
                     ...
                     IF (kgen_istrue) THEN
                         ...
                         WRITE (*, *) "** KGEN DEBUG: " // "REAL(SUM(khzt_out), 8) **", REAL(SUM(khzt_out), 8)
                     END IF 
