...
! KGEN version: 0.4.13
    ...
    USE rrtmg_state, ONLY : kgen_verify_mod6 => kgen_verify
        ...
        SUBROUTINE radiation_tend(fsns, fsnt, fsds, state, cam_out, cam_in, kgen_unit, usecs)
                USE kgen_utils_mod, ONLY : kgen_dp, check_t, kgen_init_check, kgen_print_check
#ifdef _MPI
                use mpi
#endif
...
!                            PRINT *, TRIM(kname), ": Total time (sec): ", (stop_clock - start_clock)/REAL(rate_clock)
                           ...
                           usecs = 1.0e6*(stop_clock - start_clock)/REAL(rate_clock*maxiter)
