...
PROGRAM kernel_driver
    ...
    REAL(KIND=r8) :: usecs, total_usecs

    ! mpi:
    real(kind=r8) :: mpi_total_usecs
    integer :: iError, mpiSize, mpiRank
    ...
    DO kgen_repeat_counter = 0, 1
        ...
        call radiation_tend(fsns, fsnt, fsds, state, cam_out, cam_in, kgen_unit, usecs)
      ...
      if (mpiRank == 0) then
          print *, "Average time per rank: ", mpi_total_usecs / real(mpiSize)
       endif
       call MPI_Finalize(iError)
#else
        print *, "Total time for kernel: ", total_usecs 
#endif
