        ...
        DO WHILE( kgen_cur_rank < kgen_mpi_size )
            IF (ANY(kgen_mpi_rank == kgen_mpi_rank_at) .AND. kgen_cur_rank == kgen_mpi_rank) THEN
                IF (ANY(kgen_counter == kgen_counter_at)) THEN
                    ...
                    kgen_filepath = "/glade/u/home/youngsun/trepo/temp/kgen_system_test/branches/initial/test_cases/CLUBB_pdf_closure/kernel/pdf_closure." // TRIM(ADJUSTL(kgen_counter_conv)) // "." // TRIM(ADJUSTL(kgen_mpi_rank_conv))
                    ...
                    WRITE (UNIT = kgen_unit) hydromet_dim
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(p_in_pa, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(exner, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(thv_ds_zt, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(wm_zt, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim2(wp2hmp, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim2(rtphmp_zt, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim2(thlphmp_zt, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(wp3, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(rtm, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(wprtp, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(thlm, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(wpthlp, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim2(sclrm, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(rcm, kgen_unit)
                    CALL kw_kgen_subpname_0(pdf_params, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(cloud_frac, kgen_unit)
                    CALL kw_advance_clubb_core_real__core_rknd_dim1(ice_supersat_frac, kgen_unit)
                    ...
                    WRITE (UNIT = kgen_unit) err_code_pdf_closure
                    IF (SIZE(wpsclrp_zt)==1) THEN
                        IF (UBOUND(wpsclrp_zt, 1)<LBOUND(wpsclrp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(wpsclrp_zt, 1)==0 .AND. LBOUND(wpsclrp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(wpsclrp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    ...
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(wpsclrp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) wpsclrp_zt
                    END IF 
                    IF (SIZE(sclrp2_zt)==1) THEN
                        IF (UBOUND(sclrp2_zt, 1)<LBOUND(sclrp2_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(sclrp2_zt, 1)==0 .AND. LBOUND(sclrp2_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(sclrp2_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(sclrp2_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) sclrp2_zt
                    END IF 
                    IF (SIZE(sclrprtp_zt)==1) THEN
                        IF (UBOUND(sclrprtp_zt, 1)<LBOUND(sclrprtp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(sclrprtp_zt, 1)==0 .AND. LBOUND(sclrprtp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(sclrprtp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(sclrprtp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) sclrprtp_zt
                    END IF 
                    IF (SIZE(sclrpthlp_zt)==1) THEN
                        IF (UBOUND(sclrpthlp_zt, 1)<LBOUND(sclrpthlp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(sclrpthlp_zt, 1)==0 .AND. LBOUND(sclrpthlp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(sclrpthlp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(sclrpthlp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) sclrpthlp_zt
                    END IF 
                    IF (SIZE(wphydrometp_zt)==1) THEN
                        IF (UBOUND(wphydrometp_zt, 1)<LBOUND(wphydrometp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(wphydrometp_zt, 1)==0 .AND. LBOUND(wphydrometp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(wphydrometp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(wphydrometp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) wphydrometp_zt
                    END IF 
                    IF (SIZE(wp4_zt)==1) THEN
                        IF (UBOUND(wp4_zt, 1)<LBOUND(wp4_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(wp4_zt, 1)==0 .AND. LBOUND(wp4_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(wp4_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(wp4_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) wp4_zt
                    END IF 
                    IF (SIZE(wpthvp_zt)==1) THEN
                        IF (UBOUND(wpthvp_zt, 1)<LBOUND(wpthvp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(wpthvp_zt, 1)==0 .AND. LBOUND(wpthvp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(wpthvp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(wpthvp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) wpthvp_zt
                    END IF 
                    IF (SIZE(rtpthvp_zt)==1) THEN
                        IF (UBOUND(rtpthvp_zt, 1)<LBOUND(rtpthvp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(rtpthvp_zt, 1)==0 .AND. LBOUND(rtpthvp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(rtpthvp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(rtpthvp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) rtpthvp_zt
                    END IF 
                    IF (SIZE(thlpthvp_zt)==1) THEN
                        IF (UBOUND(thlpthvp_zt, 1)<LBOUND(thlpthvp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(thlpthvp_zt, 1)==0 .AND. LBOUND(thlpthvp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(thlpthvp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(thlpthvp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) thlpthvp_zt
                    END IF 
                    IF (SIZE(wprcp_zt)==1) THEN
                        IF (UBOUND(wprcp_zt, 1)<LBOUND(wprcp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(wprcp_zt, 1)==0 .AND. LBOUND(wprcp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(wprcp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(wprcp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) wprcp_zt
                    END IF 
                    IF (SIZE(rtprcp_zt)==1) THEN
                        IF (UBOUND(rtprcp_zt, 1)<LBOUND(rtprcp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(rtprcp_zt, 1)==0 .AND. LBOUND(rtprcp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(rtprcp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(rtprcp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) rtprcp_zt
                    END IF 
                    IF (SIZE(thlprcp_zt)==1) THEN
                        IF (UBOUND(thlprcp_zt, 1)<LBOUND(thlprcp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(thlprcp_zt, 1)==0 .AND. LBOUND(thlprcp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(thlprcp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(thlprcp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) thlprcp_zt
                    END IF 
                    IF (SIZE(rcp2_zt)==1) THEN
                        IF (UBOUND(rcp2_zt, 1)<LBOUND(rcp2_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(rcp2_zt, 1)==0 .AND. LBOUND(rcp2_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(rcp2_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(rcp2_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) rcp2_zt
                    END IF 
                    IF (SIZE(rc_coef_zt)==1) THEN
                        IF (UBOUND(rc_coef_zt, 1)<LBOUND(rc_coef_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(rc_coef_zt, 1)==0 .AND. LBOUND(rc_coef_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(rc_coef_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(rc_coef_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) rc_coef_zt
                    END IF 
                    IF (SIZE(sclrpthvp_zt)==1) THEN
                        IF (UBOUND(sclrpthvp_zt, 1)<LBOUND(sclrpthvp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(sclrpthvp_zt, 1)==0 .AND. LBOUND(sclrpthvp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(sclrpthvp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(sclrpthvp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) sclrpthvp_zt
                    END IF 
                    IF (SIZE(sclrprcp_zt)==1) THEN
                        IF (UBOUND(sclrprcp_zt, 1)<LBOUND(sclrprcp_zt, 1)) THEN
                            kgen_istrue = .FALSE.
                        ELSE IF (UBOUND(sclrprcp_zt, 1)==0 .AND. LBOUND(sclrprcp_zt, 1)==0) THEN
                            kgen_istrue = .FALSE.
                        ELSE
                            kgen_istrue = .TRUE.
                        END IF 
                    ELSE IF (SIZE(sclrprcp_zt)==0) THEN
                        kgen_istrue = .FALSE.
                    ELSE
                        kgen_istrue = .TRUE.
                    END IF 
                    WRITE (UNIT = kgen_unit) kgen_istrue
                    IF (kgen_istrue) THEN
                        kgen_array_sum = REAL(SUM(sclrprcp_zt), 8)
                        WRITE (UNIT = kgen_unit) kgen_array_sum
                        WRITE (UNIT = kgen_unit) sclrprcp_zt
                    END IF 
                END IF 
            END IF 
...
! End Vince Larson's commenting.
	...
	rtm(:) = edsclrm(:,edsclr_dim)
      ...
      !write state subroutine for kw_advance_clubb_core_real__core_rknd_dim1
      SUBROUTINE kw_advance_clubb_core_real__core_rknd_dim1(var, kgen_unit, printvar)
      ...
      END SUBROUTINE kw_advance_clubb_core_real__core_rknd_dim1
      
      !write state subroutine for kw_advance_clubb_core_real__core_rknd_dim2
      SUBROUTINE kw_advance_clubb_core_real__core_rknd_dim2(var, kgen_unit, printvar)
          REAL(KIND=core_rknd), INTENT(IN), DIMENSION(:,:) :: var
          INTEGER, INTENT(IN) :: kgen_unit
          CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: printvar
          LOGICAL :: kgen_istrue
      ...
      END SUBROUTINE kw_advance_clubb_core_real__core_rknd_dim2
      
      !write state subroutine for kw_kgen_subpname_0
      SUBROUTINE kw_kgen_subpname_0(var, kgen_unit, printvar)
      ...
      END SUBROUTINE kw_kgen_subpname_0
