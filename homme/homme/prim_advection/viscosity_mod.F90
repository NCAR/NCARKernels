...
#ifdef _PRIM
public :: biharmonic_wk_scalar, biharmonic_wk_scalar_q
...
subroutine biharmonic_wk_scalar_q(elem,qtens,deriv,edgeq,hybrid,nets,nete)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! compute weak biharmonic operator
!    input:  qtens = Q
!    output: qtens = weak biharmonic of Q
...
type (element_t)     , intent(inout), target :: elem(:)
integer :: nets,nete
real (kind=real_kind), dimension(np,np,nlev,nets:nete) :: qtens
type (EdgeBuffer_t)  , intent(inout) :: edgeq
type (derivative_t)  , intent(in) :: deriv
...
logical var_coef1
   ...
   do ie=nets,nete
#if (defined COLUMN_OPENMP)
!$omp parallel do private(k, lap_p)
#endif
         do k=1,nlev    !  Potential loop inversion (AAM)
            lap_p(:,:)=qtens(:,:,k,ie)
! Original use of qtens on left and right hand sides caused OpenMP errors (AAM)
         ...
         enddo
      call edgeVpack(edgeq, qtens(:,:,:,ie),nlev,0,ie)
   enddo

   call bndry_exchangeV(hybrid,edgeq)

   do ie=nets,nete
      call edgeVunpack(edgeq, qtens(:,:,:,ie),nlev,0,ie)
...
#endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
end subroutine biharmonic_wk_scalar_q
...
subroutine neighbor_minmax_q(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
   ...
   do ie=nets,nete
      kptr = 0
      call  edgeSpack(edgeMinMax,min_neigh(:,ie),nlev,kptr,ie)
      kptr = nlev
      call  edgeSpack(edgeMinMax,max_neigh(:,ie),nlev,kptr,ie)
   enddo
   ...
   do ie=nets,nete
      kptr = 0
      call  edgeSunpackMIN(edgeMinMax,min_neigh(:,ie),nlev,kptr,ie)
      kptr = nlev
      call  edgeSunpackMAX(edgeMinMax,max_neigh(:,ie),nlev,kptr,ie)
      do k=1,nlev
          min_neigh(k,ie) = max(min_neigh(k,ie),0d0)
      enddo
   enddo

end subroutine neighbor_minmax_q
