...
MODULE mo_taugb03
    ...
    IMPLICIT NONE
    PRIVATE
    PUBLIC taugb03_lwr,taugb03_upr
    CONTAINS
    SUBROUTINE taugb03_lwr(ncol, laytrop, nlayers,         &
        ...
        real(kind=wp), intent(out), dimension(ncol,nlayers,ng3) :: taug  ! kernel result
        real(kind=wp), intent(out), dimension(ncol,nlayers),ng3 :: fracs ! kernel result
		...
		END DO
			...
			tauself(icol) = selffac(icol,lay)*(selfref(indself(icol,lay),ig) +&
				selffrac(icol,lay,ig)*(selfref(indself(icol,lay)+1,ig)- selfref(indself(icol,lay),ig)))
			...
			taug(icol,lay,ig) = tau_major(icol,0) + tau_major(icol,1) +tauself(icol) + taufor(icol) + adjcoln2o(icol)*absn2o(icol)
    ...
    END SUBROUTINE taugb03_lwr


    SUBROUTINE taugb03_upr(ncol, laytrop, nlayers,                     &
                rat_h2oco2, colco2, colh2o, coln2o, coldry, &
                fac0, fac1, minorfrac, &
                forfac,forfrac,        &
                jp, jt, ig,        &
		...
		DO icol=1,ncol ! Vectorizes with dir 14.0.2
			...
			absn2o(icol)  = n2om1(icol) + minorfrac(icol,lay)*(n2om2(icol) -n2om1(icol))
			taug(icol,lay,ig) =  tau_major(icol,0) + tau_major(icol,1) + taufor(icol) + adjcoln2o(icol)*absn2o(icol)
			fracs(icol,lay,ig) =  fracrefb(ig,jpl(icol)) + fpl(icol)*(fracrefb(ig,jpl(icol)+1)-fracrefb(ig,jpl(icol)))
		END DO
	   END DO ! end ig loop
        END DO  ! nlayers loop

    END SUBROUTINE taugb03_upr

END MODULE mo_taugb03
