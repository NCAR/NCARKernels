module mo_taugb02
    ...
    IMPLICIT NONE
    PRIVATE
    PUBLIC taugb02_lwr,taugb02_upr
    CONTAINS
    SUBROUTINE taugb02_lwr(ncol, laytrop, nlayers, &
        ...
        ! ======================
            ...
            DO icol=1,ncol  ! Vectorizes with dir 14.0.2
                ind0(icol) = ((jp(icol,lay)-1)*5+(jt(icol,lay)-1))*nspa(2) + 1
                ind1(icol) = (jp(icol,lay)*5+(jt1(icol,lay)-1))*nspa(2) + 1
                scalen2(icol) = colbrd(icol,lay) * scaleminorn2(icol,lay)
                corradj(icol) = 1._r8 - .05_r8 * (pavel(icol,lay) - 100._r8) / 900._r8
    ...
    SUBROUTINE taugb01_upr(ncol, laytrop, nlayers, &
        ...
        DO lay = laytrop+1, nlayers
            !dir$ SIMD
            DO icol=1,ncol  ! Vectorizes with dir 14.0.2
                ind0(icol) = ((jp(icol,lay)-13)*5+(jt(icol,lay)-1))*nspb(2) + 1
                ind1(icol) = ((jp(icol,lay)-12)*5+(jt1(icol,lay)-1))*nspb(2) + 1
                scalen2(icol) = colbrd(icol,lay) * scaleminorn2(icol,lay)
        ...
        END DO
    END SUBROUTINE taugb02_upr
END module mo_taugb02
