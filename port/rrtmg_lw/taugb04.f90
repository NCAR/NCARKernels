...
! the taumol04 calculation in the gas optics part of the PSRAD
! atmospheric
! radiation code.
!
! It is meant to show conceptually how one might "SIMD-ize" swaths of
! the taumol04 code related to calculating the
...
! that is going on in the taug parent taumol04 code, but I don't claim
...
MODULE mo_taugb04
    ...
    IMPLICIT NONE
    PRIVATE
    PUBLIC taugb04_lwr,taugb04_upr
    CONTAINS
    SUBROUTINE taugb04_lwr(ncol, laytrop, nlayers,         &
                rat_h2oco2, colco2, colh2o, coldry, &
                fac0, fac1, minorfrac,                      &
        ...
        real(kind=wp), intent(in), dimension(ncol,nlayers) :: colco2,colh2o,coldry ! these appear to be gas concentrations
        ...
        real(kind=wp), intent(out), dimension(ncol,nlayers,ng4) :: taug  ! kernel result
        real(kind=wp), intent(out), dimension(ncol,nlayers,ng4) :: fracs ! kernel result
        ...
        integer, dimension(ncol) :: js, ind0, ind00, ind01, ind02, jpl
        ...
        ! ======================
            ...
            DO i=0,1         ! loop over specparm types
                ...
                !dir$ SIMD
                DO icol=1,ncol  ! Vectorizes with dir 14.0.2
                    ind0(icol) = ((jp(icol,lay)-(1*MOD(i+1,2)))*5+(jt(icol,i,lay)-1))*nspa(4) +js(icol)
		...
		END DO
			...
			taug(icol,lay,ig) = tau_major(icol,0) + tau_major(icol,1) +tauself(icol) + taufor(icol) 
			fracs(icol,lay,ig) = fracrefa(ig,jpl(icol)) + fpl(icol)*(fracrefa(ig,jpl(icol)+1)-fracrefa(ig,jpl(icol)))
		    END DO
	    END DO
        END DO ! end lower atmosphere loop
    END SUBROUTINE taugb04_lwr


    SUBROUTINE taugb04_upr(ncol, laytrop, nlayers,                     &
                rat_o3co2, colco2, colo3, coldry, &
                fac0, fac1, minorfrac, &
                forfac,forfrac,        &
                jp, jt, ig,        &
                indfor, &
        ...
        integer, intent(in) :: nlayers               ! total number of layers
        real(kind=wp), intent(in), dimension(ncol,0:1,nlayers) :: rat_o3co2,fac0,fac1    ! not sure of ncol depend
        real(kind=wp), intent(in), dimension(ncol,nlayers) :: colco2,colo3,coldry ! these appear to be gas concentrations
        real(kind=wp), intent(in), dimension(ncol,nlayers) :: forfac,forfrac ! not sure of ncol depend
        real(kind=wp), intent(in), dimension(ncol,nlayers) :: minorfrac      ! not sure of ncol depend
        ...
        real(kind=wp), dimension(ncol,0:1) :: tau_major
        real(kind=wp), dimension(ncol) :: taufor,tauself
        integer, dimension(ncol) :: js, ind0, jpl
        ...
        real(kind=wp) :: p2,fk0,fk1,fk2
        real(kind=wp) :: refrat_planck_a, refrat_planck_b
        REAL(KIND=wp), dimension(ngc(4)) :: stratcorrect = (/ 1., 1., 1., 1.,1., 1., 1., .92, .88, 1.07, 1.1, &
                                                        .99, .88, .943 /)
        !dir$ assume_aligned jp:64
        !dir$ assume_aligned jt:64
        !dir$ assume_aligned rat_o3co2:64
        !dir$ assume_aligned colco2:64
        !dir$ assume_aligned colo3:64
        !dir$ assume_aligned fac0:64
        !dir$ assume_aligned fac1:64
        !dir$ assume_aligned taug:64
        ...
        DO lay = laytrop+1, nlayers
            ...
            !dir$ SIMD
                DO icol=1,ncol ! Vectorizes with dir 14.0.2
                    speccomb(icol) = colo3(icol,lay) + rat_o3co2(icol,i,lay)*colco2(icol,lay)
                    specparm(icol) = colo3(icol,lay)/speccomb(icol)
                    IF (specparm(icol) .ge. oneminus) specparm(icol) = oneminus
                    specmult(icol) = 4.0_wp*(specparm(icol))
                    js(icol) = 1 + INT(specmult(icol))
                    fs(icol) = MOD(specmult(icol),1.0_wp)
                    ind0(icol) = ((jp(icol,lay)-13+i)*5+(jt(icol,i,lay)-1))*nspb(4) +js(icol)
                END DO
            ...
            DO icol=1,ncol ! loop vectorizes with directive 14.0.2
                speccomb_planck(icol) = colo3(icol,lay)+refrat_planck_b*colco2(icol,lay)
                specparm_planck(icol) = colo3(icol,lay)/speccomb_planck(icol)
                IF (specparm_planck(icol) .GE. oneminus) specparm_planck(icol)=oneminus
		...
		!dir$ SIMD
		DO icol=1,ncol ! loop vectorizes with directive 14.0.2
			taug(icol,lay,ig) =  (tau_major(icol,0) + tau_major(icol,1) ) * stratcorrect(ig) 
			fracs(icol,lay,ig) =  fracrefb(ig,jpl(icol)) + fpl(icol)*(fracrefb(ig,jpl(icol)+1)-fracrefb(ig,jpl(icol)))
		END DO
	    END DO
        END DO  ! nlayers loop

    END SUBROUTINE taugb04_upr

END MODULE mo_taugb04
